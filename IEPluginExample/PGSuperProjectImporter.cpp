...
CPGSuperProjectImporter2::CPGSuperProjectImporter2()
{
   AFX_MANAGE_STATE(AfxGetStaticModuleState());
   VERIFY(m_Bitmap.LoadBitmap(IDB_IEPLUGIN));
}

CString CPGSuperProjectImporter2::GetItemText() const
{
   return CString("Example Importer (New)");
}

CLSID CPGSuperProjectImporter2::GetCLSID() const
{
   return CLSID_PGSuperProjectImporter;
}

STDMETHODIMP CPGSuperProjectImporter2::GetIcon(HICON* phIcon) const
{
   AFX_MANAGE_STATE(AfxGetStaticModuleState());
   *phIcon = AfxGetApp()->LoadIcon(IDI_IMPORTER);
   return S_OK;
...
STDMETHODIMP CPGSuperProjectImporter2::Import(IBroker* pBroker)
{
   AfxMessageBox(_T("This project importer simulates importing data from an external source by creating a default bridge. A real project importer would connect to an external data source and programatically create a PGSuper model."), MB_OK);
...
void CPGSuperProjectImporter2::BuildBridge(IBroker* pBroker)
{
   GET_IFACE2(pBroker, ILibraryNames, pLibNames);
...
void CPGSuperProjectImporter2::SetSpecification(IBroker* pBroker)
{
   GET_IFACE2(pBroker, ILibraryNames, pLibNames);
...
void CPGSuperProjectImporter2::InitGirderData(IBroker* pBroker)
{
   const auto* pPool = WBFL::LRFD::StrandPool::GetInstance();
   const auto* pStrand = pPool->GetStrand(
      WBFL::Materials::PsStrand::Grade::Gr1860,
...
void CPGSuperProjectImporter2::InitTimelineManager(IBroker* pBroker, CBridgeDescription2& bridge)
{
   // NOTE: The actual timing doesn't matter since we aren't doing a true time-step analysis
   // We will just use reasonable times so the sequence is correct
   CTimelineManager timelineMgr;
