...
Tensile Stress (Service I)(Effective Prestress + Permanent Loads, Final Stress without Live Load) | Enabling this option provides unique tension stress limits for intermediate load stages of spliced girders and splice girder closure joints. And, checks the final girder stress condition without live load. The Service I limit state combination is used for these checks. See notes 1 and 2 below. Enter the coefficient for the tensile stress limit. Check the "But not greater than" box to establish an upper limit on the tensile stress limit. **Note that if this option is enabled, it is also enabled on the Closure Joints tab** (@ref ug_library_dialogs_project_criteria_closure_joints tab), so you must enter values on both tabs.
