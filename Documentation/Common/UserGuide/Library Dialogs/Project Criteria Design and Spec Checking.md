...
Horizontal Transverse Tie | Check the horizontal transverse tie requirements per LRFD 5.9.4.4.3. The tie force is always computed, however if this option is unselected it does not factor into the specification check results. Enter the horizontal tension tie zone length as a fraction of the girder height (this parameterizes the h/4 zone length from LRFD 5.9.4.4.3).
