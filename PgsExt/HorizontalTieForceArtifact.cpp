...
Float64 pgsHorizontalTieForceArtifact::GetBearingReactionLocation(pgsTypes::SideType side) const
{
   ...
   return (m_bb / 2.) * (1. - (Float64)m_nf[side] / (Float64)m_Nw);
}
...
Float64 pgsHorizontalTieForceArtifact::GetTieForce() const
{
   ...
   for (int i = 0; i < 2; i++)
   {
      ...
      Float64 h = m_xp[side] / (m_hb - m_yp[side]) + (m_nf[side] == 0 ? 0. : (m_xp[side] - cb) / m_yp[side]);
      Float64 t = ((Float64)m_nf[side] / (Float64)m_Nw) * h * m_Vu / m_phi; // this is the total force assuming symmetry
      T += t;
   }
   T /= 2.; // average value.
   return T;
