...
void CTimeStepLossEngineer::InitializeTimeStepAnalysis(IntervalIndexType intervalIdx,const pgsPointOfInterest& poi,LOSSDETAILS& details)
{
   ...
   // Material Properties
   Float64 ESegment = (bIsClosure ? pMaterials->GetClosurePourEc(segmentKey,intervalIdx) : pMaterials->GetSegmentEc(segmentKey,intervalIdx));
   Float64 ESlab = pMaterials->GetDeckEc(intervalIdx);
   ...
   tsDetails.Slab.An  = pSectProp->GetNetAd(intervalIdx,poi);
   tsDetails.Slab.In  = pSectProp->GetNetId(intervalIdx,poi);
   tsDetails.Slab.Ytn = pSectProp->GetNetYtd(intervalIdx,poi); // distance from CG of net section to top of slab
   tsDetails.Slab.Ybn = pSectProp->GetNetYbd(intervalIdx,poi); // distance from CG of net section to bottom of slab

   // deck rebar
   if ( intervalIdx < compositeDeckIntervalIdx  )
   {
      // no deck in this interval
      tsDetails.DeckRebar[pgsTypes::drmTop   ].As = 0;
      tsDetails.DeckRebar[pgsTypes::drmTop   ].Ys = 0;

      tsDetails.DeckRebar[pgsTypes::drmBottom].As = 0;
      tsDetails.DeckRebar[pgsTypes::drmBottom].Ys = 0;
   }
   else
   {
   ...
   tsDetails.Slab.ec.reserve(intervalIdx);
   tsDetails.Slab.rc.reserve(intervalIdx);
   ...
   if ( 0 < intervalIdx )
   {
      for ( IntervalIndexType i = 0; i < intervalIdx-1; i++ )
      {
         ...
         Float64 e = details.TimeStepDetails[i].Girder.dee*(Ce-Cs);
         tsDetails.Girder.ec.push_back(e);

         Float64 r = details.TimeStepDetails[i].Girder.der*(Ce-Cs);
         tsDetails.Girder.rc.push_back(r);
         ...
         e = details.TimeStepDetails[i].Slab.dee*(Ce-Cs);
         tsDetails.Slab.ec.push_back(e);

         r = details.TimeStepDetails[i].Slab.der*(Ce-Cs);
         tsDetails.Slab.rc.push_back(r);
      }
      ...
      tsDetails.Girder.e = std::accumulate(tsDetails.Girder.ec.begin(),tsDetails.Girder.ec.end(),0.0); // Tadros 1977, Eqn 3
      tsDetails.Girder.r = std::accumulate(tsDetails.Girder.rc.begin(),tsDetails.Girder.rc.end(),0.0); // Tadros 1977, Eqn 4
...
#endif
      tsDetails.Slab.esi = esi;
      tsDetails.Slab.e = std::accumulate(tsDetails.Slab.ec.begin(),tsDetails.Slab.ec.end(),0.0); // Tadros 1977, Eqn 3
      tsDetails.Slab.r = std::accumulate(tsDetails.Slab.rc.begin(),tsDetails.Slab.rc.end(),0.0); // Tadros 1977, Eqn 4

      // Compute forces to restrain creep and shrinkage deformations
      tsDetails.Girder.PrCreep = -tsDetails.Girder.e * tsDetails.Girder.An * ESegment; // Tadros 1977, Eqn 10
      tsDetails.Girder.MrCreep = -tsDetails.Girder.r * tsDetails.Girder.In * ESegment; // Tadros 1977, Eqn 11
      tsDetails.Girder.PrShrinkage = -tsDetails.Girder.esi * tsDetails.Girder.An * ESegment; // Tadros 1977, Eqn 10

      tsDetails.Slab.PrCreep = -tsDetails.Slab.e * tsDetails.Slab.An * ESlab; // Tadros 1977, Eqn 10
      tsDetails.Slab.MrCreep = -tsDetails.Slab.r * tsDetails.Slab.In * ESlab; // Tadros 1977, Eqn 11
      tsDetails.Slab.PrShrinkage = -tsDetails.Slab.esi * tsDetails.Slab.An * ESlab; // Tadros 1977, Eqn 10
   }

   // STRAND PARAMETERS
   if ( !bIsClosure ) // skip if at a closure pour (no strands in a closure pour)
   {
      for ( int i = 0; i < 3; i++ )
      {
         ...
         {
            // strands not stressed yet, therefore, not relaxation
            tsDetails.Strands[strandType].fr = 0;
         }
         else if ( intervalIdx == stressStrandsIntervalIdx )
         {
            // strands are stressed in this interval... the relax over the duration of the interval. 
            // The effective stress at the start of the interval is fpj
   #if defined IGNORE_RELAXATION_EFFECTS
            tsDetails.Strands[strandType].fr = 0;
   #else
            // ignoring initial relaxation for testing purposes
         ...
         else
         {
            ...
            if ( intervalIdx == releaseIntervalIdx )
            {
               ...
               tsDetails.P += P;
               tsDetails.M += P*(tsDetails.Strands[strandType].Ys - tsDetails.Ytr);
            }
         ...
         tsDetails.Strands[strandType].Pr = EStrand[strandType]*tsDetails.Strands[strandType].As*tsDetails.Strands[strandType].er; // Tadros 1977, Eqn 10
         if ( intervalIdx == releaseIntervalIdx )
            tsDetails.Strands[strandType].Pr *= -1;
      }
   }
   ...
   for ( DuctIndexType ductIdx = 0; ductIdx < nDucts; ductIdx++ )
   {
      ...
      else if ( intervalIdx == stressTendonIntervalIdx )
      {
         ...
         // Negative sign because the force resisted by the girder section is equal and opposite
         // the force in the strand
         Float64 P = -tsTendon.As*tsTendon.fpj; //details.TimeStepDetails[intervalIdx-1].Tendons[ductIdx].fpe;
         tsDetails.P += P;
         tsDetails.M += P*(tsTendon.Ys - tsDetails.Ytr);
      ...
      // force required to resist apparent relaxation strain
      tsTendon.Pr = ETendon*tsTendon.As*tsTendon.er;
   ...
   if ( releaseIntervalIdx <= intervalIdx )
   {
      ...
      tsDetails.Pr[TIMESTEP_CREEP]      = tsDetails.Girder.PrCreep + tsDetails.Slab.PrCreep;
      tsDetails.Pr[TIMESTEP_SHRINKAGE]  = tsDetails.Girder.PrShrinkage + tsDetails.Slab.PrShrinkage;
      tsDetails.Pr[TIMESTEP_RELAXATION] = tsDetails.Strands[pgsTypes::Straight ].Pr
                                        + tsDetails.Strands[pgsTypes::Harped   ].Pr
                                        + tsDetails.Strands[pgsTypes::Temporary].Pr;

      tsDetails.Mr[TIMESTEP_CREEP] = tsDetails.Girder.MrCreep + tsDetails.Girder.PrCreep*(tsDetails.Girder.Ytn - tsDetails.Ytr)
                                   + tsDetails.Slab.MrCreep   - tsDetails.Slab.PrCreep   *(tsDetails.Slab.Ybn + tsDetails.Ytr);
      tsDetails.Mr[TIMESTEP_SHRINKAGE] = tsDetails.Girder.PrShrinkage*(tsDetails.Girder.Ytn - tsDetails.Ytr)
                                       - tsDetails.Slab.PrShrinkage  *(tsDetails.Slab.Ybn + tsDetails.Ytr);

      tsDetails.Mr[TIMESTEP_RELAXATION] = tsDetails.Strands[pgsTypes::Straight ].Pr*(tsDetails.Strands[pgsTypes::Straight ].Ys - tsDetails.Ytr)
                                        + tsDetails.Strands[pgsTypes::Harped   ].Pr*(tsDetails.Strands[pgsTypes::Harped   ].Ys - tsDetails.Ytr)
                                        + tsDetails.Strands[pgsTypes::Temporary].Pr*(tsDetails.Strands[pgsTypes::Temporary].Ys - tsDetails.Ytr);

      for (DuctIndexType ductIdx = 0; ductIdx < nDucts; ductIdx++ )
      {
         tsDetails.Pr[TIMESTEP_RELAXATION] += tsDetails.Tendons[ductIdx].Pr;
         tsDetails.Mr[TIMESTEP_RELAXATION] += tsDetails.Tendons[ductIdx].Pr*(tsDetails.Tendons[ductIdx].Ys - tsDetails.Ytr);
      }
   }
...
//   //pExtLoading->CreateLoadGroup(strLoadGroupName[TIMESTEP_CREEP].c_str());
//   //pExtLoading->CreateLoadGroup(strLoadGroupName[TIMESTEP_SHRINKAGE].c_str());
//   //pExtLoading->CreateLoadGroup(strLoadGroupName[TIMESTEP_RELAXATION].c_str());
//   // Using pre-defined product load types pftCreep, pftShrinkage, pftRelaxation
   ...
   for ( ; iter2 != end; iter1++, iter2++ )
   {
      ...
      for ( int i = 0; i < 3; i++ )
      {
         ...
         // Negative sign because we are applying deformation that is equal
         // and opposite to the initial strain deformation (we are restraining the
         // deformations)
...
void CTimeStepLossEngineer::FinalizeTimeStepAnalysis(IntervalIndexType intervalIdx,const pgsPointOfInterest& poi,LOSSDETAILS& details)
{
   ...
   // Get Service III live load factor
   GET_IFACE(ILoadFactors,pLoadFactors);
   Float64 gammaLL = pLoadFactors->GetLoadFactors()->LLIMmax[pgsTypes::ServiceIII];
   Float64 gammaDC = pLoadFactors->GetLoadFactors()->DCmax[pgsTypes::ServiceIII];
   Float64 gammaDW = pLoadFactors->GetLoadFactors()->DWmax[pgsTypes::ServiceIII];

   // Get Live Load Moment
   Float64 Mmin, Mmax;
   pForces->GetCombinedLiveLoadMoment(pgsTypes::lltDesign,intervalIdx,poi,pgsTypes::ContinuousSpan,&Mmin,&Mmax);

   // Get section forces due to externally applied loads in this interval
   Float64 Mdc = pForces->GetMoment(lcDC,intervalIdx,poi,ctIncremental,pgsTypes::ContinuousSpan);
   Float64 Mdw = pForces->GetMoment(lcDW,intervalIdx,poi,ctIncremental,pgsTypes::ContinuousSpan);
   ...
   if ( intervalIdx == storageIntervalIdx )
   {
      ...
      // change in moment.
      Float64 MdcRelease = pForces->GetMoment(lcDC,releaseIntervalIdx,poi,ctIncremental,pgsTypes::ContinuousSpan);
      Mdc -= MdcRelease;

      Float64 MdwRelease = pForces->GetMoment(lcDW,releaseIntervalIdx,poi,ctIncremental,pgsTypes::ContinuousSpan);
      Mdw -= MdwRelease;
   }
   else if ( intervalIdx == erectionIntervalIdx )
   {
      ...
      Float64 MdcStorage = pForces->GetMoment(lcDC,storageIntervalIdx,poi,ctIncremental,pgsTypes::ContinuousSpan);
      Mdc -= MdcStorage;

      Float64 MdwStorage = pForces->GetMoment(lcDW,storageIntervalIdx,poi,ctIncremental,pgsTypes::ContinuousSpan);
      Mdw -= MdwStorage;
   }

   tsDetails.M += gammaDC*Mdc + gammaDW*Mdw;

   Float64 ESegment  = (bIsClosure ? pMaterials->GetClosurePourEc(segmentKey,intervalIdx) : pMaterials->GetSegmentEc(segmentKey,intervalIdx));
   Float64 EaSegment = (bIsClosure ? pMaterials->GetClosurePourAgeAdjustedEc(segmentKey,intervalIdx) : pMaterials->GetSegmentAgeAdjustedEc(segmentKey,intervalIdx));
   
   Float64 ESlab     = pMaterials->GetDeckEc(intervalIdx);
   Float64 EaSlab    = pMaterials->GetDeckAgeAdjustedEc(intervalIdx);
   ...
   pMaterials->GetSlabRebarProperties(&EDeckRebar,&Fy,&Fu);
   ...
   if ( intervalIdx < releaseIntervalIdx )
   {
      ...
      tsDetails.Girder.de = 0;
      tsDetails.Girder.dr = 0;

      tsDetails.Slab.de = 0;
      tsDetails.Slab.dr = 0;

      tsDetails.Girder.dP = 0;
      tsDetails.Girder.dM = 0;

      tsDetails.Slab.dP = 0;
      tsDetails.Slab.dM = 0;

      tsDetails.DeckRebar[pgsTypes::drmTop].de = 0;
      tsDetails.DeckRebar[pgsTypes::drmTop].dP = 0;

      tsDetails.DeckRebar[pgsTypes::drmBottom].de = 0;
      tsDetails.DeckRebar[pgsTypes::drmBottom].dP = 0;
      ...
      for ( int i = 0; i < 3; i++ )
      {
         ...
         tsDetails.Strands[strandType].de = 0;

         if ( intervalIdx < stressStrandsIntervalIdx )
         {
            // interval is before strands are stressed
            tsDetails.Strands[strandType].P   = 0;
            tsDetails.Strands[strandType].fpe  = 0;
            tsDetails.Strands[strandType].loss = 0;
         }
         else
         {
            // strands are stressed, but not released
            // effective stress at end of interval is fpj - relaxation this interval
            tsDetails.Strands[strandType].P    = tsDetails.Strands[strandType].Pj  + tsDetails.Strands[strandType].Pr;
            tsDetails.Strands[strandType].fpe  = tsDetails.Strands[strandType].fpj - tsDetails.Strands[strandType].fr;
         ...
         tsDetails.Strands[strandType].dFps = -tsDetails.Strands[strandType].loss; // also = -tsDetails.Strands[strandType].dP/tsDetails.Strands[strandType].As;
      }

      for ( DuctIndexType ductIdx = 0; ductIdx < nDucts; ductIdx++ )
      {
         IntervalIndexType stressTendonIntervalIdx  = pIntervals->GetStressTendonInterval(girderKey,ductIdx);
         tsDetails.Tendons[ductIdx].de = 0;

         // tendons cannot be stressed before prestress release so we are clearly before the stressing tendon interval
         tsDetails.Tendons[ductIdx].P   = 0;
         tsDetails.Tendons[ductIdx].fpe = 0;

         tsDetails.Tendons[ductIdx].dP   = tsDetails.Tendons[ductIdx].fr * tsDetails.Tendons[ductIdx].As;
         tsDetails.Tendons[ductIdx].dFps = -tsDetails.Tendons[ductIdx].fr; // -tsDetails.Tendons.dP/tsDetails.Tendons.As;
      }

      tsDetails.Girder.P = 0;
      tsDetails.Girder.M = 0;

      tsDetails.Slab.P = 0;
      tsDetails.Slab.M = 0;

      tsDetails.DeckRebar[pgsTypes::drmTop   ].P = 0;
      tsDetails.DeckRebar[pgsTypes::drmBottom].P = 0;

      tsDetails.Girder.dee = 0;
      tsDetails.Girder.der = 0;

      tsDetails.Slab.dee = 0;
      tsDetails.Slab.der = 0;
   }
   else
   {
      // Compute deformation of the composite cross section due to externally applied loads and restraining forces
      // deformations are axial strain and curvature
      if ( EaSegment*tsDetails.Atr == 0 )
      {
         tsDetails.er = 0;
      }
      else
      {
         Float64 P = tsDetails.P
                   + (tsDetails.Pre[TIMESTEP_CREEP]      - tsDetails.Pr[TIMESTEP_CREEP])
                   + (tsDetails.Pre[TIMESTEP_SHRINKAGE]  - tsDetails.Pr[TIMESTEP_SHRINKAGE])
                   + (tsDetails.Pre[TIMESTEP_RELAXATION] - tsDetails.Pr[TIMESTEP_RELAXATION]);
         tsDetails.er = P/(EaSegment*tsDetails.Atr);
      }

      if ( EaSegment*tsDetails.Itr == 0 )
      {
         tsDetails.rr = 0;
      }
      else
      {
         Float64 M = tsDetails.M
                   + (tsDetails.Mre[TIMESTEP_CREEP]      - tsDetails.Mr[TIMESTEP_CREEP])
                   + (tsDetails.Mre[TIMESTEP_SHRINKAGE]  - tsDetails.Mr[TIMESTEP_SHRINKAGE])
                   + (tsDetails.Mre[TIMESTEP_RELAXATION] - tsDetails.Mr[TIMESTEP_RELAXATION]);
         tsDetails.rr = M/(EaSegment*tsDetails.Itr);
      }

      //
      // Compute deformations at centroid of each cross section elements
      //

      // Segment
      tsDetails.Girder.de = tsDetails.er + tsDetails.rr*(tsDetails.Girder.Ytn - tsDetails.Ytr);
      tsDetails.Girder.dr = tsDetails.rr;

      // Slab & Deck Rebar
      if ( intervalIdx < compositeDeckIntervalIdx )
      {
         // slab is not part of the cross section yet... 
         tsDetails.Slab.de = 0;
         tsDetails.Slab.dr = 0;
      ...
      {
         tsDetails.Slab.de = tsDetails.er - tsDetails.rr*(tsDetails.Slab.Ybn + tsDetails.Ytr);
         tsDetails.Slab.dr = tsDetails.rr;

         tsDetails.DeckRebar[pgsTypes::drmTop   ].de = (tsDetails.DeckRebar[pgsTypes::drmTop   ].As == 0 ? 0 : tsDetails.er - tsDetails.rr*(tsDetails.DeckRebar[pgsTypes::drmTop   ].Ys + tsDetails.Ytr));
         tsDetails.DeckRebar[pgsTypes::drmBottom].de = (tsDetails.DeckRebar[pgsTypes::drmBottom].As == 0 ? 0 : tsDetails.er - tsDetails.rr*(tsDetails.DeckRebar[pgsTypes::drmBottom].Ys + tsDetails.Ytr));
      }
      ...
      {
         TIME_STEP_REBAR& tsRebar(*iter);
         tsRebar.de = tsDetails.er + tsDetails.rr*(tsRebar.Ys - tsDetails.Ytr);
      }

      //
      // Compute component forces due to deformations in this interval
      //

      // Girder
      tsDetails.Girder.dP = tsDetails.Girder.de*EaSegment*tsDetails.Girder.An + tsDetails.Girder.PrCreep + tsDetails.Girder.PrShrinkage;
      tsDetails.Girder.dM = tsDetails.Girder.dr*EaSegment*tsDetails.Girder.In + tsDetails.Girder.MrCreep;

      // Slab
      tsDetails.Slab.dP = tsDetails.Slab.de*EaSlab*tsDetails.Slab.An + tsDetails.Slab.PrCreep + tsDetails.Slab.PrShrinkage;
      tsDetails.Slab.dM = tsDetails.Slab.dr*EaSlab*tsDetails.Slab.In + tsDetails.Slab.MrCreep;

      // Deck Rebar
      tsDetails.DeckRebar[pgsTypes::drmTop   ].dP = tsDetails.DeckRebar[pgsTypes::drmTop   ].de*EDeckRebar*tsDetails.DeckRebar[pgsTypes::drmTop   ].As;
      tsDetails.DeckRebar[pgsTypes::drmBottom].dP = tsDetails.DeckRebar[pgsTypes::drmBottom].de*EDeckRebar*tsDetails.DeckRebar[pgsTypes::drmBottom].As;

      // Girder Rebar
      iter = tsDetails.GirderRebar.begin();
      end  = tsDetails.GirderRebar.end();
      for ( ; iter != end; iter++ )
      {
         TIME_STEP_REBAR& tsRebar(*iter);
         tsRebar.dP = tsRebar.de*EGirderRebar*tsRebar.As;
      }

      // Componenent forces at the end of this interval (= forces at end of previous interval plus forces due
      // to deformation in this interval)

      // Segment
      tsDetails.Girder.P = details.TimeStepDetails[intervalIdx-1].Girder.P + tsDetails.Girder.dP;
      tsDetails.Girder.M = details.TimeStepDetails[intervalIdx-1].Girder.M + tsDetails.Girder.dM;

      // Slab & Deck Rebar
      tsDetails.Slab.P = details.TimeStepDetails[intervalIdx-1].Slab.P + tsDetails.Slab.dP;
      tsDetails.Slab.M = details.TimeStepDetails[intervalIdx-1].Slab.M + tsDetails.Slab.dM;
      ...
      // Girder Rebar
      std::vector<TIME_STEP_REBAR>::iterator begin(tsDetails.GirderRebar.begin());
      iter = begin;
      end  = tsDetails.GirderRebar.end();
      for ( ; iter != end; iter++ )
      {
         TIME_STEP_REBAR& tsRebar(*iter);
         IndexType rebarIdx = iter - begin;
      ...
      for ( int i = 0; i < 3; i++ )
      {
         ...
         // Deformation of strands this interval
         tsDetails.Strands[strandType].de = (tsDetails.Strands[strandType].As == 0 ? 0 : tsDetails.er + tsDetails.rr*(tsDetails.Strands[strandType].Ys - tsDetails.Ytr));

         // change in strand force
         tsDetails.Strands[strandType].dP = tsDetails.Strands[strandType].de*EStrand[strandType]*tsDetails.Strands[strandType].As + tsDetails.Strands[strandType].Pr;
         ...
         tsDetails.Strands[strandType].dFps = (tsDetails.Strands[strandType].As == 0 ? 0 : tsDetails.Strands[strandType].dP/tsDetails.Strands[strandType].As);
         ...
         ATLASSERT(IsEqual(tsDetails.Strands[strandType].loss,details.TimeStepDetails[intervalIdx].Strands[strandType].fpj- tsDetails.Strands[strandType].fpe));
         ...
         {
            tsDetails.Strands[strandType].dFll = 0;
         }
         else
         {
            // live load is on the structure... add elastic effect
            tsDetails.Strands[strandType].dFll = gammaLL*Mmax*(tsDetails.Strands[strandType].Ys - tsDetails.Ytr)/tsDetails.Itr;
         }
         tsDetails.Strands[strandType].fpeLL  = tsDetails.Strands[strandType].fpe + tsDetails.Strands[strandType].dFll;
         tsDetails.Strands[strandType].lossLL = tsDetails.Strands[strandType].loss - tsDetails.Strands[strandType].dFll;
      }
      ...
      for ( DuctIndexType ductIdx = 0; ductIdx < nDucts; ductIdx++ )
      {
         ...
         // Deformation of tendon during this interval
         if ( intervalIdx <= stressTendonIntervalIdx || IsZero(tsDetails.Tendons[ductIdx].As) )
         {
            tsDetails.Tendons[ductIdx].de = 0; // tendon not installed or grouted yet
         }
         else
         {
            tsDetails.Tendons[ductIdx].de = tsDetails.er + tsDetails.rr*(tsDetails.Tendons[ductIdx].Ys - tsDetails.Ytr);
         }
         ...
         {
            tsDetails.Tendons[ductIdx].dP = tsDetails.Tendons[ductIdx].de*ETendon*tsDetails.Tendons[ductIdx].As + tsDetails.Tendons[ductIdx].Pr;
         ...
         {
            tsDetails.Tendons[ductIdx].dFll = 0;
         }
         else
         {
            // live load is on the structure... add elastic effect
            tsDetails.Tendons[ductIdx].dFll = gammaLL*Mmax*(tsDetails.Tendons[ductIdx].Ys - tsDetails.Ytr)/tsDetails.Itr;
         }
         tsDetails.Tendons[ductIdx].fpeLL  = tsDetails.Tendons[ductIdx].fpe + tsDetails.Tendons[ductIdx].dFll;
         tsDetails.Tendons[ductIdx].lossLL = tsDetails.Tendons[ductIdx].loss - tsDetails.Tendons[ductIdx].dFll;
      }

      // Elastic strains in concrete components during this interval
      // (only the elastic portion of the strain undergoes creep in later intervals. the creep portion of the total
      // strain does not undergo creep in later intervals)

      // Girder
      tsDetails.Girder.dee = (ESegment*tsDetails.Girder.An == 0 ? 0 : tsDetails.Girder.dP/(ESegment*tsDetails.Girder.An));
      tsDetails.Girder.der = (ESegment*tsDetails.Girder.In == 0 ? 0 : tsDetails.Girder.dM/(ESegment*tsDetails.Girder.In));

      // Slab
      tsDetails.Slab.dee = (ESlab*tsDetails.Slab.An == 0 ? 0 : tsDetails.Slab.dP/(ESlab*tsDetails.Slab.An));
      tsDetails.Slab.der = (ESlab*tsDetails.Slab.In == 0 ? 0 : tsDetails.Slab.dM/(ESlab*tsDetails.Slab.In));
   }

   // Girder stress at end of interval
   // f = f end of previous interval + change in stress this interval
   if ( intervalIdx == 0 )
   {
      tsDetails.Girder.fTop = (tsDetails.Girder.An == 0 ? 0 : tsDetails.Girder.dP/tsDetails.Girder.An - tsDetails.Girder.dM*tsDetails.Girder.Ytn/tsDetails.Girder.In);
      tsDetails.Girder.fBot = (tsDetails.Girder.An == 0 ? 0 : tsDetails.Girder.dP/tsDetails.Girder.An + tsDetails.Girder.dM*tsDetails.Girder.Ybn/tsDetails.Girder.In);

      tsDetails.Slab.fTop = (tsDetails.Slab.An == 0 ? 0 : tsDetails.Slab.dP/tsDetails.Slab.An - tsDetails.Slab.dM*tsDetails.Slab.Ytn/tsDetails.Slab.In);
      tsDetails.Slab.fBot = (tsDetails.Slab.An == 0 ? 0 : tsDetails.Slab.dP/tsDetails.Slab.An + tsDetails.Slab.dM*tsDetails.Slab.Ybn/tsDetails.Slab.In);
   }
   else
   {
      tsDetails.Girder.fTop = details.TimeStepDetails[intervalIdx-1].Girder.fTop + (tsDetails.Girder.An == 0 ? 0 : tsDetails.Girder.dP/tsDetails.Girder.An - tsDetails.Girder.dM*tsDetails.Girder.Ytn/tsDetails.Girder.In);
      tsDetails.Girder.fBot = details.TimeStepDetails[intervalIdx-1].Girder.fBot + (tsDetails.Girder.An == 0 ? 0 : tsDetails.Girder.dP/tsDetails.Girder.An + tsDetails.Girder.dM*tsDetails.Girder.Ybn/tsDetails.Girder.In);

      tsDetails.Slab.fTop = details.TimeStepDetails[intervalIdx-1].Slab.fTop + (tsDetails.Slab.An == 0 ? 0 : tsDetails.Slab.dP/tsDetails.Slab.An - tsDetails.Slab.dM*tsDetails.Slab.Ytn/tsDetails.Slab.In);
      tsDetails.Slab.fBot = details.TimeStepDetails[intervalIdx-1].Slab.fBot + (tsDetails.Slab.An == 0 ? 0 : tsDetails.Slab.dP/tsDetails.Slab.An + tsDetails.Slab.dM*tsDetails.Slab.Ybn/tsDetails.Slab.In);
   }
...
#pragma Reminder("UPDATE: don't need equilibrium checks in release version")
   ...
   tsDetails.dPext = tsDetails.P;
   tsDetails.dMext = tsDetails.M;

   tsDetails.dPint = tsDetails.Girder.dP 
                   + tsDetails.Slab.dP 
                   + tsDetails.DeckRebar[pgsTypes::drmTop   ].dP 
                   + tsDetails.DeckRebar[pgsTypes::drmBottom].dP
                   - tsDetails.Pre[TIMESTEP_CREEP] - tsDetails.Pre[TIMESTEP_SHRINKAGE] - tsDetails.Pre[TIMESTEP_RELAXATION];

   tsDetails.dMint = tsDetails.Girder.dM + tsDetails.Girder.dP*(tsDetails.Girder.Ytn - tsDetails.Ytr)
                   + tsDetails.Slab.dM   - tsDetails.Slab.dP  *(tsDetails.Slab.Ybn   + tsDetails.Ytr)
                   - tsDetails.DeckRebar[pgsTypes::drmTop   ].dP*(tsDetails.DeckRebar[pgsTypes::drmTop   ].Ys + tsDetails.Ytr)
                   - tsDetails.DeckRebar[pgsTypes::drmBottom].dP*(tsDetails.DeckRebar[pgsTypes::drmBottom].Ys + tsDetails.Ytr)
                   - tsDetails.Mre[TIMESTEP_CREEP] - tsDetails.Mre[TIMESTEP_SHRINKAGE] - tsDetails.Mre[TIMESTEP_RELAXATION];
