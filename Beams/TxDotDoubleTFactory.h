...
namespace PGS
{
   namespace Beams
   {
      class CTxDotDoubleTFactory : public BeamFactory, public BeamFactorySingleton<CTxDotDoubleTFactory>
      {
      public:
         static std::shared_ptr<CTxDotDoubleTFactory> CreateInstance() { return std::shared_ptr<CTxDotDoubleTFactory>(new CTxDotDoubleTFactory()); }
      ...
      private:
         ...
         void DimensionAndPositionBeam(const CPrecastSegmentData* pSegment,const BeamFactory::Dimensions& dimensions, IMultiWeb2* pBeam) const;
      };
   };
};
