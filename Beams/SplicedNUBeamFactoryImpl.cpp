...
INIT_BEAM_FACTORY_SINGLETON(SplicedNUBeamFactory)
...
void SplicedNUBeamFactory::CreateSegmentShape(std::shared_ptr<WBFL::EAF::Broker> pBroker, const CPrecastSegmentData* pSegment, Float64 Xs, pgsTypes::SectionBias sectionBias, IShape** ppShape) const
{
   ...
   {
      Float64 nominalT; beam->get_T(&nominalT);
      Float64 maxDeltaW = (pSegment->WebThickeningWidth - nominalT) / 2.0;
      if (maxDeltaW > 0.0)
      {
...
std::unique_ptr<DistFactorEngineer> SplicedNUBeamFactory::CreateDistFactorEngineer(std::shared_ptr<WBFL::EAF::Broker> pBroker,StatusGroupIDType statusGroupID,const pgsTypes::SupportedBeamSpacing* pSpacingType,const pgsTypes::SupportedDeckType* pDeckType, const pgsTypes::AdjacentTransverseConnectivity* pConnect) const
{
...
bool SplicedNUBeamFactory::SupportsWebThickening() const
{
   return true;
}
