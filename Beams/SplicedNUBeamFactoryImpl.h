...
namespace PGS
{
   namespace Beams
   {
      class CSplicedNUBeamFactory : public SplicedBeamFactory, public BeamFactorySingleton<CSplicedNUBeamFactory>
      {
      public:
         static std::shared_ptr<CSplicedNUBeamFactory> CreateInstance() { return std::shared_ptr<CSplicedNUBeamFactory>(new CSplicedNUBeamFactory()); }
      ...
      private:
         ...
         void DimensionAndPositionBeam(const BeamFactory::Dimensions& dimensions, Float64 Hg, Float64 Hbf, INUBeam* pBeam) const;
      };
   };
};
