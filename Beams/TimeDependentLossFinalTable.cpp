...
CTimeDependentLossFinalTable::CTimeDependentLossFinalTable(ColumnIndexType NumColumns, IDisplayUnits* pDispUnit) :
...
CTimeDependentLossFinalTable* CTimeDependentLossFinalTable::PrepareTable(rptChapter* pChapter,IBroker* pBroker,SpanIndexType span,GirderIndexType gdr,IDisplayUnits* pDispUnit,Uint16 level)
{
   ...
   CTimeDependentLossFinalTable* table = new CTimeDependentLossFinalTable( numColumns, pDispUnit );
   ...
   *pParagraph << table << rptNewLine;
   ...
   (*table)(0,1) << COLHDR(symbol(DELTA) << Sub2("f","pSD"), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,2) << COLHDR(symbol(DELTA) << Sub2("f","pCD"), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,3) << COLHDR(symbol(DELTA) << Sub2("f","pR2"), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,4) << COLHDR(symbol(DELTA) << Sub2("f","pSS"), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,5) << COLHDR(symbol(DELTA) << "f" << subscript(ON) << "pLT" << subscript(ON) << "df" << subscript(OFF) << subscript(OFF), rptStressUnitTag, pDispUnit->GetStressUnit() );
...
void CTimeDependentLossFinalTable::AddRow(rptChapter* pChapter,IBroker* pBroker,RowIndexType row,LOSSDETAILS& details,IDisplayUnits* pDispUnit,Uint16 level)
{
