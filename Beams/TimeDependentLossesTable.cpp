...
CTimeDependentLossesTable::CTimeDependentLossesTable(ColumnIndexType NumColumns, IDisplayUnits* pDispUnit) :
...
CTimeDependentLossesTable* CTimeDependentLossesTable::PrepareTable(rptChapter* pChapter,IBroker* pBroker,SpanIndexType span,GirderIndexType gdr,IDisplayUnits* pDispUnit,Uint16 level)
{
   ...
   CTimeDependentLossesTable* table = new CTimeDependentLossesTable( numColumns, pDispUnit );
   ...
   *pParagraph << table << rptNewLine;
   ...
   (*table)(0,1) << COLHDR(symbol(DELTA) << "f" << subscript(ON) << "pLT" << subscript(ON) << "id" << subscript(OFF) << subscript(OFF), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,2) << COLHDR(symbol(DELTA) << "f" << subscript(ON) << "pLT" << subscript(ON) << "df" << subscript(OFF) << subscript(OFF), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,3) << COLHDR(symbol(DELTA) << Sub2("f","pLT"), rptStressUnitTag, pDispUnit->GetStressUnit() );

   return table;
}

void CTimeDependentLossesTable::AddRow(rptChapter* pChapter,IBroker* pBroker,RowIndexType row,LOSSDETAILS& details,IDisplayUnits* pDispUnit,Uint16 level)
{
