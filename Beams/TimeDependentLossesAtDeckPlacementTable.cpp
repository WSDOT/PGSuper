...
CTimeDependentLossesAtDeckPlacementTable::CTimeDependentLossesAtDeckPlacementTable(ColumnIndexType NumColumns, IDisplayUnits* pDispUnit) :
...
CTimeDependentLossesAtDeckPlacementTable* CTimeDependentLossesAtDeckPlacementTable::PrepareTable(rptChapter* pChapter,IBroker* pBroker,SpanIndexType span,GirderIndexType gdr,IDisplayUnits* pDispUnit,Uint16 level)
{
   ...
   CTimeDependentLossesAtDeckPlacementTable* table = new CTimeDependentLossesAtDeckPlacementTable( numColumns, pDispUnit );
   ...
   *pParagraph << table << rptNewLine;
   ...
   (*table)(0,2) << COLHDR(symbol(DELTA) << Sub2("f","pCR"), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,3) << COLHDR(symbol(DELTA) << Sub2("f","pR1"), rptStressUnitTag, pDispUnit->GetStressUnit() );
   (*table)(0,4) << COLHDR(symbol(DELTA) << "f" << subscript(ON) << "pLT" << subscript(ON) << "id" << subscript(OFF) << subscript(OFF), rptStressUnitTag, pDispUnit->GetStressUnit() );
...
void CTimeDependentLossesAtDeckPlacementTable::AddRow(rptChapter* pChapter,IBroker* pBroker,RowIndexType row,LOSSDETAILS& details,IDisplayUnits* pDispUnit,Uint16 level)
{
