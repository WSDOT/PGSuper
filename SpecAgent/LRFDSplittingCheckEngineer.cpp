...
std::shared_ptr<pgsSplittingCheckArtifact> pgsLRFDSplittingCheckEngineer::Check(const CSegmentKey& segmentKey, const GDRCONFIG* pConfig) const
{
   ...
   GET_IFACE2(broker, IGirder, pGdr);
   GET_IFACE2(broker, ILosses, pLosses);
   GET_IFACE2(broker, IPretensionForce, pPrestressForce);

   // get POI at point of prestress transfer
   // this is where the prestress force is fully transferred
...
#pragma Reminder("Simplify code - change methods on IStrandGeometry to work with pConfig=null so the code below can be refactored into a single set of logic and combined with the loop above")
   ...
   if (pConfig == nullptr)
   {
      GET_IFACE2(broker, IStrandGeometry, pStrandGeometry);
      for (int i = 0; i < 2; i++)
      {
         pgsTypes::MemberEndType endType = (pgsTypes::MemberEndType)i;
         for (int j = 0; j < 3; j++)
...
#pragma Reminder("Simplify code - change methods on IStrandGeometry to work with pConfig=null so the code below can be refactored into a single set of logic and combined with the loop above")
   ...
   if (pConfig == nullptr)
   {
      GET_IFACE2(broker, ISegmentData, pSegmentData);
   ...
   if (pConfig == nullptr)
   {
      GET_IFACE2(broker, IStirrupGeometry, pStirrupGeometry);
...
void pgsLRFDSplittingCheckEngineer::ReportDemand(rptParagraph* pPara, std::shared_ptr<IEAFDisplayUnits> pDisplayUnits, const std::_tstring& strSplittingType, const pgsSplittingCheckArtifact* pArtifact, pgsTypes::MemberEndType endType) const
{
   ...
   auto broker = EAFGetBroker();
   GET_IFACE2_NOCHECK(broker, ISpecification, pSpecification); // not used if bInitialRelaxation is false
...
std::shared_ptr<pgsSplittingCheckArtifact> pgsPCIUHPCSplittingCheckEngineer::Check(const CSegmentKey& segmentKey,const GDRCONFIG* pConfig) const
{
   ...
   GET_IFACE2(broker, IGirder, pGdr);
   GET_IFACE2(broker, IPretensionForce, pPrestress);
...
std::shared_ptr<pgsSplittingCheckArtifact> pgsUHPCSplittingCheckEngineer::Check(const CSegmentKey& segmentKey, const GDRCONFIG* pConfig) const
{
   ...
   GET_IFACE2(broker, IConcreteStressLimits, pLimits);
