...
public:
   ...
   bool operator()(const pgsPointOfInterest& poi) const
   {
      ...
      {
         return false; // does not have a POI_SPAN attribute so we don't want to keep it
      }
      ...
      Float64 Xspan;
      m_pBridgeAgent->ConvertPoiToSpanPoint(poi,&spanIdx,&Xspan);

      if ( m_SpanKey.spanIndex == spanIdx && IsEqual(Xspan,m_Lspan) )
      {
      ...
      if ( poi.HasAttribute(POI_SPAN | POI_0L) && m_SpanKey.spanIndex == spanIdx+1 )
      {
         ...
         // in the target span
         Float64 LprevSpan = m_pBridgeAgent->GetSpanLength(CSpanKey(spanIdx,m_SpanKey.girderIndex));
      ...
      return (spanIdx != m_SpanKey.spanIndex ? true /* poi is NOT in the target span*/ : false /* poi is in the target span*/);
   };
...
HRESULT CBridgeAgentImp::FinalConstruct()
{
   ...
   m_pSectProps[SPT_GROSS]       = std::auto_ptr<SectPropContainer>(new SectPropContainer);
   m_pSectProps[SPT_TRANSFORMED] = std::auto_ptr<SectPropContainer>(new SectPropContainer);
   m_pSectProps[SPT_NET_GIRDER]  = std::auto_ptr<SectPropContainer>(new SectPropContainer);
   m_pSectProps[SPT_NET_DECK]    = std::auto_ptr<SectPropContainer>(new SectPropContainer);
...
//      LOG(_T("Invalidating Bridge Model"));
      ...
      for ( int i = 0; i < 4; i++ )
      {
         SectPropContainer* pOldSectProps = m_pSectProps[i].release();
         m_pSectProps[i] = std::auto_ptr<SectPropContainer>(new SectPropContainer);
   ...
   for(CollectionIndexType loadIdx = 0; loadIdx < nLoads; loadIdx++)
   {
      ...
      for ( ; spanIter != spanIterEnd; spanIter++ )
      {
         ...
         for ( ; gdrIter != gdrIterEnd; gdrIter++ )
         {
            ...
            {
               if(0.0 <= pPointLoad->m_Location && pPointLoad->m_Location <= 1.0)
               {
            ...
            {
               if(0.0 <= pPointLoad->m_Location && pPointLoad->m_Location <= span_length)
               {
            ...
            Float64 Xs;
            GetSegmentLocation(span,girder,pPointLoad->m_Location,&segmentKey,&Xs);
            ...
            {
               intervalIdx = m_IntervalManager.GetLiveLoadInterval(segmentKey);
            }
            else
            {
               intervalIdx = m_IntervalManager.GetUserLoadInterval(segmentKey,pPointLoad->m_LoadCase,pPointLoad->m_EventIndex);
            }
   ...
   for(CollectionIndexType loadIdx = 0; loadIdx < nLoads; loadIdx++)
   {
      ...
      for (; spanIter != spanIterEnd; spanIter++)
      {
         ...
         for ( ; girderIter != girderIterEnd; girderIter++)
         {
            ...
            CSegmentKey segmentKey;
            Float64 Xs;
            GetSegmentLocation(span,girder,upl.m_StartLocation,&segmentKey,&Xs);
            ...
            GetSegmentLocation(span,girder,upl.m_EndLocation,&segmentKey,&Xs);
            ...
            {
               intervalIdx = m_IntervalManager.GetUserLoadInterval(segmentKey,pDistLoad->m_LoadCase,pDistLoad->m_EventIndex);
            }
            ATLASSERT(intervalIdx != INVALID_INDEX);
   ...
   for(CollectionIndexType loadIdx = 0; loadIdx < nLoads; loadIdx++)
   {
      ...
      for ( ; spanIter != spanIterEnd; spanIter++ )
      {
         ...
         for ( ; gdrIter != gdrIterEnd; gdrIter++ )
         {
            ...
            CSegmentKey segmentKey;
            Float64 Xs;
            GetSegmentLocation(thisspanKey.spanIndex,thisspanKey.girderIndex,pMomentLoad->m_Location,&segmentKey,&Xs);
            ...
            {
               intervalIdx = m_IntervalManager.GetUserLoadInterval(segmentKey,pMomentLoad->m_LoadCase,pMomentLoad->m_EventIndex);
            }
            ATLASSERT(intervalIdx != INVALID_INDEX);
...
#pragma Reminder("UPDATE: width of surface is arbitrary")
   // this is an arbitary value. it could be determined from the bridge deck
...
bool CBridgeAgentImp::BuildBridgeGeometryModel()
{
   ...
   bridgeGeometry->put_AlignmentOffset(alignment_offset);
   ...
   for ( GroupIndexType grpIdx = 0; grpIdx < nGroups; grpIdx++ )
   {
      ...
      for ( GirderIndexType gdrIdx = 0; gdrIdx < nGirders; gdrIdx++ )
      {
         ...
         for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
         {
            ...
            // Let beam factory builds segments as necessary to represent the beam
            // this includes setting the cross section
            beamFactory->CreateSegment(m_pBroker,m_StatusGroupID,segmentKey,&m_IntervalManager,ssmbr);
...
#pragma Reminder("UPDATE: Haunch depth isn't modeled correctly")
            ...
            StageIndexType stressStrandStageIdx = m_IntervalManager.GetStage(segmentKey,stressStrandIntervalIdx);
            ...
            straightStrandMaterial->put_InstallationStage(stressStrandStageIdx);
            ...
            harpedStrandMaterial->put_InstallationStage(stressStrandStageIdx);
            ...
            temporaryStrandMaterial->put_InstallationStage(stressStrandStageIdx);
...
#pragma Reminder("REVIEW: verify that harp point locations are in Segment Coordinates")
            ...
            StageIndexType    releaseStageIdx    = m_IntervalManager.GetStage(segmentKey,releaseIntervalIdx);

            CComPtr<ISection> startSection, hp1Section, hp2Section, endSection;
            segment->get_Section(releaseStageIdx, XsStart,  &startSection);
            segment->get_Section(releaseStageIdx, XsHp1Loc, &hp1Section);
            segment->get_Section(releaseStageIdx, XsHp2Loc, &hp2Section);
            segment->get_Section(releaseStageIdx, XsEnd,    &endSection);

            // bounding boxes of the section (height of section is height of bounding box)
            CComPtr<IRect2d> bbStart, bbHP1, bbHP2, bbEnd;
            startSection->get_BoundingBox(&bbStart);
            hp1Section->get_BoundingBox(&bbHP1);
            hp2Section->get_BoundingBox(&bbHP2);
            endSection->get_BoundingBox(&bbEnd);
               ...
               for ( ; iter != iterEnd; iter++ )
               {
                  ...
                  for ( int i = 0; i < 4; i++ )
                  {
                     ...
                     else
                     {
                        ...
                        segment->get_Section(releaseStageIdx, Z[i],  &section);
...
bool CBridgeAgentImp::LayoutDeck(const CBridgeDescription2* pBridgeDesc)
{
   ...
   IntervalIndexType nIntervals = m_IntervalManager.GetIntervalCount(girderKey);
   IntervalIndexType lastIntervalIdx = nIntervals-1;
   StageIndexType lastStageIdx = m_IntervalManager.GetStage(girderKey,lastIntervalIdx);

#if defined _DEBUG
   // the last stage should be the same for all girders
   GroupIndexType nGroups = pBridgeDesc->GetGirderGroupCount();
   GirderIndexType nGirders = pBridgeDesc->GetGirderGroup(nGroups-1)->GetGirderCount();
   CGirderKey lastGirderKey(nGroups-1,nGirders-1);
   lastIntervalIdx = m_IntervalManager.GetIntervalCount(lastGirderKey)-1;
   ATLASSERT(lastStageIdx == m_IntervalManager.GetStage(lastGirderKey,lastIntervalIdx));
#endif
   ...
   if ( pDeck->WearingSurface == pgsTypes::wstSacrificialDepth || pDeck->WearingSurface == pgsTypes::wstFutureOverlay)
   {
      ...
      m_Bridge->put_SacrificialDepthStage(lastStageIdx);
   }
   else
   {
   ...
   if ( pDeck->WearingSurface == pgsTypes::wstOverlay || pDeck->WearingSurface == pgsTypes::wstFutureOverlay )
   {
      ...
      IntervalIndexType overlayIntervalIdx = m_IntervalManager.GetOverlayInterval(girderKey);
      StageIndexType overlayStageIdx = m_IntervalManager.GetStage(girderKey,overlayIntervalIdx);
#if defined _DEBUG
      overlayIntervalIdx = m_IntervalManager.GetOverlayInterval(lastGirderKey);
      ATLASSERT(overlayStageIdx == m_IntervalManager.GetStage(lastGirderKey,overlayIntervalIdx));
#endif
      m_Bridge->put_WearingSurfaceStage(overlayStageIdx);
   }
   else
   {
   ...
   if ( deck )
   {
      ...
      StageIndexType nStages = m_IntervalManager.GetStageCount();
      for ( StageIndexType stageIdx = 0; stageIdx < nStages; stageIdx++ )
      {
         const std::map<CGirderKey,IntervalIndexType>& intervalMap = m_IntervalManager.GetIntervalMap(stageIdx);
         CGirderKey thisGirderKey = intervalMap.begin()->first;
         IntervalIndexType intervalIdx = intervalMap.begin()->second;

         IntervalIndexType compositeDeckIntervalIdx = m_IntervalManager.GetCompositeDeckInterval(thisGirderKey);

         Float64 E(0), D(0);
         if ( compositeDeckIntervalIdx <= intervalIdx )
         {
            E = GetDeckAgeAdjustedEc(girderKey,intervalIdx);
            D = GetDeckWeightDensity(girderKey,intervalIdx);
         }

         deck_material->put_E(stageIdx,E);
         deck_material->put_Density(stageIdx,D);
      }
...
void CBridgeAgentImp::CreateBarrierObject(IBarrier** pBarrier, const TrafficBarrierEntry*  pBarrierEntry, pgsTypes::TrafficBarrierOrientation orientation)
{
   ...
   for ( StageIndexType stageIdx = 0; stageIdx < nStages; stageIdx++ )
   {
      ...
      IntervalIndexType installRailingSystemIntervalIdx = m_IntervalManager.GetInstallRailingSystemInterval(thisGirderKey);

      Float64 E(0), D(0);
      if ( installRailingSystemIntervalIdx < intervalIdx )
      {
         E = GetRailingSystemEc(orientation,thisGirderKey,intervalIdx);
         D = GetRailingSystemWeightDensity(orientation,thisGirderKey,intervalIdx);
      }
...
#pragma Reminder("UPDATE: DEAL WITH OVER CONSTRAINED SEGMENTS")
...
void CBridgeAgentImp::LayoutSpanPoi(const CSpanKey& spanKey,Uint16 nPnts)
{
   ...
   for ( Uint16 i = 0; i <= nPnts; i++ )
   {
      ...
      pgsPointOfInterest poi = ConvertSpanPointToPoi(spanKey.spanIndex,spanKey.girderIndex,Xspan);
...
#pragma Reminder("UPDATE: don't need HP POI if they don't fall between bearings") // remove commented out code at end if it is obsolete
...
void CBridgeAgentImp::LayoutPoiForDiaphragmLoads(const CSegmentKey& segmentKey,Float64 segmentOffset)
{
   ...
   for ( ; iter != end; iter++ )
   {
      ...
      Float64 Xsp = start_offset  + Xs; // location in segment coordinates
      ...
      pgsPointOfInterest poi( segmentKey, Xs, Xsp, Xg, Xgp, POI_CONCLOAD);
      m_PoiMgr.AddPointOfInterest( poi );
   }
...
void CBridgeAgentImp::LayoutPoiForSegmentBarCutoffs(const CSegmentKey& segmentKey,Float64 segmentOffset)
{
   ...
   for (CollectionIndexType rebarLayoutItemIdx = 0; rebarLayoutItemIdx < nRebarLayoutItems; rebarLayoutItemIdx++)
   {
      ...
      {
#pragma Reminder("REVIEW: no bar cutoff or dev length POI in closure joint")
         // NOTE: if we decide to put dev length POI in closure joint, then
         // fc used to compute closure joint below needs to be re-examined... it is fc for the girder
...
void CBridgeAgentImp::LayoutPoiForTendon(const CGirderKey& girderKey,const CLinearDuctGeometry& ductGeometry)
{
   ...
   for  ( IndexType ptIdx = 0; ptIdx < nPoints; ptIdx++ )
   {
      ...
      poi.SetReferencedAttributes(0);
      poi.SetNonReferencedAttributes(0);
      poi.SetNonReferencedAttributes(POI_CONCLOAD);
      m_PoiMgr.AddPointOfInterest(poi);
   }
}
...
void CBridgeAgentImp::GetRoadwaySurface(Float64 station,IPoint2dCollection** ppPoints)
{
   Float64 distFromStartOfBridge = GetDistanceFromStartOfBridge(station);
   Float64 leftEdgeOffset  = GetLeftSlabEdgeOffset(distFromStartOfBridge);
   Float64 rightEdgeOffset = GetRightSlabEdgeOffset(distFromStartOfBridge);
   ...
   CComPtr<ISurface> surface;
   profile->GetSurface(varStation,&surface);
   ...
   {
      Float64 x,y;
      profile->RidgePointElevation(varStation,ridgePointIdx,alignmentRidgePointIdx,&x,&y);
...
Float64 CBridgeAgentImp::GetDistanceFromStartOfBridge(const pgsPointOfInterest& poi)
{
   Float64 station,offset;
   GetStationAndOffset(poi,&station,&offset);
   return GetDistanceFromStartOfBridge(station);
}
...
std::vector<Float64> CBridgeAgentImp::GetGirderSpacing(PierIndexType pierIdx,pgsTypes::PierFaceType pierFace,pgsTypes::MeasurementLocation measureLocation,pgsTypes::MeasurementType measureType)
{
   ...
   ATLASSERT( IsBoundaryPier(pierIdx) ); // pier must be a boundary pier
   // girder spacing is defined at the ends of segments. girder spacing
   // is only available at boundary piers and temporary supports.
#pragma Reminder("REVIEW: this should also work for piers with segments on either side")
   ...
   for ( GirderIndexType gdrIdx = 1; gdrIdx < nGirders; gdrIdx++ ) // start at gdrIdx 1 (spacing between gdrIdx 0 and 1)
   {
      ...
      GirderIDType leftGdrID  = ::GetGirderSegmentLineID(leftGdrSegmentKey.groupIndex, leftGdrSegmentKey.girderIndex, leftGdrSegmentKey.segmentIndex);
      GirderIDType rightGdrID = ::GetGirderSegmentLineID(rightGdrSegmentKey.groupIndex,rightGdrSegmentKey.girderIndex,rightGdrSegmentKey.segmentIndex);
      ...
      if ( measureType == pgsTypes::NormalToItem )
      {
         ...
         if ( measureLocation == pgsTypes::AtCenterlineBearing )
         {
            ...
            station = (pierFace == pgsTypes::Back ? GetBackBearingStation(pierIdx,leftGirderKey) : GetAheadBearingStation(pierIdx,leftGirderKey));

            Float64 offset;
            ConnectionLibraryEntry::BearingOffsetMeasurementType measureType;
            pIBridgeDesc->GetPier(pierIdx)->GetBearingOffset(pierFace,&offset,&measureType);
...
{
#pragma Reminder("REVIEW: this looks like it is something that should be in WBFLBridgeGeometry")
   ATLASSERT(spanIdx != ALL_SPANS);
   ...
   for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
   {
      ...
      if ( startSpanIdx <= spanIdx && spanIdx <= endSpanIdx )
      {
         ...
         if ( startSpanIdx < spanIdx && spanIdx == endSpanIdx )
         {
            ...
            PierIndexType pierIdx = (PierIndexType)spanIdx; // index of the pier at the start of the span
            ...
            if ( segIdx == nSegments-1 && grpIdx == nGroups-1 && spanIdx == nSpans-1 )
            {
         ...
         else if ( startSpanIdx == spanIdx && spanIdx < endSpanIdx )
         {
            ...
            if ( segIdx == 0 && grpIdx == 0 && spanIdx == 0 )
            {
...
void CBridgeAgentImp::GetSegmentLocation(SpanIndexType spanIdx,GirderIndexType girderIdx,Float64 Xspan,CSegmentKey* pSegmentKey,Float64* pXs)
{
   pgsPointOfInterest poi = ConvertSpanPointToPoi(spanIdx,girderIdx,Xspan);
   *pSegmentKey = poi.GetSegmentKey();
   *pXs = poi.GetDistFromStart();
}

void CBridgeAgentImp::GetSegmentPathLocation(SpanIndexType spanIdx,GirderIndexType girderIdx,Float64 Xspan,CSegmentKey* pSegmentKey,Float64* pXsp)
{
   Float64 Xs;
   GetSegmentLocation(spanIdx,girderIdx,Xspan,pSegmentKey,&Xs);
...
Float64 CBridgeAgentImp::GetSpanLength(const CSpanKey& spanKey)
{
   ...
   return GetSpanLength(spanKey.spanIndex,spanKey.girderIndex);
}
...
Float64 CBridgeAgentImp::GetSpanLength(SpanIndexType spanIdx,GirderIndexType gdrIdx)
{
   ...
   for ( ; iter != iterEnd; iter++ )
   {
      ...
      if ( (*iter).first == 0 && grpIdx != 0 )
      {
         ...
         {
            CSegmentKey segmentKey(grpIdx,gdrIdx,(*iter).first);
...
Float64 CBridgeAgentImp::GetFullSpanLength(SpanIndexType spanIdx,GirderIndexType gdrIdx)
{
   Float64 span_length = 0;
   std::vector<std::pair<SegmentIndexType,Float64>> seg_lengths(GetSegmentLengths(spanIdx,gdrIdx));
   std::vector<std::pair<SegmentIndexType,Float64>>::iterator iter(seg_lengths.begin());
...
{
   return GetDistanceFromStartOfBridge(pgsPointOfInterest(segmentKey,distFromStartOfSegment));
}
...
#pragma Reminder("UPDATE: assuming precast girder bridge")
   ...
   config.Fc       = GetSegmentFc(segmentKey,m_IntervalManager.GetLiveLoadInterval(segmentKey));
   config.Fci      = GetSegmentFc(segmentKey,m_IntervalManager.GetPrestressReleaseInterval(segmentKey));
   ...
   config.Ec  = GetSegmentEc(segmentKey,m_IntervalManager.GetLiveLoadInterval(segmentKey));
   config.Eci = GetSegmentEc(segmentKey,m_IntervalManager.GetPrestressReleaseInterval(segmentKey));
   config.bUserEci = pMaterial->Concrete.bUserEci;
   config.bUserEc  = pMaterial->Concrete.bUserEc;

   // Slab offset
   //GET_IFACE(IBridgeDescription,pIBridgeDesc);
   //const CPrecastSegmentData* pSegment = pIBridgeDesc->GetSegment(segmentKey);
   //const CPierData2* pStartPier, pEndPier;
   //const CTemporarySupportData* pStartTS, pEndTS;
   //pSegment->GetSupport(pgsTypes::metStart,&pStartPier,&pStartTS);
   //pSegment->GetSupport(pgsTypes::metEnd,&pEndPier,&pEndTS);
   //ATLASSERT(pStartPier != NULL && pEndPier != NULL);
   //ATLASSERT(pStartPier->GetIndex()+1 = pEndPier->GetIndex());
   PierIndexType startPierIdx, endPierIdx;
#pragma Reminder("UPDATE: WORKING HERE: assuming precast girder bridge")
   // assumes there is exactly one segment per girder so slab offsets at start and end of the group are what we want
   // we really want to look at the segment and get it's end support types (pier or TS) and then get the slab
   // offset at the end supports... OR .... assert that this is a PGSuper file since CONFIG isn't used for PGSplice
...
Float64 CBridgeAgentImp::GetEndDiaphragmLoadLocationAtStart(const CSegmentKey& segmentKey)
{
   ...
   {
      ATLASSERT(false); // end diaphragm load is not applied
      dist = 0.0;
   }
...
#pragma Reminder("UPDATE: update diaphragm model") // update the diaphragm model to match the model in TxDOT BGS
   // This method should take a girder key instead of a segment key
   ...
   for ( GirderIndexType i = 0; i < nGirders; i++ )
   {
      ...
      max_span_length = Max(max_span_length, GetSegmentSpanLength( CSegmentKey(segmentKey.groupIndex,i,segIdx) ));
   }
...
{
   IntervalIndexType castDeckIntervalIdx = m_IntervalManager.GetCastDeckInterval(poi.GetSegmentKey());
...
Float64 CBridgeAgentImp::GetSlabOffset(const pgsPointOfInterest& poi,const GDRCONFIG& config)
{
   ...
   Float64 dist_from_start_of_segment = poi.GetDistFromStart();
   ...
   Float64 dist_from_brg_to_poi = dist_from_start_of_segment - dist_to_start_brg;
...
bool CBridgeAgentImp::IsFutureOverlay()
{
   ...
   // we only need the relative occurance of the overlay compared to the live load
   // this is the same for all girders, so use girder(0,0). it is as good as any other girder
   IntervalIndexType overlayIntervalIdx  = GetOverlayInterval(CGirderKey(0,0));
   IntervalIndexType liveLoadIntervalIdx = GetLiveLoadInterval(CGirderKey(0,0));
...
{
   pgsPointOfInterest poi = ConvertSpanPointToPoi(spanIdx,0,Xspan);
...
Float64 CBridgeAgentImp::GetRightCurbOffset(SpanIndexType spanIdx,Float64 Xspan)
{
   ...
   pgsPointOfInterest poi = ConvertSpanPointToPoi(spanIdx,nGirders-1,Xspan);
   Float64 distFromStartOfBridge = GetDistanceFromStartOfBridge(poi);
...
void CBridgeAgentImp::GetSegmentsAtTemporarySupport(GirderIndexType gdrIdx,SupportIndexType tsIdx,CSegmentKey* pLeftSegmentKey,CSegmentKey* pRightSegmentKey)
{
   ...
   const CBridgeDescription2* pBridgeDesc = pIBridgeDesc->GetBridgeDescription();
   const CTemporarySupportData* pTS = pBridgeDesc->GetTemporarySupport(tsIdx);
   const CClosureJointData* pClosure = pTS->GetClosureJoint(gdrIdx);
   const CSpanData2* pSpan = pTS->GetSpan();
   const CGirderGroupData* pGroup = pBridgeDesc->GetGirderGroup(pSpan);
...
Float64 CBridgeAgentImp::GetClosureJointWeightDensity(const CSegmentKey& closureKey,IntervalIndexType intervalIdx)
{
   Float64 age = GetClosureJointConcreteAge(closureKey,intervalIdx);
   if ( age < 0 )
   {
      return 0;
   }
...
Float64 CBridgeAgentImp::GetDeckWeightDensity(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 age = GetDeckConcreteAge(girderKey,intervalIdx);
   if ( age < 0 )
   {
      return 0;
   }
...
Float64 CBridgeAgentImp::GetRailingSystemWeightDensity(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 age = GetRailingSystemAge(orientation,girderKey,intervalIdx);
   if ( age < 0 )
   {
      return 0;
   }
...
Float64 CBridgeAgentImp::GetSegmentConcreteAge(const CSegmentKey& segmentKey,IntervalIndexType intervalIdx)
{
   ...
   EventIndexType constructSegmentIdx = pTimelineMgr->GetSegmentConstructionEventIndex(segmentID);
   EventIndexType eventIdx = m_IntervalManager.GetStartEvent(segmentKey,intervalIdx);
   ...
   Float64 middle = m_IntervalManager.GetMiddle(segmentKey,intervalIdx);
...
Float64 CBridgeAgentImp::GetClosureJointConcreteAge(const CSegmentKey& closureKey,IntervalIndexType intervalIdx)
{
   GET_IFACE(IBridgeDescription,pIBridgeDesc);
   const CTimelineManager* pTimelineMgr = pIBridgeDesc->GetTimelineManager();
   ...
   EventIndexType castClosureIdx = pTimelineMgr->GetCastClosureJointEventIndex(closureID);
   EventIndexType eventIdx = m_IntervalManager.GetStartEvent(closureKey,intervalIdx);
   ...
   // want age at middle of interval
   Float64 middle = m_IntervalManager.GetMiddle(closureKey,intervalIdx);
...
Float64 CBridgeAgentImp::GetDeckConcreteAge(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   GET_IFACE(IBridgeDescription,pIBridgeDesc);
   const CTimelineManager* pTimelineMgr = pIBridgeDesc->GetTimelineManager();

   // want age at middle of interval
   Float64 middle = m_IntervalManager.GetMiddle(girderKey,intervalIdx);
...
Float64 CBridgeAgentImp::GetRailingSystemAge(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   GET_IFACE(IBridgeDescription,pIBridgeDesc);
   const CTimelineManager* pTimelineMgr = pIBridgeDesc->GetTimelineManager();

   EventIndexType railingSystemEventIdx = pTimelineMgr->GetRailingSystemLoadEventIndex();
   EventIndexType eventIdx = m_IntervalManager.GetStartEvent(girderKey,intervalIdx);
...
Float64 CBridgeAgentImp::GetClosureJointFc(const CClosureKey& closureKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(closureKey,intervalIdx);
   return m_ConcreteManager.GetClosureJointFc(closureKey,middle);
}

Float64 CBridgeAgentImp::GetDeckFc(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(girderKey,intervalIdx);
   return m_ConcreteManager.GetDeckFc(middle);
}

Float64 CBridgeAgentImp::GetRailingSystemFc(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(girderKey,intervalIdx);
...
Float64 CBridgeAgentImp::GetClosureJointEc(const CClosureKey& closureKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(closureKey,intervalIdx);
   return m_ConcreteManager.GetClosureJointEc(closureKey,middle);
}
...
Float64 CBridgeAgentImp::GetDeckEc(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(girderKey,intervalIdx);
   return m_ConcreteManager.GetDeckEc(middle);
}

Float64 CBridgeAgentImp::GetRailingSystemEc(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(girderKey,intervalIdx);
...
Float64 CBridgeAgentImp::GetClosureJointFlexureFr(const CSegmentKey& closureKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(closureKey,intervalIdx);
   return m_ConcreteManager.GetClosureJointFlexureFr(closureKey,middle);
}

Float64 CBridgeAgentImp::GetClosureJointShearFr(const CClosureKey& closureKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(closureKey,intervalIdx);
   return m_ConcreteManager.GetClosureJointShearFr(closureKey,middle);
}

Float64 CBridgeAgentImp::GetDeckFlexureFr(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(girderKey,intervalIdx);
   return m_ConcreteManager.GetDeckFlexureFr(middle);
}

Float64 CBridgeAgentImp::GetDeckShearFr(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 middle = m_IntervalManager.GetMiddle(girderKey,intervalIdx);
...
{
#pragma Reminder("REVIEW/UPDATE: Concrete Aging Coefficient")
   // the aging coefficient should be 1.0 for loads that are applied immediately and
   // something less for loads that are applied over time (such as restraint forces due to creep)
   // Just using 1.0 for now because the aging coefficient is causing wierd results for
   // single span bridges. Namely, there aren't any forces due to creep/shrinkage/relaxation
   // because the bridge isn't restrained so the value should be 1.0 and we were returning 0.7
   // This made the deflections jump around in ways that should not have been
   return 1.0;
   //Float64 duration = m_IntervalManager.GetDuration(segmentKey,intervalIdx);
...
{
   return 1.0;
   //Float64 duration = m_IntervalManager.GetDuration(closureKey,intervalIdx);
   //return (duration == 0.0 ? 1.0 : 0.7);
}

Float64 CBridgeAgentImp::GetDeckAgingCoefficient(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   return 1.0;
   //Float64 duration = m_IntervalManager.GetDuration(girderKey,intervalIdx);
   //return (duration == 0.0 ? 1.0 : 0.7);
}

Float64 CBridgeAgentImp::GetRailingSystemAgingCoefficient(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   return 1.0;
   //Float64 duration = m_IntervalManager.GetDuration(girderKey,intervalIdx);
   //return (duration == 0.0 ? 1.0 : 0.7);
...
Float64 CBridgeAgentImp::GetDeckAgeAdjustedEc(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 Ec = GetDeckEc(girderKey,intervalIdx);
   Float64 Y  = GetDeckCreepCoefficient(girderKey,intervalIdx,pgsTypes::Middle,intervalIdx,pgsTypes::End);
   Float64 X  = GetDeckAgingCoefficient(girderKey,intervalIdx);
   Float64 Ea = Ec/(1+X*Y);
   return Ea;
}

Float64 CBridgeAgentImp::GetRailingSystemAgeAdjustedEc(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   Float64 Ec = GetRailingSystemEc(orientation,girderKey,intervalIdx);
   Float64 Y  = GetRailingSystemCreepCoefficient(orientation,girderKey,intervalIdx,pgsTypes::Middle,intervalIdx,pgsTypes::End);
   Float64 X  = GetRailingSystemAgingCoefficient(orientation,girderKey,intervalIdx);
   Float64 Ea = Ec/(1+X*Y);
   return Ea;
}
...
{
   Float64 time = m_IntervalManager.GetTime(segmentKey,intervalIdx,timeType);
...
{
   Float64 time = m_IntervalManager.GetTime(closureKey,intervalIdx,timeType);
   return m_ConcreteManager.GetClosureJointFreeShrinkageStrain(closureKey,time);
}

Float64 CBridgeAgentImp::GetDeckFreeShrinkageStrain(const CGirderKey& girderKey,IntervalIndexType intervalIdx,pgsTypes::IntervalTimeType timeType)
{
   Float64 time = m_IntervalManager.GetTime(girderKey,intervalIdx,timeType);
   return m_ConcreteManager.GetDeckFreeShrinkageStrain(time);
}

Float64 CBridgeAgentImp::GetRailingSystemFreeShrinakgeStrain(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx,pgsTypes::IntervalTimeType timeType)
{
   Float64 time = m_IntervalManager.GetTime(girderKey,intervalIdx,timeType);
...
Float64 CBridgeAgentImp::GetDeckFreeShrinkageStrain(const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   return GetDeckFreeShrinkageStrain(girderKey,intervalIdx,pgsTypes::End) - GetDeckFreeShrinkageStrain(girderKey,intervalIdx,pgsTypes::Start);
}

Float64 CBridgeAgentImp::GetRailingSystemFreeShrinakgeStrain(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx)
{
   return GetRailingSystemFreeShrinakgeStrain(orientation,girderKey,intervalIdx,pgsTypes::End) - GetRailingSystemFreeShrinakgeStrain(orientation,girderKey,intervalIdx,pgsTypes::Start);
}
...
{
   Float64 loading_time = m_IntervalManager.GetTime(segmentKey,loadingIntervalIdx,loadingTimeType);
   Float64 time         = m_IntervalManager.GetTime(segmentKey,intervalIdx,timeType);
...
{
   Float64 loading_time = m_IntervalManager.GetTime(closureKey,loadingIntervalIdx,loadingTimeType);
   Float64 time = m_IntervalManager.GetTime(closureKey,intervalIdx,timeType);
   return m_ConcreteManager.GetClosureJointCreepCoefficient(closureKey,time,loading_time);
}

Float64 CBridgeAgentImp::GetDeckCreepCoefficient(const CGirderKey& girderKey,IntervalIndexType loadingIntervalIdx,pgsTypes::IntervalTimeType loadingTimeType,IntervalIndexType intervalIdx,pgsTypes::IntervalTimeType timeType)
{
   Float64 loading_time = m_IntervalManager.GetTime(girderKey,loadingIntervalIdx,loadingTimeType);
   Float64 time = m_IntervalManager.GetTime(girderKey,intervalIdx,timeType);
   return m_ConcreteManager.GetDeckCreepCoefficient(time,loading_time);
}

Float64 CBridgeAgentImp::GetRailingSystemCreepCoefficient(pgsTypes::TrafficBarrierOrientation orientation,const CGirderKey& girderKey,IntervalIndexType intervalIdx,pgsTypes::IntervalTimeType timeType,IntervalIndexType loadingIntervalIdx,pgsTypes::IntervalTimeType loadingTimeType)
{
   Float64 loading_time = m_IntervalManager.GetTime(girderKey,loadingIntervalIdx,loadingTimeType);
   Float64 time = m_IntervalManager.GetTime(girderKey,intervalIdx,timeType);
...
Float64 CBridgeAgentImp::GetPPRTopHalf(const pgsPointOfInterest& poi)
{
   ...
   Float64 E,fs,fu;
   if ( IsInClosureJoint(poi) )
   {
      GetClosureJointLongitudinalRebarProperties(segmentKey,&E,&fs,&fu);
   }
   else
   {
...
Float64 CBridgeAgentImp::GetVertStirrupAvs(const pgsPointOfInterest& poi, matRebar::Size* pSize, Float64* pSingleBarArea, Float64* pCount, Float64* pSpacing)
{
   ...
   {
      GET_IFACE(IShear,pShear);
      pShearData = pShear->GetClosureJointShearData(poi.GetSegmentKey());
   }
   else
   {
...
ZoneIndexType CBridgeAgentImp::GetPrimaryShearZoneIndexAtPoi(const pgsPointOfInterest& poi, const CShearData2* pShearData)
{
   ...
   if ( IsInClosureJoint(poi) || poi.HasAttribute(POI_BOUNDARY_PIER) )
   {
      length = GetClosureJointLength(segmentKey);
...
Float64 CBridgeAgentImp::GetStrandOffset(IntervalIndexType intervalIdx,const pgsPointOfInterest& poi,pgsTypes::StrandType strandType, Float64* nEffectiveStrands)
{
   IntervalIndexType releaseIntervalIdx = m_IntervalManager.GetPrestressReleaseInterval(poi.GetSegmentKey());
   ...
   {
   case pgsTypes::Straight:
      ecc = GetSsEccentricity(releaseIntervalIdx,poi,nEffectiveStrands);
      break;

   case pgsTypes::Harped:
      ecc = GetHsEccentricity(releaseIntervalIdx,poi,nEffectiveStrands);
      break;

   case pgsTypes::Temporary:
      ecc = GetTempEccentricity(releaseIntervalIdx,poi,nEffectiveStrands);
      break;
   ...
   Float64 Yt = GetY(releaseIntervalIdx,poi,pgsTypes::TopGirder);
   Float64 offset = -(Yt + ecc);
   ATLASSERT( offset <= 0 ); // this is in Girder Section Coordinates. Since strand is below top of girder its position should be negative
   ...
   else
   {
      ...
      *nEffectiveStrands = bond_factor*num_strand_positions;
      ...
      if (0 < num_strand_positions)
      {
         ...
         Float64 Yb = GetY(releaseIntervalIdx,poi,pgsTypes::BottomGirder);
...
#pragma Reminder("REVIEW: for the non-config version, number of effective strands is zero for start/end of girder... that isn't true here")// should it be?
      ...
      else
      {
         ...
         for (CollectionIndexType strandPositionIdx = 0; strandPositionIdx < num_strand_positions; strandPositionIdx++)
         {
            ...
            if ( dbidx != -1)
            {
               ...
               // strand is debonded
               lft_bond = poi_loc - dbinfo.DebondLength[pgsTypes::metStart];
               rgt_bond = girder_length - dbinfo.DebondLength[pgsTypes::metEnd] - poi_loc;
            }
            else
            {
               lft_bond = poi_loc;
...
Float64 CBridgeAgentImp::GetHsEccentricity(pgsTypes::SectionPropertyType spType,IntervalIndexType intervalIdx,const pgsPointOfInterest& poi, Float64* nEffectiveStrands)
{
   ...
   if ( intervalIdx < GetPrestressReleaseInterval(segmentKey) || IsInClosureJoint(poi) || poi.HasAttribute(POI_BOUNDARY_PIER) )
   {
      return 0;
   }
...
Float64 CBridgeAgentImp::GetSsEccentricity(pgsTypes::SectionPropertyType spType,IntervalIndexType intervalIdx,const pgsPointOfInterest& poi, Float64* nEffectiveStrands)
{
   ...
   if (0 < num_strands)
   {
      ...
      else if (IsEqual(dist,segment_length))
      {
...
void CBridgeAgentImp::GetHarpedEndOffsetBoundsEx(LPCTSTR strGirderName, Float64 HgStart, Float64 HgHp1, Float64 HgHp2, Float64 HgEnd, const ConfigStrandFillVector& rHarpedFillArray, Float64* DownwardOffset, Float64* UpwardOffset)
{
...
void CBridgeAgentImp::GetHarpedHpOffsetBoundsEx(LPCTSTR strGirderName, Float64 HgStart, Float64 HgHp1, Float64 HgHp2, Float64 HgEnd, const ConfigStrandFillVector& rHarpedFillArray, Float64* DownwardOffset, Float64* UpwardOffset)
{
   if ( !AreStrandsInConfigFillVec(rHarpedFillArray) )
   {
      *DownwardOffset = 0.0;
...
Float64 CBridgeAgentImp::ComputeAbsoluteHarpedOffsetEnd(const CSegmentKey& segmentKey, const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType measurementType, Float64 offset)
{
   ...
   Float64 HgStart = GetHeight(poiStart);
   Float64 HgHp1   = GetHeight(poiHp1);
   Float64 HgHp2   = GetHeight(poiHp2);
   Float64 HgEnd   = GetHeight(poiEnd);

   Float64 absOffset = ComputeAbsoluteHarpedOffsetEnd(pGirder->GetGirderName(),HgStart,HgHp1,HgHp2,HgEnd,rHarpedFillArray,measurementType,offset);
...
Float64 CBridgeAgentImp::ComputeAbsoluteHarpedOffsetEnd(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType measurementType, Float64 offset)
{
   // returns the offset value measured from the original strand locations defined in the girder library
   // Up is positive
   if ( !AreStrandsInConfigFillVec(rHarpedFillArray) )
...
Float64 CBridgeAgentImp::ComputeAbsoluteHarpedOffsetHp(const CSegmentKey& segmentKey, const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType measurementType, Float64 offset)
{
   ...
   Float64 HgStart = GetHg(intervalIdx,poiStart);
   Float64 HgHp1   = GetHg(intervalIdx,poiHp1);
   Float64 HgHp2   = GetHg(intervalIdx,poiHp2);
   Float64 HgEnd   = GetHg(intervalIdx,poiEnd);

   Float64 absOffset = ComputeAbsoluteHarpedOffsetHp(pGirder->GetGirderName(), HgStart, HgHp1, HgHp2, HgEnd, rHarpedFillArray,measurementType,offset);
...
Float64 CBridgeAgentImp::ComputeAbsoluteHarpedOffsetHp(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType measurementType, Float64 offset)
{
   // returns the offset value measured from the original strand locations defined in the girder library
   // Up is positive
   if ( !AreStrandsInConfigFillVec(rHarpedFillArray) )
...
Float64 CBridgeAgentImp::ComputeHarpedOffsetFromAbsoluteEnd(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType measurementType, Float64 absoluteOffset)
{
   // all we really need to know is the distance and direction between the coords, compute absolute
   // from zero
   Float64 absol = ComputeAbsoluteHarpedOffsetEnd(strGirderName, HgStart, HgHp1, HgHp2, HgEnd, rHarpedFillArray, measurementType, 0.0);
...
Float64 CBridgeAgentImp::ComputeHarpedOffsetFromAbsoluteHp(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType measurementType, Float64 absoluteOffset)
{
   Float64 absol = ComputeAbsoluteHarpedOffsetHp(strGirderName, HgStart, HgHp1, HgHp2, HgEnd, rHarpedFillArray, measurementType, 0.0);
...
void CBridgeAgentImp::ComputeValidHarpedOffsetForMeasurementTypeEnd(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray,HarpedStrandOffsetType measurementType, Float64* lowRange, Float64* highRange)
{
   Float64 absDown, absUp;
   GetHarpedEndOffsetBoundsEx(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,rHarpedFillArray, &absDown,&absUp);

   Float64 offDown = ComputeHarpedOffsetFromAbsoluteEnd(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,  rHarpedFillArray, measurementType, absDown);
   Float64 offUp =   ComputeHarpedOffsetFromAbsoluteEnd(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,  rHarpedFillArray, measurementType, absUp);
...
void CBridgeAgentImp::ComputeValidHarpedOffsetForMeasurementTypeHp(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType measurementType, Float64* lowRange, Float64* highRange)
{
   Float64 absDown, absUp;
   GetHarpedHpOffsetBoundsEx(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,rHarpedFillArray, &absDown,&absUp);

   Float64 offDown = ComputeHarpedOffsetFromAbsoluteHp(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,  rHarpedFillArray, measurementType, absDown);
   Float64 offUp =   ComputeHarpedOffsetFromAbsoluteHp(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,  rHarpedFillArray, measurementType, absUp);
...
Float64 CBridgeAgentImp::ConvertHarpedOffsetEnd(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType fromMeasurementType, Float64 offset, HarpedStrandOffsetType toMeasurementType)
{
   Float64 abs_offset = ComputeAbsoluteHarpedOffsetEnd(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,rHarpedFillArray,fromMeasurementType,offset);
   Float64 result = ComputeHarpedOffsetFromAbsoluteEnd(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,rHarpedFillArray,toMeasurementType,abs_offset);
   return result;
}
...
Float64 CBridgeAgentImp::ConvertHarpedOffsetHp(LPCTSTR strGirderName,Float64 HgStart,Float64 HgHp1,Float64 HgHp2,Float64 HgEnd,const ConfigStrandFillVector& rHarpedFillArray, HarpedStrandOffsetType fromMeasurementType, Float64 offset, HarpedStrandOffsetType toMeasurementType)
{
   Float64 abs_offset = ComputeAbsoluteHarpedOffsetHp(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,rHarpedFillArray,fromMeasurementType,offset);
   Float64 result = ComputeHarpedOffsetFromAbsoluteHp(strGirderName, HgStart, HgHp1, HgHp2, HgEnd,rHarpedFillArray,toMeasurementType,abs_offset);
   return result;
}
...
pgsPointOfInterest CBridgeAgentImp::GetPointOfInterest(const CGirderKey& girderKey,PierIndexType pierIdx)
{
   ...
   return ConvertGirderCoordinateToPoi(girderKey,Xg);
}

pgsPointOfInterest CBridgeAgentImp::ConvertSpanPointToPoi(SpanIndexType spanIdx,GirderIndexType gdrIdx,Float64 Xspan)
{
   ...
   {
      bTesting = true;
      SpanIndexType testSpanIdx;
      Float64 testXspan;
      ConvertPoiToSpanPoint(poi,&testSpanIdx,&testXspan);
      if ( testSpanIdx == spanIdx-1 )
      {
      ...
      else if ( spanIdx == testSpanIdx-1 )
      {
         ...
         // as the input location
         ATLASSERT(IsEqual(Xspan,GetSpanLength(spanIdx,gdrIdx))); // should be at the end of the span
         ATLASSERT(IsZero(testXspan)); // should be at the start of the next span
      }
      else
      {
         ATLASSERT(testSpanIdx == spanIdx && IsEqual(testXspan,Xspan));
      }
      bTesting = false;
   }
#endif
...
void CBridgeAgentImp::ConvertPoiToSpanPoint(const pgsPointOfInterest& poi,SpanIndexType* pSpanIdx,Float64* pXspan)
{
...
#if defined _DEBUG
   pgsPointOfInterest testPoi = ConvertSpanPointToPoi(*pSpanIdx,girderKey.girderIndex,*pXspan);
...
{
#pragma Reminder("UPDATE: this is inefficient") // when the vector gets pushed into the list a copy is made
...
pgsPointOfInterest CBridgeAgentImp::ConvertSegmentPathCoordinateToPoi(const CSegmentKey& segmentKey,Float64 Xsp)
{
...
pgsPointOfInterest CBridgeAgentImp::ConvertGirderCoordinateToPoi(const CGirderKey& girderKey,Float64 Xg)
{
   ...
   else
   {
      ...
      for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
      {
         ...
         if ( segIdx == 0 && girderKey.groupIndex == 0 )
         {
         ...
         if ( segIdx == nSegments-1 && girderKey.groupIndex == nGroups-1 )
         {
...
bool CBridgeAgentImp::IsInClosureJoint(const pgsPointOfInterest& poi)
{
   const CSegmentKey& segmentKey(poi.GetSegmentKey());
   ...
   {
      ATLASSERT(::InRange(Ls,poi.GetDistFromStart(),Ls+Lc));
   }
#endif

   if ( Ls < poi.GetDistFromStart() && poi.GetDistFromStart() < Ls+Lc )
   {
      return true;
   }
   else
...
Float64 CBridgeAgentImp::GetY(pgsTypes::SectionPropertyType spType,IntervalIndexType intervalIdx,const pgsPointOfInterest& poi,pgsTypes::StressLocation location)
{
   ...
   case pgsTypes::TopDeck:
      {
         IntervalIndexType compositeDeckIntervalIdx = GetCompositeDeckInterval(poi.GetSegmentKey());
...
Float64 CBridgeAgentImp::GetS(pgsTypes::SectionPropertyType spType,IntervalIndexType intervalIdx,const pgsPointOfInterest& poi,pgsTypes::StressLocation location)
{
   ...
   {
      IntervalIndexType compositeDeckInterval = m_IntervalManager.GetCompositeDeckInterval(segmentKey);
         ...
         )
      {
         ...
         Float64 Eg = (IsInClosureJoint(poi) ? GetClosureJointEc(segmentKey,intervalIdx) : GetSegmentEc(segmentKey,intervalIdx));
         Float64 Es = GetDeckEc(segmentKey,intervalIdx);
...
Float64 CBridgeAgentImp::GetQSlab(const pgsPointOfInterest& poi)
{
   ...
   IntervalIndexType compositeDeckIntervalIdx = m_IntervalManager.GetCompositeDeckInterval(poi.GetSegmentKey());
...
Float64 CBridgeAgentImp::GetAcBottomHalf(const pgsPointOfInterest& poi)
{
   ...
   IntervalIndexType compositeDeckInterval = m_IntervalManager.GetCompositeDeckInterval(poi.GetSegmentKey());
...
{
   CGirderKey girderKey(0,0);
   IntervalIndexType lastIntervalIdx = m_IntervalManager.GetIntervalCount(girderKey) - 1;
   StageIndexType lastStageIdx = m_IntervalManager.GetStage(girderKey,lastIntervalIdx);

#if defined _DEBUG
   GET_IFACE(IBridgeDescription,pIBridgeDesc);
   GroupIndexType nGroups = pIBridgeDesc->GetGirderGroupCount();
   GirderIndexType nGirders = pIBridgeDesc->GetGirderGroup(nGroups-1)->GetGirderCount();
   CGirderKey lastGirderKey(nGroups-1,nGirders-1);
   lastIntervalIdx = m_IntervalManager.GetIntervalCount(lastGirderKey)-1;
   ATLASSERT(lastStageIdx == m_IntervalManager.GetStage(lastGirderKey,lastIntervalIdx));
#endif
   ...
   CComPtr<ISection> section;
   m_SectCutTool->CreateBridgeSection(m_Bridge,distFromStart,lastStageIdx,bscStructurallyContinuousOnly,&section);
...
Float64 CBridgeAgentImp::GetSegmentHeightAtTemporarySupport(const CSegmentKey& segmentKey,SupportIndexType tsIdx)
{
   ...
   return GetHg(GetIntervalCount(segmentKey)-1,poi);
}
...
#pragma Reminder("UPDATE: these shapes can probably be cached")
   ...
   StageIndexType stageIdx = m_IntervalManager.GetStage(segmentKey,intervalIdx);
   HRESULT hr = m_SectCutTool->CreateGirderShapeBySegment(m_Bridge,gdrID,segmentKey.segmentIndex,Xs,leftGdrID,rightGdrID,stageIdx,ppShape);
   ATLASSERT(SUCCEEDED(hr));
...
Float64 CBridgeAgentImp::GetSidewalkWeight(pgsTypes::TrafficBarrierOrientation orientation)
{
   ...
   if ( pRailingSystem->bUseSidewalk )
   {
      ...
      CGirderKey girderKey(0,0); // doesn't matter which girder
      IntervalIndexType railingSystemIntervalIdx = GetInstallRailingSystemInterval(girderKey);
      ...
      Float64 density = GetRailingSystemWeightDensity(orientation,girderKey,railingSystemIntervalIdx);
...
#pragma Reminder("UPDATE: assuming precast girder bridge")
   ...
   if ( IsCompositeDeck() )
   {
      ...
      if ( intervalIdx < m_IntervalManager.GetCompositeDeckInterval(segmentKey) )
      {
...
{
   IntervalIndexType liveLoadIntervalIdx = m_IntervalManager.GetLiveLoadInterval(girderKey);
...
Float64 CBridgeAgentImp::GetShearWidth(const pgsPointOfInterest& poi)
{
   ...
   //
   IntervalIndexType intervalIdx = GetIntervalCount(girderKey) - 1;
...
Float64 CBridgeAgentImp::GetTopGirderReferenceChordElevation(const pgsPointOfInterest& poi)
{
   ...
   Float64 yc = ::LinInterp(Xg,startElevation,endElevation,length);
...
#pragma Reminder("UPDATE: need to account for elevation changes at temporary supports")
...
void CBridgeAgentImp::GetSegment(const CGirderKey& girderKey,Float64 Xg,SegmentIndexType* pSegIdx,Float64* pXs)
{
#pragma Reminder("REVIEW: is this method necessary?")
   //ConvertGirderCoordinateToPoi does the same thing
   ATLASSERT(false); // just want to see if this method ever gets called
   CComPtr<ISuperstructureMember> ssMbr;
   GetSuperstructureMember(girderKey,&ssMbr);
...
#pragma Reminder("UPDATE: use the methods from WBFL::GenericBridge") // remove this implementation and use the code in WBFL Generic Bridge
   ...
   for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
   {
      ...
      if ( segIdx == 0 )
      {
         ...
         Float64 brgOffset = GetSegmentStartBearingOffset(segmentKey);
         Float64 endDist   = GetSegmentStartEndDistance(segmentKey);
         xSegmentStart = brgOffset - endDist;
         segment_length -= (brgOffset-endDist);
      }
...
//////////////////////////////////////////////////////////////////////////
// IIntervals
IntervalIndexType CBridgeAgentImp::GetIntervalCount(const CGirderKey& girderKey)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetIntervalCount(girderKey);
}

EventIndexType CBridgeAgentImp::GetStartEvent(const CGirderKey& girderKey,IntervalIndexType idx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetStartEvent(girderKey,idx);
}

EventIndexType CBridgeAgentImp::GetEndEvent(const CGirderKey& girderKey,IntervalIndexType idx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetEndEvent(girderKey,idx);
}

Float64 CBridgeAgentImp::GetStart(const CGirderKey& girderKey,IntervalIndexType idx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetStart(girderKey,idx);
}

Float64 CBridgeAgentImp::GetMiddle(const CGirderKey& girderKey,IntervalIndexType idx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetMiddle(girderKey,idx);
}

Float64 CBridgeAgentImp::GetEnd(const CGirderKey& girderKey,IntervalIndexType idx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetEnd(girderKey,idx);
}

Float64 CBridgeAgentImp::GetDuration(const CGirderKey& girderKey,IntervalIndexType idx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetDuration(girderKey,idx);
}

LPCTSTR CBridgeAgentImp::GetDescription(const CGirderKey& girderKey,IntervalIndexType idx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetDescription(girderKey,idx);
}

IntervalIndexType CBridgeAgentImp::GetInterval(const CGirderKey& girderKey,EventIndexType eventIdx)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetInterval(girderKey,eventIdx);
}
...
IntervalIndexType CBridgeAgentImp::GetCastClosureJointInterval(const CClosureKey& closureKey)
{
   VALIDATE(BRIDGE);
   GET_IFACE(IBridgeDescription,pIBridgeDesc);
   EventIndexType castClosureJointEventIdx = pIBridgeDesc->GetCastClosureJointEventIndex(closureKey.groupIndex,closureKey.segmentIndex);
   return m_IntervalManager.GetInterval(closureKey,castClosureJointEventIdx);
}
...
void CBridgeAgentImp::GetContinuityInterval(const CGirderKey& girderKey,PierIndexType pierIdx,IntervalIndexType* pBack,IntervalIndexType* pAhead)
{
   ...
   *pBack  = GetInterval(girderKey,backContinuityEventIdx);
   *pAhead = GetInterval(girderKey,aheadContinuityEventIdx);
}

IntervalIndexType CBridgeAgentImp::GetCastDeckInterval(const CGirderKey& girderKey)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetCastDeckInterval(girderKey);
}

IntervalIndexType CBridgeAgentImp::GetCompositeDeckInterval(const CGirderKey& girderKey)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetCompositeDeckInterval(girderKey);
}
...
IntervalIndexType CBridgeAgentImp::GetLiveLoadInterval(const CGirderKey& girderKey)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetLiveLoadInterval(girderKey);
}
...
#pragma Reminder("UPDATE: assuming load ratings occur in the final interval")
   ...
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetIntervalCount(girderKey)-1;
}

IntervalIndexType CBridgeAgentImp::GetOverlayInterval(const CGirderKey& girderKey)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetOverlayInterval(girderKey);
}

IntervalIndexType CBridgeAgentImp::GetInstallRailingSystemInterval(const CGirderKey& girderKey)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetInstallRailingSystemInterval(girderKey);
}
...
IntervalIndexType CBridgeAgentImp::GetTemporarySupportRemovalInterval(const CGirderKey& girderKey,SupportIDType tsID)
{
   VALIDATE(BRIDGE);
   return m_IntervalManager.GetTemporarySupportRemovalInterval(girderKey,tsID);
}

std::vector<IntervalIndexType> CBridgeAgentImp::GetTemporarySupportRemovalIntervals(const CGirderKey& girderKey)
{
   ...
   while ( pSpan != pEndSpan )
   {
      ...
      for ( ; tsIter != tsIterEnd; tsIter++ )
      {
         ...
         IntervalIndexType intervalIdx = GetTemporarySupportRemovalInterval(girderKey,tsID);
         ATLASSERT(intervalIdx != INVALID_INDEX);
...
std::vector<IntervalIndexType> CBridgeAgentImp::GetUserDefinedLoadIntervals(const CGirderKey& girderKey)
{
   ...
   IntervalIndexType nIntervals = GetIntervalCount(girderKey);
   for ( IntervalIndexType intervalIdx = 0; intervalIdx < nIntervals; intervalIdx++ )
   {
      for ( GroupIndexType grpIdx = startGroupIdx; grpIdx <= endGroupIdx; grpIdx++ )
      {
...
std::vector<IntervalIndexType> CBridgeAgentImp::GetUserDefinedLoadIntervals(const CGirderKey& girderKey,ProductForceType pfType)
{
   std::vector<IntervalIndexType> vIntervals;
   ...
   ATLASSERT(pfType == pftUserDC || pfType == pftUserDW);
   IUserDefinedLoads::UserDefinedLoadCase loadCase = (pfType == pftUserDC ? IUserDefinedLoads::userDC : IUserDefinedLoads::userDW);
...
std::vector<IntervalIndexType> CBridgeAgentImp::GetSpecCheckIntervals(const CGirderKey& girderKey)
{
   ...
   for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
   {
      ...
      vIntervals.push_back(GetPrestressReleaseInterval(segmentKey));

      // These are different animals so don't include them here
      //vIntervals.push_back(GetLiftSegmentInterval(segmentKey));
      //vIntervals.push_back(GetHaulSegmentInterval(segmentKey));

      // Only spec check at segment erection if there is a support mid-segment that causes negative moments in the segment
      if ( bIsContinuousOverSupport )
      {
         vIntervals.push_back(GetErectSegmentInterval(segmentKey));
      }
   ...
   vIntervals.push_back(GetCastDeckInterval(girderKey));
   vIntervals.push_back(GetInstallRailingSystemInterval(girderKey));
   ...
   // Need to spec check whenever a user defined load is applied
   std::vector<IntervalIndexType> vUserLoadIntervals(GetUserDefinedLoadIntervals(girderKey));
   ...
   vIntervals.push_back(GetIntervalCount(girderKey)-1); // last interval
...
CBridgeAgentImp::SectProp CBridgeAgentImp::GetSectionProperties(IntervalIndexType intervalIdx,const pgsPointOfInterest& poi,int sectPropType)
{
   ...
   IntervalIndexType releaseIntervalIdx       = GetPrestressReleaseInterval(segmentKey);
   IntervalIndexType compositeDeckIntervalIdx = GetCompositeDeckInterval(segmentKey);

   // if the POI is at a closure joint and the closure joint isn't composite with the girder yet, then
   // the POI is at a non-structural section so the section properties are zero
   bool bIsSection = true;
   if ( (IsInClosureJoint(poi) && intervalIdx < GetCompositeClosureJointInterval(segmentKey)) )
   {
   ...
   // if the girder concrete has not been cured yet, then the properties are taken to be zero
   ...
   if ( bIsGirderConcreteCured && bIsDeckComposite && bIsSection )
   {
      ...
      if ( sectPropType == SPT_GROSS || sectPropType == SPT_TRANSFORMED || sectPropType == SPT_NET_GIRDER )
      {
         ...
         CComPtr<ISection> section;
         StageIndexType stageIdx = m_IntervalManager.GetStage(segmentKey,intervalIdx);
         HRESULT hr = m_SectCutTool->CreateGirderSectionBySegment(m_Bridge,gdrID,segmentKey.segmentIndex,Xs,leftGdrID,rightGdrID,stageIdx,(SectionPropertyMethod)sectPropType,&section);
         ATLASSERT(SUCCEEDED(hr));
         ...
         Float64 Egdr = (IsInClosureJoint(poi) ? GetClosureJointAgeAdjustedEc(segmentKey,intervalIdx) : GetSegmentAgeAdjustedEc(segmentKey,intervalIdx)); // this method returns the correct E even for gross and tranformed properties without time-step losses
      ...
      else if ( sectPropType == SPT_NET_DECK )
      {
         ...
         StageIndexType stageIdx = m_IntervalManager.GetStage(segmentKey,intervalIdx);
         HRESULT hr = m_SectCutTool->CreateNetDeckSection(m_Bridge,gdrID,segmentKey.segmentIndex,Xs,leftGdrID,rightGdrID,stageIdx,&deckSection);
         ATLASSERT(SUCCEEDED(hr));
         ...
         // we always do this because the deck must be transformed, even for net properties
         CComPtr<IShapeProperties> shapeprops;
         Float64 Edeck = GetDeckAgeAdjustedEc(segmentKey,intervalIdx); // this method returns the correct E even for gross and tranformed properties without time-step losses
...
void CBridgeAgentImp::LayoutDeckRebar(const CDeckDescription2* pDeck,IBridgeDeck* deck)
{
   ...
   CGirderKey girderKey(0,0);
   IntervalIndexType compositeDeckIntervalIdx = m_IntervalManager.GetCompositeDeckInterval(girderKey);
   StageIndexType compositeDeckStageIdx = m_IntervalManager.GetStage(girderKey,compositeDeckIntervalIdx);
   ...
   if ( rebar_data.TopRebarSize != matRebar::bsNone )
   {
      ...
      CComPtr<IRebar> rebar;
      rebar_factory->CreateRebar(matSpec,matGrade,matSize,unit_convert,compositeDeckStageIdx,&rebar);
   ...
   if ( !IsZero(rebar_data.TopLumpSum) )
   {
      ...
      // the lump sum bar
      CComPtr<IRebar> rebar;
      rebar_factory->CreateRebar(matSpec,matGrade,bs3,unit_convert,compositeDeckStageIdx,&rebar);
   ...
   if ( 0 < rebar_rows.size() )
   {
      ...
      for ( RowIndexType idx = 0; idx < rebar_rows.size(); idx++ )
      {
         ...
         if (info.GetRebarStartEnd(segment_length, &startLayout, &endLayout))
         {
            ...
            CComPtr<IRebar> rebar;
            rebar_factory->CreateRebar(matSpec,matGrade,bar_size,unit_convert,releaseStageIdx,&rebar);
...
void CBridgeAgentImp::LayoutClosureJointRebar(const CClosureKey& closureKey)
{
   ...
   // until the concrete is cured
   IntervalIndexType compositeClosureIntervalIdx = GetCompositeClosureJointInterval(closureKey);
   StageIndexType compositeClosureStageIdx = m_IntervalManager.GetStage(closureKey,compositeClosureIntervalIdx);
   ...
   for ( IndexType idx = 0; idx < nRows; idx++ )
   {
      ...
      if ( 0 < info.BarSize && 0 < info.NumberOfBars )
      {
         ...
         BarSize bar_size = GetBarSize(info.BarSize);
         CComPtr<IRebar> rebar;
         rebar_factory->CreateRebar(matSpec,matGrade,bar_size,unit_convert,compositeClosureStageIdx,&rebar);
   ...
   if ( drt == ILongRebarGeometry::Primary || drt == ILongRebarGeometry::All )
   {
      ...
      if ( bTopMat )
      {
         if ( rebarData.TopRebarSize != matRebar::bsNone )
         {
            ...
            IndexType nBars = IsZero(rebarData.TopSpacing) ? 0 : IndexType(Weff/rebarData.TopSpacing) + 1;
      ...
      if ( bBottomMat )
      {
         if ( rebarData.BottomRebarSize != matRebar::bsNone )
         {
            ...
            IndexType nBars = IsZero(rebarData.BottomSpacing) ? 0 : IndexType(Weff/rebarData.BottomSpacing) + 1;
            Float64 As = nBars*pBar->GetNominalArea()/Weff;
            YbAs_Bottom += Yb*As;
            As_Bottom   += As;
         }
...
void CBridgeAgentImp::CreateTendons(const CBridgeDescription2* pBridgeDesc,const CGirderKey& girderKey,ISuperstructureMember* pSSMbr,ITendonCollection** ppTendons)
{
   ...
   for ( DuctIndexType ductIdx = 0; ductIdx < nDucts; ductIdx++ )
   {
      ...
      // that is why there is a +1 in the call to GetStage
      IntervalIndexType stressTendonIntervalIdx = m_IntervalManager.GetStressTendonInterval(girderKey,ductIdx);
      StageIndexType stressTendonStageIdx = m_IntervalManager.GetStage(girderKey,stressTendonIntervalIdx+1);
      ...
      tendonMaterial->put_InstallationStage(stressTendonStageIdx);
