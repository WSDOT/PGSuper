...
CGirderModelManager::CGirderModelManager(SHARED_LOGFILE lf,std::weak_ptr<WBFL::EAF::Broker> pBroker,StatusGroupIDType statusGroupID) :
LOGFILE(lf),
m_pBroker(pBroker),
...
REACTION CGirderModelManager::GetReaction(IntervalIndexType intervalIdx, pgsTypes::ProductForceType pfType, SupportIndexType supportIdx, pgsTypes::SupportType supportType, const CGirderKey& girderKey, pgsTypes::BridgeAnalysisType bat, ResultsType resultsType) const
{
   ...
   for ( IndexType i = 0; i < nResults; i++ )
   {
...
std::pair<Float64,Float64> CGirderModelManager::GetDeckShrinkageStresses(const pgsPointOfInterest& poi, pgsTypes::StressLocation topStressLocation, pgsTypes::StressLocation botStressLocation, const GDRCONFIG* pConfig) const
{
   ...
   if (pPoi->IsOnSegment(poi))
   {
      ...
      if (pConfig)
      {
         ...
         GET_IFACE2(GetBroker(),ISectionProperties, pProps);
...
Float64 CGirderModelManager::GetReaction(IntervalIndexType intervalIdx,LoadingCombinationType combo,PierIndexType pierIdx,const CGirderKey& girderKey,pgsTypes::BridgeAnalysisType bat,ResultsType resultsType) const
{
   ...
   Float64 R = 0;
   for (IndexType ldGroupIdx = 0; ldGroupIdx < nLoadGroups; ldGroupIdx++)
   {
      CComBSTR lg_name;
...
std::vector<Float64> CGirderModelManager::GetDeflection(IntervalIndexType intervalIdx,LoadingCombinationType combo,const PoiList& vPoi,pgsTypes::BridgeAnalysisType bat,ResultsType resultsType,bool bIncludePreErectionUnrecov) const
{
   ...
   if (bDoApplyStorageCorrection)
   {
      ...
      GET_IFACE2(GetBroker(),IProductForces2, pProdForces);
...
void CGirderModelManager::GetStress(IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,const PoiList& vPoi,pgsTypes::BridgeAnalysisType bat,pgsTypes::StressLocation stressLocation,bool bIncludePrestress,std::vector<Float64>* pMin,std::vector<Float64>* pMax) const
{
   ...
   for(const pgsPointOfInterest& poi : vPoi)
   {
      ...
      IndexType stress_point_index = GetStressPointIndex(stressLocation);
...
std::vector<Float64> CGirderModelManager::GetSlabDesignMoment(pgsTypes::LimitState limitState,const PoiList& vPoi,pgsTypes::BridgeAnalysisType bat) const
{
   ...
   for(const pgsPointOfInterest& poi : vPoi)
   {
      ...
      if ( bExcludeNoncompositeMoments )
      {
         ...
         for ( IndexType loadCaseIdx = 0; loadCaseIdx < nLoadCases; loadCaseIdx++ )
         {
            Float64 g;
            CComBSTR bstr;
   ...
   {
      GET_IFACE2(GetBroker(),ICombinedForces2,pForces);
                                  ...
                                  Float64* pRmin,Float64* pRmax) const
{
   ...
   else
   {
      ...
      *pRmax = 0.0;
      ...
      for (IndexType lc_idx = 0; lc_idx < lc_cnt; lc_idx++)
      {
         CComBSTR lc_name;
...
void CGirderModelManager::BuildLBAM(GirderIndexType gdrLineIdx,bool bContinuousModel,IContraflexureResponse* pContraflexureResponse,IContraflexureResponse* pDeflContraflexureResponse,ILBAMModel* pModel) const
{
   try
   {
      ...
      GET_IFACE2(GetBroker(),ILoadModifiers,pLoadModifiers);
...
void CGirderModelManager::CreateLBAMSpans(GirderIndexType gdr,bool bContinuousModel,const WBFL::LRFD::LoadModifier& loadModifier,ILBAMModel* pModel) const
{
   ...
   for (const auto& thisGirderKey : vGirderKeys)
   {
      ...
      for (SpanIndexType spanIdx = startSpanIdx; spanIdx <= endSpanIdx; spanIdx++)
      {
         ...
         objSpan->SetLoadModifier(lctStrength, loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI, WBFL::LRFD::Optimization::Min), loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI, WBFL::LRFD::Optimization::Max));
         spans->Add(objSpan);
   ...
   for ( SupportIndexType tsIdx = 0; tsIdx < nTS; tsIdx++ )
   {
      ...
      if ( pBridge->GetSegmentConnectionTypeAtTemporarySupport(tsIdx) == pgsTypes::tsctContinuousSegment )
      {
         ...
         objTS->SetLoadModifier(lctStrength,loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Min),loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Max));
      ...
      else
      {
         ...
         objLeftTS->SetLoadModifier(lctStrength,loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Min),loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Max));
         ...
         objRightTS->SetLoadModifier(lctStrength,loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Min),loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Max));
...
void CGirderModelManager::CreateLBAMSupport(GirderIndexType gdrLineIdx,bool bContinuousModel,const WBFL::LRFD::LoadModifier& loadModifier,const CPierData2* pPier,ISupport** ppSupport) const
{
   CComPtr<ISupport> objSupport;
   ...
   objSupport->SetLoadModifier(lctStrength,loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Min),loadModifier.GetLoadModifier(WBFL::LRFD::LimitState::StrengthI,WBFL::LRFD::Optimization::Max));

   objSupport.CopyTo(ppSupport);
}

void CGirderModelManager::CreateLBAMSuperstructureMembers(GirderIndexType gdr,bool bContinuousModel,WBFL::LRFD::LoadModifier& loadModifier,ILBAMModel* pModel) const
{
...
void CGirderModelManager::ApplyUserDefinedLoads(ILBAMModel* pModel,GirderIndexType gdr) const
{
   ...
   GET_IFACE2(GetBroker(),IUserDefinedLoads, pUdls);
...
{
   GET_IFACE2(GetBroker(),IIntervals,pIntervals);
   GET_IFACE2(GetBroker(),IGirderTendonGeometry,pTendonGeometry);
   GET_IFACE2(GetBroker(),ILosses,pLosses);
...
{
   GET_IFACE2(GetBroker(),IUserDefinedLoadData,pLoads);
...
void CGirderModelManager::GetMainConstructionLoad(const CSegmentKey& segmentKey, std::vector<ConstructionLoad>* pConstructionLoads) const
{
   ...
   GET_IFACE2(GetBroker(),IPointOfInterest,pIPoi);
   PoiList vPoi;
...
MemberIDType CGirderModelManager::GetFirstSuperstructureMemberID(const CSegmentKey& segmentKey) const
{
   ...
   GET_IFACE2(GetBroker(),IBridge,pBridge);
   GET_IFACE2_NOCHECK(GetBroker(),IBridgeDescription,pIBridgeDesc); // only used if there are more than one segment per girder
...
IndexType CGirderModelManager::GetStressPointIndex(pgsTypes::StressLocation loc) const
{
...
std::pair<Float64,Float64> CGirderModelManager::GetStress(IntervalIndexType intervalIdx,const pgsPointOfInterest& poi,pgsTypes::StressLocation topLoc,pgsTypes::StressLocation botLoc,bool bIncludeLiveLoad, pgsTypes::LimitState limitState, VehicleIndexType vehicleIdx,const GDRCONFIG* pConfig) const
{
   ...
   GET_IFACE2_NOCHECK(GetBroker(),ILosses, pLosses);
...
Float64 CGirderModelManager::GetStressFromSegmentPT(IntervalIndexType intervalIdx, const pgsPointOfInterest& poi, pgsTypes::StressLocation stressLocation, DuctIndexType ductIdx) const
{
   ...
   GET_IFACE2(GetBroker(),ISegmentTendonGeometry, pTendonGeometry);
