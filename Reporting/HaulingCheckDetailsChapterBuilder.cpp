      ...
      for ( GirderIndexType gdrIdx = firstGdrIdx; gdrIdx <= lastGdrIdx; gdrIdx++ )
      {
         ...
         for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
         {
            ...
            *pChapter << p;
            ...
            const pgsHaulingAnalysisArtifact* pHaul = pSegmentArtifact->GetHaulingAnalysisArtifact();
            ...
            *p << p_table<<rptNewLine;
            ...
            poi_vec = pGirderHaulingPointsOfInterest->GetHaulingPointsOfInterest(thisSegmentKey,0,POIFIND_OR);
            ...
            for (i = poi_vec.begin(); i!= poi_vec.end(); i++)
            {
               ...
               const pgsHaulingStressAnalysisArtifact* stressArtifact =  pHaul->GetHaulingStressAnalysisArtifact(poi.GetDistFromStart());
            ...
            *p << RPT_STRESS(_T("t")) << _T(" = governing tension stress")<<rptNewLine;
            ...
            for (i = poi_vec.begin(); i!= poi_vec.end(); i++)
            {
               ...
               const pgsHaulingCrackingAnalysisArtifact* crackArtifact =  pHaul->GetHaulingCrackingAnalysisArtifact(poi.GetDistFromStart());
...
void CHaulingCheckDetailsChapterBuilder::BuildRebarTable(IBroker* pBroker,rptChapter* pChapter, const CSegmentKey& segmentKey,
                                                         ImpactDir dir) const
{
   GET_IFACE2(pBroker,IEAFDisplayUnits,pDisplayUnits);
   rptRcScalar scalar;
   ...
   *pChapter << p;
   ...
   const pgsHaulingAnalysisArtifact* pHaul = pArtifact->GetHaulingAnalysisArtifact();
