...
rptChapter* CLiftingCheckDetailsChapterBuilder::Build(CReportSpecification* pRptSpec,Uint16 level) const
{
   ...
   *pChapter << pTitle;
   *pTitle << _T("Details for Check for Lifting at Release [5.5.4.3]")<<rptNewLine;
      ...
      for ( GirderIndexType gdrIdx = firstGdrIdx; gdrIdx <= lastGdrIdx; gdrIdx++ )
      {
         ...
         for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
         {
            ...
            *pChapter << p;
            ...
            const pgsLiftingCheckArtifact* pLift = pSegmentArtifact->GetLiftingCheckArtifact();
            ...
            *p << Sub2(_T("y"),_T("t"))<<_T(" = Vertical distance from pick point to girder C.G. = ")<<dim.SetValue(pLift->GetVerticalDistanceFromPickPointToGirderCg())<<_T(" ")<<dim.GetUnitTag()<<rptNewLine;
            ...
            *p << Sub2(_T("e"),_T("s"))<<_T(" = eccentricity due to sweep = ")<<dim.SetValue(pLift->GetEccentricityDueToSweep())<<_T(" ")<<dim.GetUnitTag()<<rptNewLine;
            *p << Sub2(_T("e"),_T("lift"))<<_T(" = eccentricity due to placement tolerance = ")<<dim.SetValue(pLift->GetEccentricityDueToPlacementTolerance())<<_T(" ")<<dim.GetUnitTag()<<rptNewLine;
            *p << Sub2(_T("F"),_T("o"))<<_T(" = offset factor = ") << _T("(") << Sub2(_T("L"),_T("l")) << _T("/") << Sub2(_T("L"),_T("g")) << _T(")") << Super(_T("2")) << _T(" - 1/3 = ") <<pLift->GetOffsetFactor()<<rptNewLine;
            *p << Sub2(_T("e"),_T("i"))<<_T(" = total initial eccentricity = e")<<Sub(_T("s"))<<_T("*F")<<Sub(_T("o"))<<_T(" + e")<<Sub(_T("lift"))<<_T(" = ")<<dim.SetValue(pLift->GetTotalInitialEccentricity())<<_T(" ")<<dim.GetUnitTag()<<rptNewLine;
            ...
            *p << _T("Adjusted ") << Sub2(_T("y"),_T("r"))<<_T(" = ") << Sub2(_T("y"),_T("t")) << _T(" - ")<< Sub2(_T("F"),_T("o")) << _T("(") << symbol(DELTA)<<Sub(_T("total"))<<_T(") = ")<<dim.SetValue(pLift->GetAdjustedYr())<<_T(" ")<<dim.GetUnitTag()<<rptNewLine;
            ...
            *p << Sub2(symbol(theta),_T("i")) << _T(" = initial tilt angle = ") << Sub2(_T("e"),_T("i")) << _T(" / ") << Sub2(_T("y"),_T("r"))<<_T(" = ")<<angle.SetValue(pLift->GetInitialTiltAngle())<<_T(" ")<<angle.GetUnitTag()<<rptNewLine;
            ...
            *p << p_table<<rptNewLine;
            ...
            Float64 overhang = pLift->GetLeftOverhang();
            ATLASSERT(IsEqual(pLift->GetLeftOverhang(),pLift->GetRightOverhang())); // the assumption is that overhangs are equal

            GET_IFACE2(pBroker,IGirderLiftingPointsOfInterest,pGirderLiftingPointsOfInterest);
            std::vector<pgsPointOfInterest> poi_vec( pGirderLiftingPointsOfInterest->GetLiftingPointsOfInterest(thisSegmentKey,0,POIFIND_OR) );

            RowIndexType row=1;
            std::vector<pgsPointOfInterest>::const_iterator i(poi_vec.begin());
            std::vector<pgsPointOfInterest>::const_iterator end(poi_vec.end());
            for ( ; i != end; i++)
            {
            ...
            *p << p_table2 << rptNewLine;
            ...
            for ( ; i != end; i++)
            {
               ...
               (*p_table)(row1,0) << location.SetValue( POI_LIFT_SEGMENT,poi,overhang );
               ...
               (*p_table2)(row2,0) << location.SetValue( POI_LIFT_SEGMENT, poi, overhang );
      ...
      for ( GirderIndexType gdrIdx = firstGdrIdx; gdrIdx <= lastGdrIdx; gdrIdx++ )
      {
         ...
         for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
         {
            ...
            *pChapter << p;
            ...
            const pgsSegmentArtifact* pSegmentArtifact = pArtifacts->GetSegmentArtifact(thisSegmentKey);
            const pgsLiftingAnalysisArtifact* pLift = pSegmentArtifact->GetLiftingAnalysisArtifact();
                                                         ...
                                                         ImpactDir dir) const
{
   ...
   *p << pTable << rptNewLine;
   ...
   for (std::vector<pgsPointOfInterest>::iterator i = vPoi.begin(); i!= vPoi.end(); i++)
   {
      ...
      (*pTable)(row,0) << location.SetValue( POI_LIFT_SEGMENT, poi, overhang );
