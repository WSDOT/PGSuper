...
rptChapter* CLiftingCheckDetailsChapterBuilder::Build(CReportSpecification* pRptSpec,Uint16 level) const
{
   ...
   INIT_UV_PROTOTYPE( rptAngleUnitValue, degangle,        pDispUnit->GetAngleUnit(),         false );
   ...
   *p << p_table2 << rptNewLine;
   (*p_table2)(0,0) << COLHDR("Location from" << rptNewLine << "Left Pick Point",    rptLengthUnitTag, pDispUnit->GetSpanLengthUnit() );
   (*p_table2)(0,1) << COLHDR(Sub2("Y","na")<<rptNewLine<<" Impact" << rptNewLine << "Up",rptLengthUnitTag, pDispUnit->GetComponentDimUnit() );
   (*p_table2)(0,2) << COLHDR(Sub2("Y","na")<<rptNewLine<<" No" << rptNewLine << "Impact",rptLengthUnitTag, pDispUnit->GetComponentDimUnit() );
   (*p_table2)(0,3) << COLHDR(Sub2("Y","na")<<rptNewLine<<" Impact" << rptNewLine << "Down",rptLengthUnitTag, pDispUnit->GetComponentDimUnit() );
   (*p_table2)(0,4) << COLHDR(Sub2("A","t"),rptAreaUnitTag, pDispUnit->GetAreaUnit() );
   (*p_table2)(0,5) << COLHDR("T",rptForceUnitTag, pDispUnit->GetGeneralForceUnit() );
   (*p_table2)(0,6) << COLHDR(Sub2("A","s"),rptAreaUnitTag, pDispUnit->GetAreaUnit() );
   ...
   *p << rptRcImage(pgsReportStyleHolder::GetImagePath() + "FS Cracking - Lifting.jpg" )<<rptNewLine;
   ...
   (*p_table)(0,0) << COLHDR("Location from" << rptNewLine << "Left Pick Point",    rptLengthUnitTag, pDispUnit->GetSpanLengthUnit() );
   (*p_table)(0,1) << COLHDR("f"<<Sub("t"),rptStressUnitTag, pDispUnit->GetStressUnit() );
   ...
   (*p_table)(0,5) << COLHDR(symbol(theta)<<Sub("max"),rptAngleUnitTag, pDispUnit->GetRadAngleUnit() );
