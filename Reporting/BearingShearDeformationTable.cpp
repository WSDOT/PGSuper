...
RowIndexType ConfigureThermalDeformationTableHeading(std::shared_ptr<WBFL::EAF::Broker> pBroker, rptRcTable* p_table,
    std::shared_ptr<IEAFDisplayUnits> pDisplayUnits, SHEARDEFORMATIONDETAILS* pDetails)
...
rptRcTable* CBearingShearDeformationTable::BuildThermalDeformationTable(std::shared_ptr<WBFL::EAF::Broker> pBroker, const CGirderKey& girderKey,
    std::shared_ptr<IEAFDisplayUnits> pDisplayUnits, bool bCold, SHEARDEFORMATIONDETAILS* details) const
{
    ...
    if (bCold)
    {
        label = _T("Bearing Design Thermal Deformations - Cold Climate");
    }
    else
    {
        label = _T("Bearing Design Thermal Deformations - Moderate Climate");
    }



    rptRcTable* p_table = rptStyleManager::CreateDefaultTable(nCols, label);
    RowIndexType row = ConfigureThermalDeformationTableHeading(pBroker, p_table, pDisplayUnits, details);
...
RowIndexType ConfigureTendonDeformationTableHeading(std::shared_ptr<WBFL::EAF::Broker> pBroker, rptRcTable* p_table,
    std::shared_ptr<IEAFDisplayUnits> pDisplayUnits, SHEARDEFORMATIONDETAILS* pDetails)

{
    RowIndexType rowSpan = 2;
    ...
    else
    {
        if (pLossParams->GetLossMethod() != PrestressLossCriteria::LossMethodType::TIME_STEP)
        {
            ...
            (*p_table)(0, col) << Sub2(symbol(DELTA) << _T("L"), _T("ten"));
...
rptRcTable* CBearingShearDeformationTable::BuildTendonDeformationTable(std::shared_ptr<WBFL::EAF::Broker> pBroker, const CGirderKey& girderKey,
    std::shared_ptr<IEAFDisplayUnits> pDisplayUnits, SHEARDEFORMATIONDETAILS* details) const
{
    ...
    CString label{ _T("Deformation at Tendon Centroid due to Time-Dependent Losses") };

    rptRcTable* p_table = rptStyleManager::CreateDefaultTable(nCols, label);
    RowIndexType row = ConfigureTendonDeformationTableHeading(pBroker, p_table, pDisplayUnits, details);
...
RowIndexType ConfigureShearDeformationTableHeading(std::shared_ptr<WBFL::EAF::Broker> pBroker, rptRcTable* p_table,
    std::shared_ptr<IEAFDisplayUnits> pDisplayUnits, SHEARDEFORMATIONDETAILS* pDetails)

{
    ...
    if (pLossParams->GetLossMethod() == PrestressLossCriteria::LossMethodType::GENERAL_LUMPSUM)
    {
        ...
        (*p_table)(0, col++) << COLHDR(symbol(SUM) << Sub2(symbol(DELTA), _T("s")), rptLengthUnitTag, pDisplayUnits->GetDeflectionUnit());
    }
    else
    {
        if (pLossParams->GetLossMethod() != PrestressLossCriteria::LossMethodType::TIME_STEP)
        {
            p_table->SetColumnSpan(0, col, 3);
            (*p_table)(0, col) << Sub2(symbol(DELTA), _T("s"));
            (*p_table)(1, col++) << COLHDR(_T("Creep"), rptLengthUnitTag, pDisplayUnits->GetDeflectionUnit());
            (*p_table)(1, col++) << COLHDR(_T("Shrinkage"), rptLengthUnitTag, pDisplayUnits->GetDeflectionUnit());
            (*p_table)(1, col++) << COLHDR(_T("Relaxation"), rptLengthUnitTag, pDisplayUnits->GetDeflectionUnit());
        }
...
rptRcTable* CBearingShearDeformationTable::BuildShearDeformationTable(std::shared_ptr<WBFL::EAF::Broker> pBroker, const CGirderKey& girderKey,
    std::shared_ptr<IEAFDisplayUnits> pDisplayUnits, SHEARDEFORMATIONDETAILS* details) const
{
    ...
    label = _T("Bearing Design Time-Dependent Shear Deformations");

    rptRcTable* p_table = rptStyleManager::CreateDefaultTable(nCols, label);
    RowIndexType row = ConfigureShearDeformationTableHeading(pBroker, p_table, pDisplayUnits, details);
