...
rptChapter* CDistributionFactorDetailsChapterBuilder::Build(CReportSpecification* pRptSpec,Uint16 level) const
{
   ...
   for ( SpanIndexType spanIdx = firstSpanIdx; spanIdx < lastSpanIdx; spanIdx++ )
   {
      ...
      for ( GirderIndexType gdrIdx = firstGirderIdx; gdrIdx < lastGirderIdx; gdrIdx++ )
      {
         ...
         CLiveLoadDistributionFactorTable().Build(pChapter,pBroker,spanIdx,gdrIdx,pDisplayUnits);
         pDistFact->ReportDistributionFactors(spanIdx,gdrIdx,pChapter,pDisplayUnits);
      }
   }
   return pChapter;
}
