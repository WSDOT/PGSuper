...
rptChapter* CPrincipalTensionStressDetailsChapterBuilder::Build(const std::shared_ptr<const WBFL::ReportMgr::ReportSpecification>& pRptSpec, Uint16 level) const
{
   ...
   for (SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++)
   {
      ...
      else
      {
         ...
         else if (pArtifact->GetApplicability() == pgsPrincipalTensionStressArtifact::ConcreteStrength)
         {
            ...
            *pPara << _T("Concrete strength does not exceed the ") << stress_u.SetValue(threshold) << _T(" threshold") << rptNewLine;
         }
         else
         {
