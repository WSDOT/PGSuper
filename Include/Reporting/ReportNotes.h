...
#define LIVELOAD_PER_BEARING           _T("* Live Load values are per bearing and include impact.")
#define LIVELOAD_PER_BEARING_NO_IMPACT _T("* Live Load values are per bearing and do not include impact.")

#define STRUT_AND_TIE_REQUIRED _T("* [LRFD ") << WBFL::LRFD::LrfdCw8th(_T("5.8.3.2"),_T("5.7.3.2")) << _T("] The shear stress ")<< RPT_vu << _T(" at the critical section exceeds 0.18") << RPT_FC << _T(" and the beam-type element is not built integrally with the support. The end region shall be designed using the strut-and-tie model specified in LRFD ") << WBFL::LRFD::LrfdCw8th(_T("5.6.3"),_T("5.8.2"))
#define SUPPORT_COMPRESSION    _T("[LRFD ") << WBFL::LRFD::LrfdCw8th(_T("5.8.3.2"),_T("5.7.3.2")) << _T("] The reaction introduces compression into the end of the girder. Load between the CSS and the support is transferred directly to the support by compressive arching action without causing additional stresses in the stirrups. Hence, ") << Sub2(_T("A"),_T("v")) << _T("/S in this region must be equal or greater than ") << Sub2(_T("A"),_T("v")) << _T("/S at the critical section.")
