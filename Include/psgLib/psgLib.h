...
public:
   ...
   void AddConflict(const WBFL::Library::ILibrary& rLib,
   ...
   bool IsConflict(const WBFL::Library::ILibrary& rLib, const std::_tstring& entryName, std::_tstring* pNewName) const
   {
      std::_tstring type_name(rLib.GetDisplayName());
...
LibConflictOutcome PSGLIBFUNC WINAPI psglibResolveLibraryEntryConflict(const std::_tstring& strPublisher, const std::_tstring& strConfiguration, const std::_tstring& entryName, const std::_tstring& libName, const std::vector<std::_tstring>& keylists, bool isImported, const std::vector<std::unique_ptr<DifferenceItem>>& vDifferences, bool bMustRename, std::_tstring* pNewName);

bool PSGLIBFUNC WINAPI psglibImportEntries(IStructuredLoad* pStrLoad,psgLibraryManager* pLibMgr);

// Use this function when reading library information from a project file. This function advances
