...
struct PSGLIBCLASS LiveLoadDistributionCriteria
{
   ...
   bool operator!=(const LiveLoadDistributionCriteria& other) const;
   bool Compare(const LiveLoadDistributionCriteria& other, const SpecLibraryEntryImpl& impl, std::vector<std::unique_ptr<PGS::Library::DifferenceItem>>& vDifferences,bool bReturnOnFirstDifference) const;
