...
namespace PGS
{
   namespace Beams
   {
      /// @brief Interface for computing and report prestress losses
      class BEAMSCLASS PsLossEngineerBase
      {
      public:
         PsLossEngineerBase() = delete;
         PsLossEngineerBase(std::weak_ptr<WBFL::EAF::Broker> pBroker, StatusGroupIDType statusGroupID) :
            m_pBroker(pBroker), m_StatusGroupID(statusGroupID)
         {
         }
         PsLossEngineerBase(const PsLossEngineerBase&) = delete;
         virtual ~PsLossEngineerBase() = default;

         PsLossEngineerBase& operator=(const PsLossEngineerBase&) = delete;
