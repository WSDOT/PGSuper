...
class BEAMSCLASS CDistFactorEngineerBase
{
public:
   CDistFactorEngineerBase() = delete;
   CDistFactorEngineerBase(std::weak_ptr<WBFL::EAF::Broker> pBroker, StatusGroupIDType statusGroupID) :
      m_pBroker(pBroker), m_StatusGroupID(statusGroupID)
   {
   }
   CDistFactorEngineerBase(const CDistFactorEngineerBase&) = delete;
   virtual ~CDistFactorEngineerBase() = default;
   
   CDistFactorEngineerBase& operator=(const CDistFactorEngineerBase&) = delete;
