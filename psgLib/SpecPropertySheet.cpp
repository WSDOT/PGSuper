...
BOOL CSpecPropertySheet::OnInitDialog()
{
   ...
   // before there is a previously visited node opens the wrong page.
...
WBFL::LRFD::BDSManager::Edition CSpecPropertySheet::GetSpecVersion()
{
   return m_Entry.m_pImpl->GetSpecificationCriteria().GetEdition();
}
...
void CSpecPropertySheet::ExchangeDesignConcreteData(CDataExchange* pDX)
{
   ...
   int value = (int)m_Entry.m_pImpl->m_LimitStateConcreteStrengthCriteria.LimitStateConcreteStrength;
...
void CSpecPropertySheet::ExchangeGirderTemporaryData(CDataExchange* pDX)
{
   ...
   DDX_UnitValueAndTag(pDX, IDC_RELEASE_TENSION_WITH_REBAR, IDC_RELEASE_TENSION_WITH_REBAR_UNIT, m_Entry.m_pImpl->m_PrestressedElementCriteria.TensionStressLimit_WithReinforcement_BeforeLosses.Coefficient, pDisplayUnits->SqrtPressure);
...
void CSpecPropertySheet::ExchangeClosureFinalData(CDataExchange* pDX)
{
   ...
   if (WBFL::LRFD::BDSManager::Edition::SeventhEditionWith2016Interims <= WBFL::LRFD::BDSManager::Edition::LastEdition)
   {
      tagBeforeLosses = _T("(lambda)") + tagBeforeLosses;
      tagAfterLosses = _T("(lambda)") + tagAfterLosses;
   }
...
{
   DDX_CBEnum(pDX, IDC_HAULING_METHOD, m_Entry.m_pImpl->m_HaulingCriteria.AnalysisMethod);
}
...
void CSpecPropertySheet::ExchangeHaulingKDOTData(CDataExchange* pDX)
{
   ...
   if (pDX->m_bSaveAndValidate)
   {
      //ATLASSERT(m_Hauling_KDOT.m_HaulingAnalysisMethod == pgsTypes::HaulingAnalysisMethod::KDOT);
...
void CSpecPropertySheet::ExchangeShearInterfaceData(CDataExchange* pDX)
{
   ...
   DDV_UnitValueGreaterThanZero(pDX, IDC_SMAX, m_Entry.m_pImpl->m_InterfaceShearCriteria.MaxInterfaceShearConnectorSpacing, pDisplayUnits->ComponentDim);
