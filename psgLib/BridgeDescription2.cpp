...
// that have an explicitly construction event (9.0 <= version) for diaphragms and those that do not (version <= 8.0)
...
HRESULT CBridgeDescription2::Load(IStructuredLoad* pStrLoad,std::shared_ptr<IEAFProgress> pProgress)
{
   USES_CONVERSION;
   ...
   try
   {
      ...
      // Sometime in the past, the bearing offset and end distance measurement types were inadvertently allowed to be
      ...
      if ( ::IsBridgeSpacing(m_GirderSpacingType) )
      {
         ...
         // object for the current number of girders.... so we do that here.
         //
      ...
      if (version < 9)
      {
         ...
         // prior to version 9, we didn't explicitly have an event for diaphragms. we assumed diaphragms were installed when segments were erected.
...
HRESULT CBridgeDescription2::Save(IStructuredSave* pStrSave,std::shared_ptr<IEAFProgress> pProgress)
{
   HRESULT hr = S_OK;
...
HRESULT CBridgeDescription2::LoadOldBridgeDescription(Float64 version,IStructuredLoad* pStrLoad,std::shared_ptr<IEAFProgress> pProgress)
{
...
bool CBridgeDescription2::HasStructuralLongitudinalJoints() const
{
   if (IsJointSpacing(m_GirderSpacingType))
   {
      ...
      // NOTE: the logical is a little weird here, but a bridge can have joint spacing, but no longitudinal joint.
