...
bool LiveLoadLibraryEntry::Compare(const LiveLoadLibraryEntry& rOther, std::vector<std::unique_ptr<DifferenceItem>>& vDifferences, bool& bMustRename, bool bReturnOnFirstDifference, bool considerName) const
{
   ...
   if ( m_IsNotional != rOther.m_IsNotional )
   {
      ...
      vDifferences.emplace_back(std::make_unique<DifferenceBooleanItem>(_T("Neglect axles that do not contribute to the maximum load effect under consideration"),m_IsNotional,rOther.m_IsNotional,_T("Checked"),_T("Unchecked")));
   }

   if ( m_LiveLoadConfigurationType != rOther.m_LiveLoadConfigurationType )
   {
      RETURN_ON_DIFFERENCE;
      vDifferences.emplace_back(std::make_unique<DifferenceStringItem>(_T("Load Type"),GetConfigurationType(m_LiveLoadConfigurationType),GetConfigurationType(rOther.m_LiveLoadConfigurationType)));
   }

   if ( m_LiveLoadApplicabilityType != rOther.m_LiveLoadApplicabilityType )
   {
      RETURN_ON_DIFFERENCE;
      vDifferences.emplace_back(std::make_unique<DifferenceStringItem>(_T("Usage"),GetApplicabilityType(m_LiveLoadApplicabilityType),GetApplicabilityType(rOther.m_LiveLoadApplicabilityType)));
   }

   if ( !::IsEqual(m_LaneLoad,rOther.m_LaneLoad) )
   {
      RETURN_ON_DIFFERENCE;
      vDifferences.emplace_back(std::make_unique<DifferenceForcePerLengthItem>(_T("Lane Load"),m_LaneLoad,rOther.m_LaneLoad,pDisplayUnits->ForcePerLength));
   }

   if ( !::IsEqual(m_LaneLoadSpanLength,rOther.m_LaneLoadSpanLength) )
   {
      RETURN_ON_DIFFERENCE;
      vDifferences.emplace_back(std::make_unique<DifferenceLengthItem>(_T("Lane Load Minimum Span Length"),m_LaneLoadSpanLength,rOther.m_LaneLoadSpanLength,pDisplayUnits->SpanLength));
   }
   ...
   if ( m_LiveLoadConfigurationType != LiveLoadLibraryEntry::lcLaneOnly )
   {
      ...
      {
         RETURN_ON_DIFFERENCE;
         vDifferences.emplace_back(std::make_unique<DifferenceIndexItem>(_T("Number of Axles"),nAxles,rOther.m_Axles.size()));
      }

      if ( m_VariableAxleIndex != rOther.m_VariableAxleIndex )
      {
         RETURN_ON_DIFFERENCE;
         vDifferences.emplace_back(std::make_unique<DifferenceIndexItem>(_T("Variable Axle Index"),m_VariableAxleIndex,rOther.m_VariableAxleIndex));
      }

      if ( !::IsEqual(m_MaxVariableAxleSpacing,rOther.m_MaxVariableAxleSpacing) )
      {
         RETURN_ON_DIFFERENCE;
         vDifferences.emplace_back(std::make_unique<DifferenceLengthItem>(_T("Maximum Variable Axle Spacing"),m_MaxVariableAxleSpacing,rOther.m_MaxVariableAxleSpacing,pDisplayUnits->SpanLength));
      }
      ...
      for ( AxleIndexType axleIdx = 0; axleIdx < nAxles; axleIdx++ )
      {
         ...
         if ( !::IsEqual(axle.Weight,otherAxle.Weight) )
         {
            ...
            vDifferences.emplace_back(std::make_unique<DifferenceForceItem>(strAxle,axle.Weight,otherAxle.Weight,pDisplayUnits->GeneralForce));
         }
         ...
         if ( !::IsEqual(axle.Spacing,otherAxle.Spacing) )
         {
            ...
            vDifferences.emplace_back(std::make_unique<DifferenceLengthItem>(strAxle,axle.Spacing,otherAxle.Spacing,pDisplayUnits->SpanLength));
         }
      }
