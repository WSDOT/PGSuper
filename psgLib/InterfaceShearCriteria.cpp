...
bool InterfaceShearCriteria::Compare(const InterfaceShearCriteria& other, const SpecLibraryEntryImpl& impl, std::vector<std::unique_ptr<DifferenceItem>>& vDifferences, bool bReturnOnFirstDifference) const
{
   bool bSame = true;
   ...
   {
      bSame = false;
      vDifferences.emplace_back(std::make_unique<DifferenceStringItem>(_T("Horizontal Interface Shear requirements are different"), _T(""), _T("")));
