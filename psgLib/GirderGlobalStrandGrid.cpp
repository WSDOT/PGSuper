...
static COLORREF GetEntryColor(long entryIndex)
{
...
static ROWCOL GetEntryLoad(const CGirderGlobalStrandGrid::GlobalStrandGridEntry& entry, bool useHarpedGrid)
{
   ...
   else if (useHarpedGrid && entry.m_Type==GirderLibraryEntry::stHarped && entry.m_Hend_X>0.0)
   {
      return 2;
   }
   else
...
BOOL CGirderGlobalStrandGrid::OnGridKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
   ...
   if (nChar==VK_DOWN)
   {
      long orig_idx =GetRowEntry(row);
      if ( orig_idx >= (long)(m_Entries.size()-1))
      {
...
void CGirderGlobalStrandGrid::InsertSelectedRow()
{
   ...
   if (EditEntry(sel_row, entry, true))
   {
      ...
      long size = m_Entries.size();
      ROWCOL row = sel_row;
      for (long cnt=idx; cnt<size; cnt++)
      {
...
void CGirderGlobalStrandGrid::MoveDownSelectedRow()
{
   ROWCOL sel_row = GetSelectedRow();
   long idx =GetRowEntry(sel_row);
   long num_entries = m_Entries.size();
...
ROWCOL CGirderGlobalStrandGrid::FillRowsWithEntry(ROWCOL nRow, GlobalStrandGridEntry& entry, bool useHarped, COLORREF currColor)
{
   ...
   ROWCOL rows_used = GetEntryLoad(entry, useHarped);
   if (rows_used == 2)
   {
      nRow++;
...
long CGirderGlobalStrandGrid::GetRowEntry(ROWCOL nRow)
{
...
void CGirderGlobalStrandGrid::ReverseHarpedStrandOrder()
{
   ...
   for ( iter = m_Entries.begin(); iter != m_Entries.end(); iter++ )
   {
      ...
      if ( entry.m_Type == GirderLibraryEntry::stHarped )
