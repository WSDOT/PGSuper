   ...
   m_PrivateSlabOffset{0.0,0.0}
{
   ...
   m_EndDistanceMeasurementType   = ConnectionLibraryEntry::EndDistanceMeasurementType::FromBearingNormalToPier;

   m_GirderBearingOffset          = gs_DefaultGirderBearingOffset2;
   m_BearingOffsetMeasurementType = ConnectionLibraryEntry::BearingOffsetMeasurementType::NormalToPier;
...
HRESULT CTemporarySupportData::Load(IStructuredLoad* pStrLoad,std::shared_ptr<IEAFProgress> pProgress,CBridgeDescription2* pBridgeDesc)
{
   USES_CONVERSION;
   CHRException hr;
...
HRESULT CTemporarySupportData::Save(IStructuredSave* pStrSave,std::shared_ptr<IEAFProgress> pProgress)
{
   HRESULT hr = S_OK;
...
{
   // there is only spacing at a temporary support when the segment connection type has a closure joint
...
{
   //ATLASSERT( m_ConnectionType != pgsTypes::tsctContinuousSegment );
   // this is just a warning... spacing doesn't make sense in if the connection is continuous
