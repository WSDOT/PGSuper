...
// FilletGirderGrid.cpp : implementation file
//

#include "PGSuperAppPlugin\stdafx.h"
#include "FilletGirderGrid.h"
...
GRID_IMPLEMENT_REGISTER(CFilletGirderGrid, CS_DBLCLKS, 0, 0, 0);

/////////////////////////////////////////////////////////////////////////////
// CFilletGirderGrid

CFilletGirderGrid::CFilletGirderGrid()
{
//   RegisterClass();
}
...
BEGIN_MESSAGE_MAP(CFilletGirderGrid, CGXGridWnd)
	//{{AFX_MSG_MAP(CFilletGirderGrid)
...
/////////////////////////////////////////////////////////////////////////////
// CFilletGirderGrid message handlers

int CFilletGirderGrid::GetColWidth(ROWCOL nCol)
{
   if ( IsColHidden(nCol) )
...
void CFilletGirderGrid::CustomInit(SpanIndexType nSpans, GirderIndexType maxGirdersPerSpan)
{
   // initialize units
...
void CFilletGirderGrid::FillGrid(const HaunchInputData& haunchData)
{
   GetParam()->EnableUndo(FALSE);
   ...
   FilletSpanDataConstIter spanIt = haunchData.m_FilletSpans.begin();
   for (; spanIt!=haunchData.m_FilletSpans.end(); spanIt++)
   {
      ROWCOL row = rowStart;
      const FilletSpanData& hpv = *spanIt;
      for (std::vector<Float64>::const_iterator gdrIt = hpv.m_FilletsForGirders.begin(); gdrIt!=hpv.m_FilletsForGirders.end(); gdrIt++)
      {
         Float64 fillet = *gdrIt;
         ...
         SetStyleRange(CGXRange(row,col), CGXStyle()
            ...
            .SetValue(FormatDimension(fillet,*m_pCompUnit, false))
            );
...
void CFilletGirderGrid::GetData(HaunchInputData* pData, CDataExchange* pDX)
{
   pData->m_FilletType = pgsTypes::fttGirder;

   // Number of spans and girders cannot change so use original data structure to iterate into grid
   SpanIndexType span = 0;
   FilletSpanDataIter spanIt = pData->m_FilletSpans.begin();
   for (; spanIt!=pData->m_FilletSpans.end(); spanIt++)
   {
      GirderIndexType gdr = 0;
      FilletSpanData& rhpv = *spanIt;
      for (std::vector<Float64>::iterator gdrIt = rhpv.m_FilletsForGirders.begin(); gdrIt!=rhpv.m_FilletsForGirders.end(); gdrIt++)
      {
         Float64* pVal = &(*gdrIt);

         // Extract converted value from grid
         *pVal = GetFilletAtCells( span, gdr, pDX);
...
Float64 CFilletGirderGrid::GetFilletAtCells(SpanIndexType span, GirderIndexType gdr, CDataExchange* pDX)
{
   Float64 val;
   ...
   if (!str.IsEmpty())
   {
      ...
      {
         CString str;
         str.Format(_T("Fillet value at Span %s, Girder %s is not a number - must be a positive number"), LABEL_SPAN(span), LABEL_GIRDER(gdr));
...
CString CFilletGirderGrid::GetCellValue(ROWCOL nRow, ROWCOL nCol)
{
