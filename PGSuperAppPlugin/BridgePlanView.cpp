    ...
    std::shared_ptr<WBFL::DManip::CompoundDrawPointStrategy>& pStrategy)
{
    ...
    // Keep a gravity-well from one of the created shape strategies (first one)
    std::shared_ptr<WBFL::DManip::iGravityWellStrategy> gravity_well_ptr;
    ...
    for (BearingIndexType brgIdx = 0; brgIdx < bearingCnt; brgIdx++)
    {
        ...
        if (!gravity_well_ptr)
        {
            gravity_well_ptr = std::dynamic_pointer_cast<WBFL::DManip::iGravityWellStrategy>(shapeDrawStrategy);
        }
    }
    ...
    // use the captured gravity well (if any)
    doBearing->SetGravityWellStrategy(gravity_well_ptr);
