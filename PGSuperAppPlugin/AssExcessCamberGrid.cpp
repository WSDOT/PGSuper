...
GRID_IMPLEMENT_REGISTER(CAssExcessCamberGrid, CS_DBLCLKS, 0, 0, 0);

/////////////////////////////////////////////////////////////////////////////
// CAssExcessCamberGrid

CAssExcessCamberGrid::CAssExcessCamberGrid()
{
//   RegisterClass();
}
...
BEGIN_MESSAGE_MAP(CAssExcessCamberGrid, CGXGridWnd)
	//{{AFX_MSG_MAP(CAssExcessCamberGrid)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void CAssExcessCamberGrid::CustomInit(CSplicedGirderData* pGirder)
{
   m_pGirder = pGirder;
   ...
   CString strLabel;
   strLabel.Format(_T("Assumed Excess Camber (%s)"),pDisplayUnits->GetComponentDimUnit().UnitOfMeasure.UnitTag().c_str());
...
void CAssExcessCamberGrid::FillGrid()
{
   CComPtr<IBroker> pBroker;
   EAFGetBroker(&pBroker);
   GET_IFACE2(pBroker,IEAFDisplayUnits,pDisplayUnits);
   ...
   for ( SpanIndexType iSpan = 0; iSpan < nSpansThisGroup; iSpan++ )
   {
      ...
      Float64 AssExcessCamber = pSpan->GetAssExcessCamber(gdrIdx);

      CString strValue;
      strValue.Format(_T("%s"),::FormatDimension(AssExcessCamber,pDisplayUnits->GetComponentDimUnit(),false));
...
CString CAssExcessCamberGrid::GetCellValue(ROWCOL nRow, ROWCOL nCol)
{
...
void CAssExcessCamberGrid::UpdateAssExcessCamberData()
{
   CComPtr<IBroker> pBroker;
   EAFGetBroker(&pBroker);
   GET_IFACE2(pBroker,IEAFDisplayUnits,pDisplayUnits);
   ...
   pgsTypes::AssExcessCamberType ft = m_pGroup->GetBridgeDescription()->GetAssExcessCamberType();
   ...
   for ( ROWCOL col = 1; col <= nCol; col++ )
   {
      ...
      Float64 assExcessCamber = _tstof(GetCellValue(row,col));
      assExcessCamber = ::ConvertToSysUnits(assExcessCamber,pDisplayUnits->GetComponentDimUnit().UnitOfMeasure);

      if (pgsTypes::aecGirder==ft)
      {
         pSpan->SetAssExcessCamber(gdrIdx, assExcessCamber);
      }
      else
      {
...
bool CAssExcessCamberGrid::SelectSingleValue(Float64* pValue)
{
   CComPtr<IBroker> pBroker;
   EAFGetBroker(&pBroker);
   GET_IFACE2(pBroker,IEAFDisplayUnits,pDisplayUnits);
   ...
   else
   {
      ...
      CSelectItemDlg dlg;
      dlg.m_ItemIdx = 0;
      dlg.m_strTitle = _T("Select Assumed Excess Camber Value");
      dlg.m_strLabel = _T("A single Assumed Excess Camber will be used for the entire bridge. Select a value.");
...
void CAssExcessCamberGrid::EnableWindow(BOOL bEnable)
{
