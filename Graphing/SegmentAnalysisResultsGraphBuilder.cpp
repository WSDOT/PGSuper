...
struct WebSectionManager
{
   ...
   void CreateGraphSeries(CSegmentAnalysisResultsGraphController* pGraphController, CSegmentAnalysisResultsGraphBuilder& rBuilder, grGraphXY& rGraph, const std::_tstring& graphName, IntervalIndexType intervalIdx,IndexType graphIdx)
   {
      IndexType idx = 0;
...
CGraphBuilder* CSegmentAnalysisResultsGraphBuilder::Clone() const
{
   // set the module state or the commands wont route to the
   // the graph control window
   AFX_MANAGE_STATE(AfxGetStaticModuleState());
   return new CSegmentAnalysisResultsGraphBuilder(*this);
}
