...
std::string MakeCloneName(const std::string& rOriginalName, GirderIndexType gdr)
{
   ...
   {
      return rOriginalName + std::string("_Original");
   }
   else
   {
      ASSERT(gdr == TOGA_FABR_GDR);
      return rOriginalName + std::string("_PcOptional");
   }
}
...
void CTxDOTOptionalDesignDoc::UpdatePgsuperModelWithData()
{
   ...
   // Set different girder for each girder
//   bridgeDesc.UseSameGirderForEntireBridge(false);
//   bridgeDesc.UseSameNumberOfGirdersInAllSpans( true );
