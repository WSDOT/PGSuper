...
bool CTxDOTOptionalDesignBridgeInputPage::CheckLibraryData()
{
   ...
   if(!::DoParseTemplateFile(template_name, girderEntry, leftConnEntry, rightConnEntry, projectCriteriaEntry, folderName, girderConcreteUnitWeight))
   {
      return false;
   }
...
{
   EAFHelp( EAFGetDocument()->GetDocumentationSetName(), IDH_BRIDGE_DESCRIPTION );
}
