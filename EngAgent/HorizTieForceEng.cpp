...
void pgsHorizTieForceEng::Check(const CGirderKey& girderKey, pgsGirderArtifact* pGdrArtifact) const
{
   ...
   for (iter.First(); !iter.IsDone(); iter.Next())
   {
      ...
      else
      {
         ...
         Float64 cb_left = Nw == 0 ? 0.0 : (bb / 2) * (1. - (Float64)nLeft / (Float64)Nw);
         Float64 cb_right = Nw == 0 ? 0.0 : (bb / 2) * (1. - (Float64)nRight / (Float64)Nw);

         // let h_left and h_right be the horizontal force coefficient for the strut-and-tie model
         // if nLeft/nRight are zero, yp is zero. We need to avoid the divide by zero. That is what
         // the second part of this equation is doing.
         Float64 h_left = xp_left / (hb - yp_left) + (nLeft == 0 ? 0. : (xp_left - cb_left) / yp_left);
         Float64 h_right = xp_right / (hb - yp_right) + (nRight == 0 ? 0. : (xp_right - cb_right) / yp_right);
