...
void pgsRaisedStraightStrandDesignTool::Initialize(IBroker* pBroker, StatusGroupIDType statusGroupID, pgsDesignArtifact* pArtif)
{
   ATLASSERT(pBroker);
   ...
   std::vector<pgsPointOfInterest> pois = IPoi->GetPointsOfInterest(m_Span,m_Girder,pgsTypes::CastingYard,POI_MIDSPAN);
   ATLASSERT(pois.size() == 1);
   pgsPointOfInterest poi = pois[0];

   // Strands must be above NA in order to be considered for raising
   m_Yb = pSectProp->GetYb(pgsTypes::CastingYard, poi);
   m_LowerKern = pSectProp->GetKb(poi);
   ...
   {
       if(riter->m_PermLibraryFillIdx > highestStraightIdx)
       {
