...
void pgsDesigner2::GetHaunchDetails(const CGirderKey& girderKey,bool bUseConfig,const GDRCONFIG& config,HAUNCHDETAILS* pHaunchDetails)
{
   ...
   std::vector<pgsPointOfInterest> vPoi( pPoi->GetPointsOfInterest(CSegmentKey(girderKey,ALL_SEGMENTS),POI_ERECTED_SEGMENT | POI_TENTH_POINTS) );
...
pgsGirderDesignArtifact pgsDesigner2::Design(const CGirderKey& girderKey,arDesignOptions options)
{
#if defined _DEBUG
   ...
   pgsGirderDesignArtifact girderDesignArtifact(girderKey);
   ...
   for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
   {
      ...
      if ( !options.doDesignHauling )
      {
         ...
         Float64 Loh = pSegmentHauling->GetTrailingOverhang(segmentKey);
         Float64 Roh = pSegmentHauling->GetLeadingOverhang(segmentKey);
   ...
   //            return artifact;
            ...
            LOG_EXECUTION_TIME( RefineDesignForUltimateMoment(liveLoadIntervalIdx, pgsTypes::StrengthI,pProgress) );
...
pgsEccEnvelope pgsDesigner2::GetEccentricityEnvelope(const pgsPointOfInterest& poi,const GDRCONFIG& config)
{
   ...
   for ( ; iter != end; iter++ )
   {
      ...
      pLimitStateForces->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::TopGirder,   fcgdr,startSlabOffset,endSlabOffset,batTop,   &fTopMinExt,&fTopMaxExt);
      pLimitStateForces->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::BottomGirder,fcgdr,startSlabOffset,endSlabOffset,batBottom,&fBotMinExt,&fBotMaxExt);

      pgsTypes::LimitState limitState = (liveLoadIntervalIdx <= task.intervalIdx ? pgsTypes::ServiceIII : pgsTypes::ServiceI);
      Float64 Pperm = pPrestressForce->GetPrestressForce(poi,pgsTypes::Permanent,task.intervalIdx,pgsTypes::End,limitState,config);
      Float64 Ptemp = pPrestressForce->GetPrestressForce(poi,pgsTypes::Temporary,task.intervalIdx,pgsTypes::End,limitState,config);
      Float64 Pps = Pperm + Ptemp;

      Float64 k = pLoadFactors->DCmax[task.limitState];
      Pps *= k;
...
void pgsDesigner2::CheckTendonStresses(const CGirderKey& girderKey,pgsGirderArtifact* pGirderArtifact)
{
   ...
   for ( DuctIndexType ductIdx = 0; ductIdx < nDucts; ductIdx++ )
   {
      ...
      GET_IFACE(IPointOfInterest,pPoi);
      std::vector<pgsPointOfInterest> vPoi(pPoi->GetPointsOfInterest(CSegmentKey(girderKey.groupIndex,girderKey.girderIndex,ALL_SEGMENTS)));
      std::vector<pgsPointOfInterest>::iterator iter(vPoi.begin());
      std::vector<pgsPointOfInterest>::iterator end(vPoi.end());
      for ( ; iter != end; iter++ )
      {
...
#pragma Reminder("UPDATE: Allowable stress in Post-Tensioned temporary strands needs to be checked")
   ...
   for ( ; standTypeIter != standTypeIterEnd; standTypeIter++ )
   {
      ...
      {
         pArtifact->SetCheckAtJacking( strandType, pPsForce->GetEffectivePrestress(mid_span_poi,strandType,jackIntervalIdx,pgsTypes::Start,pgsTypes::ServiceI), pAllow->GetAllowableAtJacking(segmentKey,strandType) );
      }

      if ( pAllow->CheckStressBeforeXfer() )
      {
         pArtifact->SetCheckBeforeXfer( strandType, pPsForce->GetEffectivePrestress(mid_span_poi,strandType,jackIntervalIdx,pgsTypes::End/*pgsTypes::BeforeXfer*/,pgsTypes::ServiceI), pAllow->GetAllowableBeforeXfer(segmentKey,strandType) );
      }

      if ( pAllow->CheckStressAfterXfer() )
      {
         pArtifact->SetCheckAfterXfer( strandType, pPsForce->GetEffectivePrestress(mid_span_poi,strandType,releaseIntervalIdx,pgsTypes::Start/*pgsTypes::AfterXfer*/,pgsTypes::ServiceI), pAllow->GetAllowableAfterXfer(segmentKey,strandType) );
      }
...
void pgsDesigner2::CheckSegmentStresses(const CSegmentKey& segmentKey,const std::vector<pgsPointOfInterest>& vPoi,const StressCheckTask& task,pgsSegmentArtifact* pSegmentArtifact)
{
   ...
   for ( ; poiIter != poiIterEnd; poiIter++)
   {
      ...
      for ( int i = 0; i < 4; i++ )
      {
         ...
         if ( IsGirderStressLocation(stressLocation) )
         {
            ...
            if ( pPoi->IsInClosureJoint(poi) )
            {
               ...
               artifact.IsApplicable(stressLocation,compositeClosureIntervalIdx <= task.intervalIdx ? true : false);
            }
            else
            {
               ...
               if ( task.stressType == pgsTypes::Compression )
               {
                  ...
                  artifact.IsApplicable(stressLocation, true);
               }
               else
               {
         ...
         else
         {
            ...
            if ( task.stressType == pgsTypes::Compression )
            {
               ...
               artifact.IsApplicable(stressLocation, compositeDeckIntervalIdx <= task.intervalIdx && bIsDeckPrecompressed ? true : false);
            }
            else
            {
               // tension in the deck
               ...
               else
               {
                  ...
                  artifact.IsApplicable( stressLocation, 
                     compositeDeckIntervalIdx <= task.intervalIdx && // composite deck
                     bIsDeckPrecompressed // deck is precompressed
                     && (
                           (bIsTendonStressingInterval && !bIsInPTZ) || // this is a tendon stressing interval and stress location is NOT in the PTZ -OR-
                           (!bIsTendonStressingInterval && bIsInPTZ) // this is NOT a tendon stressing interval and the stress location is in the PTZ
                         ));
               }
            }
         }
         ...
         // NOTE, don't return here if not applicable. We want to capture the stress information
         ...
         pLimitStateForces->GetStress(task.intervalIdx,task.limitState,poi,bat,false,stressLocation,&fLimitStateMin,&fLimitStateMax);
         Float64 fLimitState = (task.stressType == pgsTypes::Compression ? fLimitStateMin : fLimitStateMax );
         ...
         Float64 f = fLimitState + k*(fPretension + fPosttension);

         f = (IsZero(f) ? 0 : f);

         artifact.SetDemand(             stressLocation, f );
         artifact.SetExternalEffects(    stressLocation, fLimitState);
         artifact.SetPretensionEffects(  stressLocation, fPretension);
         artifact.SetPosttensionEffects( stressLocation, fPosttension);
         ...
         if ( bIsApplicable )
         {
            ...
            else
            {
               ...
               if (0.0 < f)
               {
                  ...
                  fc_reqd = (IsZero(t) ? -99999 : BinarySign(f)*pow(f/t,2));
      ...
      for ( int i = 0; i < 2 && task.stressType == pgsTypes::Tension; i++ )
      {
         ...
         bool IsAdequateRebar[2] = {false,false};

         bool bIsInPTZ[2] = {artifact.IsInPrecompressedTensileZone(topStressLocation),
                             artifact.IsInPrecompressedTensileZone(botStressLocation)};

         Float64 fAllowable[2][2];
         ...
         if ( pAllowable->HasAllowableTensionWithRebarOption(task.intervalIdx,bIsInPTZ[TOP],!pPoi->IsInClosureJoint(poi),segmentKey) )
         {
            ...
            fTopAllowable = altCalc.ComputeAlternativeStressRequirements(poi, NULL, fTop, fBot, fAllowable[TOP][WITHOUT_REBAR], fAllowable[TOP][WITH_REBAR],
            ...
            {
               Float64 fAllowMin = pSegmentArtifact->GetCapacityWithRebar(task.intervalIdx,task.limitState,topStressLocation);
               if ( fAllowMin < 0 )
               {
                  pSegmentArtifact->SetCapacityWithRebar(task.intervalIdx,task.limitState,topStressLocation,fTopAllowable);
               }
               else
               {
                  pSegmentArtifact->SetCapacityWithRebar(task.intervalIdx,task.limitState,topStressLocation,Min(fAllowMin,fTopAllowable));
               }
            }
         }
         ...
         if ( pAllowable->HasAllowableTensionWithRebarOption(task.intervalIdx,bIsInPTZ[BOT],!pPoi->IsInClosureJoint(poi),segmentKey) )
         {
            ...
            Float64 Yna, AreaTens, T, AsProvd, AsReqd;
            fBotAllowable = altCalc.ComputeAlternativeStressRequirements(poi, NULL, fTop, fBot, fAllowable[BOT][WITHOUT_REBAR], fAllowable[BOT][WITH_REBAR],
            ...
            {
               Float64 fAllowMin = pSegmentArtifact->GetCapacityWithRebar(task.intervalIdx,task.limitState,botStressLocation);
               if ( fAllowMin < 0 )
               {
                  pSegmentArtifact->SetCapacityWithRebar(task.intervalIdx,task.limitState,botStressLocation,fBotAllowable);
               }
               else
               {
                  pSegmentArtifact->SetCapacityWithRebar(task.intervalIdx,task.limitState,botStressLocation,Min(fAllowMin,fBotAllowable));
               }
            }
         }
...
void pgsDesigner2::CheckSegmentStressesAtRelease(const CSegmentKey& segmentKey, const GDRCONFIG* pConfig,pgsTypes::StressType type, pgsSegmentArtifact* pSegmentArtifact)
{
   ...
   for ( ; poiIter != poiIterEnd; poiIter++)
   {
      ...
      {
         fTopPretension = pPretensionStresses->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::TopGirder,*pConfig);
         fBotPretension = pPretensionStresses->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::BottomGirder,*pConfig);
      }
   ...
   if ( task.intervalIdx == releaseIntervalIdx && task.stressType == pgsTypes::Tension )
   {
       Float64 fAllow = pAllowable->GetAllowableTensionStress(vPoi.front(),pgsTypes::TopGirder,releaseIntervalIdx,pgsTypes::ServiceI,true/*with bonded rebar*/,false/*other than precompressed tensile zone*/); 
       pSegmentArtifact->SetCapacityWithRebar(releaseIntervalIdx,pgsTypes::ServiceI,pgsTypes::TopGirder,fAllow);
       pSegmentArtifact->SetCapacityWithRebar(releaseIntervalIdx,pgsTypes::ServiceI,pgsTypes::BottomGirder,fAllow);
   }
}

void pgsDesigner2::CreateFlexuralCapacityArtifact(const pgsPointOfInterest& poi,IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,const GDRCONFIG& config,bool bPositiveMoment,pgsFlexuralCapacityArtifact* pArtifact)
{
   ...
   CreateFlexuralCapacityArtifact(poi,intervalIdx,limitState,bPositiveMoment,mcd,mmcd,true/*designing*/,pArtifact);
}

void pgsDesigner2::CreateFlexuralCapacityArtifact(const pgsPointOfInterest& poi,IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,bool bPositiveMoment,pgsFlexuralCapacityArtifact* pArtifact)
{
   GET_IFACE(IMomentCapacity, pMomentCapacity);
   ...
   CreateFlexuralCapacityArtifact(poi,intervalIdx,limitState,bPositiveMoment,mcd,mmcd,false/*checking*/,pArtifact);
}

void pgsDesigner2::CreateFlexuralCapacityArtifact(const pgsPointOfInterest& poi,IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,bool bPositiveMoment,const MOMENTCAPACITYDETAILS& mcd,const MINMOMENTCAPDETAILS& mmcd,bool bDesign,pgsFlexuralCapacityArtifact* pArtifact)
{
   GET_IFACE(ILimitStateForces, pLimitStateForces);
   GET_IFACE(ILibrary,pLib);
   GET_IFACE(ISpecification, pSpec);
   ...
   if ( bPositiveMoment )
   {
      ...
      if ( bDesign )
      {
         ...
         GET_IFACE(ILoadFactors,pLF);
         const CLoadFactors* pLoadFactors = pLF->GetLoadFactors();
         Float64 k = pLoadFactors->DCmax[limitState];
                                                    ...
                                                    pgsStirrupCheckAtPoisArtifact* pArtifact)
{
   ...
   if( pConfig == NULL)
   {
      pShearCapacity->GetShearCapacityDetails( limitState, intervalIdx, poi, &scd );
   }
   else
   {
                                      ...
                                      pgsLongReinfShearArtifact* pArtifact )
{
   ...
   if ( bInCriticalSectionZone )
   {
      ...
      if(pConfig == NULL)
      {
         pShearCapacity->GetShearCapacityDetails(limitState,intervalIdx,csPoi,&scd2);
      }
      else
      {
...
void pgsDesigner2::CheckMomentCapacity(IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,pgsGirderArtifact* pGirderArtifact)
{
...
void pgsDesigner2::InitShearCheck(const CSegmentKey& segmentKey,IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,const GDRCONFIG* pConfig)
{
   GET_IFACE(ISpecification,pSpec);
   pgsTypes::AnalysisType analysisType = pSpec->GetAnalysisType();
   ...
   for ( ; csIter != csIterEnd; csIter++ )
   {
      ...
      if ( pConfig == NULL )
      {
         pShearCapacity->GetShearCapacityDetails( limitState, intervalIdx, csPoi, &scd );
      }
      else
      {
...
void pgsDesigner2::CheckShear(IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,pgsGirderArtifact* pGirderArtifact)
{
   const CGirderKey& girderKey(pGirderArtifact->GetGirderKey());
   ...
   for (SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
   {
      ...
      CheckShear(false,pSegmentArtifact->GetSegmentKey(),intervalIdx,limitState,NULL,pStirrupArtifact);
   }
}

void pgsDesigner2::CheckShear(bool bDesign,const CSegmentKey& segmentKey,IntervalIndexType intervalIdx,
                              pgsTypes::LimitState limitState,const GDRCONFIG* pConfig,pgsStirrupCheckArtifact* pStirrupArtifact)
{
#if defined _DEBUG
   // Checking shear should only be occuring at the final condition.... that is, only in intervals
   // after the live load is applied
...
void pgsDesigner2::CheckSplittingZone(const CSegmentKey& segmentKey,const GDRCONFIG* pConfig,pgsStirrupCheckArtifact* pStirrupArtifact)
{
   ...
   if (pConfig == NULL)
   {
      start_Fpj  = pPrestressForce->GetEffectivePrestress(start_poi, pgsTypes::Permanent, jackingIntervalIdx, pgsTypes::Start,pgsTypes::ServiceI);
      start_dFpT = pLosses->GetPrestressLoss( start_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End,pgsTypes::ServiceI);

      end_Fpj  = pPrestressForce->GetEffectivePrestress(end_poi, pgsTypes::Permanent, jackingIntervalIdx, pgsTypes::Start,pgsTypes::ServiceI);
      end_dFpT = pLosses->GetPrestressLoss( end_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End,pgsTypes::ServiceI);
   }
   else
   {
      start_Fpj  = pPrestressForce->GetEffectivePrestress(start_poi, pgsTypes::Permanent, jackingIntervalIdx, pgsTypes::Start,pgsTypes::ServiceI, *pConfig);
      start_dFpT = pLosses->GetPrestressLoss( start_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End,pgsTypes::ServiceI, *pConfig);

      end_Fpj  = pPrestressForce->GetEffectivePrestress(end_poi, pgsTypes::Permanent, jackingIntervalIdx, pgsTypes::Start,pgsTypes::ServiceI, *pConfig);
      end_dFpT = pLosses->GetPrestressLoss( end_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End,pgsTypes::ServiceI, *pConfig);
   }
   ...
   if ( pConfig == NULL )
   {
      ...
      Ns = pStrandGeometry->GetStrandCount(segmentKey,pgsTypes::Straight);
      Nh = pStrandGeometry->GetStrandCount(segmentKey,pgsTypes::Harped);
      Nt = pStrandGeometry->GetStrandCount(segmentKey,pgsTypes::Temporary);
   ...
   else
   {
      ...
      Nt = pConfig->PrestressConfig.GetStrandCount(pgsTypes::Temporary);
...
void pgsDesigner2::CheckSegmentStability(const CSegmentKey& segmentKey,pgsSegmentStabilityArtifact* pArtifact)
{
   ///////////////////////////////////////////////////////////////
   //
...
void pgsDesigner2::CheckConstructability(const CGirderKey& girderKey,pgsConstructabilityArtifact* pArtifact)
{
   GET_IFACE(ILibrary, pLib );
   GET_IFACE(ISpecification, pSpec );
   ...
   else
   {
      ...
      pArtifact->SetProvidedSlabOffset( providedSlabOffset );
...
void pgsDesigner2::DesignEndZone(bool firstPass, arDesignOptions options, pgsSegmentDesignArtifact& artifact, IProgress* pProgress)
{
...
void pgsDesigner2::DesignEndZoneDebonding(bool firstPass, arDesignOptions options, pgsSegmentDesignArtifact& artifact, IProgress* pProgress)
{
...
void pgsDesigner2::DesignEndZoneHarping(arDesignOptions options, pgsSegmentDesignArtifact& artifact, IProgress* pProgress)
{
...
void pgsDesigner2::DesignMidZoneFinalConcrete(IProgress* pProgress)
{
   ...
   vConcreteStrengthParameters.push_back(ConcreteStrengthParameters(pgsTypes::ServiceI,_T("Service I (BSS3)"),liveLoadIntervalIdx,pgsTypes::Compression,pgsTypes::BottomGirder,POI_HARPINGPOINT|POI_PSXFER));
   ...
   for ( ; iter != end; iter++ )
   {
      ...
      std::vector<pgsPointOfInterest> vPOI( m_StrandDesignTool.GetDesignPoi(concParams.intervalIdx,concParams.find_type) );
      ...
      for ( ; poiIter != poiIterEnd; poiIter++)
      {
         ...
         if ( analysisType == pgsTypes::Envelope )
         {
            pForces->GetDesignStress(concParams.intervalIdx,concParams.limit_state,poi,concParams.stress_location,fc_current,startSlabOffset,endSlabOffset,pgsTypes::MaxSimpleContinuousEnvelope,&min,&max);
         }
         else
         {
            pForces->GetDesignStress(concParams.intervalIdx,concParams.limit_state,poi,concParams.stress_location,fc_current,startSlabOffset,endSlabOffset,analysisType == pgsTypes::Simple ? pgsTypes::SimpleSpan : pgsTypes::ContinuousSpan,&min,&max);
         }
         ...
         if (concParams.stress_type == pgsTypes::Tension)
         {
            ...
            {
               concParams.fmax = max;
               concParams.fbpre = pPrestress->GetDesignStress(concParams.intervalIdx,concParams.limit_state,poi,concParams.stress_location,config);
...
void pgsDesigner2::DesignMidZoneAtRelease(const arDesignOptions& options, IProgress* pProgress)
{
   ...
   for ( ; poiIter != poiIterEnd; poiIter++)
   {
      ...
      fBotPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::BottomGirder,config);
   ...
   for ( ; poiIter != poiIterEnd; poiIter++)
   {
      ...
      Float64 fTopPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::TopGirder,config);
...
void pgsDesigner2::DesignMidZoneInitialStrands(bool bUseCurrentStrands,IProgress* pProgress)
{
   ...
   vInitialDesignParameters.push_back(InitialDesignParameters(liveLoadIntervalIdx,pgsTypes::ServiceI,_T("Service I"),pgsTypes::TopGirder,_T("Top"),pgsTypes::Compression));
   vInitialDesignParameters.push_back(InitialDesignParameters(liveLoadIntervalIdx,lrfdVersionMgr::GetVersion() < lrfdVersionMgr::FourthEditionWith2009Interims ? pgsTypes::ServiceIA : pgsTypes::FatigueI,lrfdVersionMgr::GetVersion() < lrfdVersionMgr::FourthEditionWith2009Interims ? _T("Service IA") : _T("Fatigue I"),pgsTypes::TopGirder,_T("Top"),pgsTypes::Compression));
   vInitialDesignParameters.push_back(InitialDesignParameters(liveLoadIntervalIdx,pgsTypes::ServiceIII,_T("Service III"),pgsTypes::BottomGirder,_T("Bottom"),pgsTypes::Tension));
   if ( pAllowStress->CheckFinalDeadLoadTensionStress() )
   {
      vInitialDesignParameters.push_back(InitialDesignParameters(railingSystemIntervalIdx,pgsTypes::ServiceI,_T("Service I"),pgsTypes::BottomGirder,_T("Bottom"),pgsTypes::Tension));
   }
   ...
   do
   {
      ...
      // even though we are designing for the live load interval, we want the eccentricity
      // for the release interval. prestressing is applied to the section at release. using
      // liveLoadIntervalIdx would give the eccentricity based on the composite section when
      // live load is applied.
      Float64 ecc;
      ecc = m_StrandDesignTool.ComputeEccentricity(poi, releaseIntervalIdx);
      ...
      for ( ; iter != end; iter++ )
      {
         ...
         designParams.Preqd = designParams.fpre/(1.0/Ag + ecc/S);
         LOG(_T("Required prestress force (") << designParams.strLimitState << _T(") = ") << ::ConvertFromSysUnits(designParams.fpre,unitMeasure::KSI) << _T("/[ 1/") << ::ConvertFromSysUnits(Ag,unitMeasure::Inch2) << _T(" + ") << ::ConvertFromSysUnits(ecc,unitMeasure::Inch) << _T("/") << ::ConvertFromSysUnits(S,unitMeasure::Inch3) << _T("] = ") << ::ConvertFromSysUnits(-designParams.Preqd,unitMeasure::Kip) << _T(" Kip"));
      ...
      if ( controllingParams.stress_type == pgsTypes::Compression )
      {
         ...
         LOG(_T("Stress due to prestressing required at top of girder to make tension control = P/Ag + Pe/Stg, where P (") << controllingParams.strLimitState << _T(") = P (Service III)"));
         ...
         Float64 c = pAllowStress->GetAllowableCompressionStressCoefficient(poi,pTensionDesignParameters->stress_location,pTensionDesignParameters->intervalIdx,pTensionDesignParameters->limit_state);
         ...
         bool bFcUpdated = m_StrandDesignTool.UpdateConcreteStrength(fc,pTensionDesignParameters->intervalIdx,controllingParams.limit_state,controllingParams.stress_type,controllingParams.stress_location);
         ...
         else
         {
            ...
            else
            {
               ...
               Float64 fBotPre = pPsStress->GetDesignStress(pTensionDesignParameters->intervalIdx,pTensionDesignParameters->limit_state,poi,pTensionDesignParameters->stress_location,config);
               Float64 fc_rq;
               ...
               if (fc_rqd <= max_girder_fc)
               {
                  ...
                  if ( bFcUpdated )
                  {
                     ...
                     if (fc_curr-fci_curr > fc_2k)
                     {
                        ...
                        m_StrandDesignTool.UpdateReleaseStrength(fci_curr+fc_2k, strength_result, pTensionDesignParameters->intervalIdx,pTensionDesignParameters->limit_state,pTensionDesignParameters->stress_type,pTensionDesignParameters->stress_location);
                        m_DesignerOutcome.SetOutcome(pgsDesignCodes::FciChanged);
                     }
...
pgsPointOfInterest pgsDesigner2::GetControllingFinalMidZonePoi(const CSegmentKey& segmentKey)
{
   ...
   GET_IFACE(ILimitStateForces,pForces);
   std::vector<pgsPointOfInterest> vPoi( m_StrandDesignTool.GetDesignPoi(liveLoadIntervalIdx,POI_ERECTED_SEGMENT) );
   ...
   for ( ; poiIter != poiIterEnd; poiIter++)
   {
      ...
      if (left_limit <= dfs && dfs <= rgt_limit)
      {
         ...
         if ( analysisType == pgsTypes::Envelope )
         {
            pForces->GetDesignStress(liveLoadIntervalIdx,pgsTypes::ServiceIII,poi,pgsTypes::BottomGirder,fc,startSlabOffset,endSlabOffset,pgsTypes::MaxSimpleContinuousEnvelope,&min,&max);
         }
         else
         {
            pForces->GetDesignStress(liveLoadIntervalIdx,pgsTypes::ServiceIII,poi,pgsTypes::BottomGirder,fc,startSlabOffset,endSlabOffset,analysisType == pgsTypes::Simple ? pgsTypes::SimpleSpan : pgsTypes::ContinuousSpan,&min,&max);
         }
...
void pgsDesigner2::DesignEndZoneReleaseStrength(IProgress* pProgress)
{
   ...
   for ( ; poiIter != poiIterEnd; poiIter++ )
   {
      ...
      Float64 fTopPretension, fBotPretension;
      fTopPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::TopGirder,config);
      fBotPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::BottomGirder,config);
...
void pgsDesigner2::DesignEndZoneReleaseHarping(const arDesignOptions& options, IProgress* pProgress)
{
   ...
   Float64 Stg = pSectProp->GetS(releaseIntervalIdx,vPOI[0],pgsTypes::TopGirder);
   Float64 Sbg = pSectProp->GetS(releaseIntervalIdx,vPOI[0],pgsTypes::BottomGirder);
   LOG(_T("Ag  = ") << ::ConvertFromSysUnits(Ag, unitMeasure::Inch2) << _T(" in^2"));
   LOG(_T("Stg = ") << ::ConvertFromSysUnits(Stg,unitMeasure::Inch3) << _T(" in^3"));
   LOG(_T("Sbg = ") << ::ConvertFromSysUnits(Sbg,unitMeasure::Inch3) << _T(" in^3"));
   ...
   GET_IFACE(IAllowableConcreteStress,pAllowable);
   Float64 allowable_tension     = pAllowable->GetSegmentAllowableTensionStress(    vPOI[0],releaseIntervalIdx,pgsTypes::ServiceI,fci,conc_res==ConcSuccessWithRebar?true:false);
   Float64 allowable_compression = pAllowable->GetSegmentAllowableCompressionStress(vPOI[0],releaseIntervalIdx,pgsTypes::ServiceI,fci);
   LOG(_T("Allowable tensile stress after Release     = ") << ::ConvertFromSysUnits(allowable_tension,unitMeasure::KSI) << _T(" KSI") );
   ...
   fTopPs = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,top_poi,pgsTypes::TopGirder,config);
   fBotPs = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,bot_poi,pgsTypes::BottomGirder,config);
...
std::vector<DebondLevelType> pgsDesigner2::DesignEndZoneReleaseDebonding(IProgress* pProgress,bool bAbortOnFail)
{
   ...
   Float64 allowable_tension     = pAllowable->GetSegmentAllowableTensionStress(    dummyPOI,releaseIntervalIdx,pgsTypes::ServiceI,fci,rebar_reqd==ConcSuccessWithRebar?true:false);
   Float64 allowable_compression = pAllowable->GetSegmentAllowableCompressionStress(dummyPOI,releaseIntervalIdx,pgsTypes::ServiceI,fci);
   ...
   GET_IFACE(IPretensionForce,pPrestressForce);
   Float64 strand_force = pPrestressForce->GetPrestressForcePerStrand(sample_poi, pgsTypes::Straight, releaseIntervalIdx, pgsTypes::Start, pgsTypes::ServiceI, config );

   StrandIndexType nss = config.PrestressConfig.GetStrandCount(pgsTypes::Straight);
...
void pgsDesigner2::DesignForLiftingHarping(const arDesignOptions& options, bool bProportioningStrands,IProgress* pProgress)
{
   ...
   if ( bProportioningStrands )
   {
      ...
      artifact.GetGirderStress(vPoi,true, true,fHpTopMin,fHpBotMin);
      artifact.GetGirderStress(vPoi,false,true,fHpTopMax,fHpBotMax);
      ...
      pgsPointOfInterest tpoi(pPoi->GetPointOfInterest(segmentKey,top_loc));
      pgsPointOfInterest bpoi(pPoi->GetPointOfInterest(segmentKey,bot_loc));

      // Get the section properties of the girder
      Float64 Agt = pSectProp->GetAg(liftSegmentIntervalIdx,tpoi);
      Float64 Agb = pSectProp->GetAg(liftSegmentIntervalIdx,bpoi);
      Float64 Stg = pSectProp->GetS(liftSegmentIntervalIdx, tpoi, pgsTypes::TopGirder);
      Float64 Sbg = pSectProp->GetS(liftSegmentIntervalIdx, bpoi, pgsTypes::BottomGirder);
...
std::vector<DebondLevelType> pgsDesigner2::DesignDebondingForLifting(HANDLINGCONFIG& liftConfig, IProgress* pProgress)
{
...

   {
      ...
      StrandIndexType nss = liftConfig.GdrConfig.PrestressConfig.GetStrandCount(pgsTypes::Straight);
      StrandIndexType nts = nss + liftConfig.GdrConfig.PrestressConfig.GetStrandCount(pgsTypes::Harped) + liftConfig.GdrConfig.PrestressConfig.GetStrandCount(pgsTypes::Temporary); // to get an average force per strand
...
void pgsDesigner2::RefineDesignForAllowableStress(IProgress* pProgress)
{
   ...
   BOOST_FOREACH(StressCheckTask& task,m_StressCheckTasks)
   {
      if ( !pAllowable->IsStressCheckApplicable(segmentKey,task.intervalIdx,task.limitState,task.stressType) )
      {
...
void pgsDesigner2::RefineDesignForAllowableStress(const StressCheckTask& task,IProgress* pProgress)
{
   const CSegmentKey& segmentKey = m_StrandDesignTool.GetSegmentKey();
   ...
   {
      fAllow = pAllowable->GetSegmentAllowableTensionStress(dummyPOI,task.intervalIdx,task.limitState,fcgdr,false);
   }
   ...
   PoiAttributeType refAttrib = (task.intervalIdx < erectSegmentIntervalIdx ? POI_RELEASED_SEGMENT : POI_ERECTED_SEGMENT);
   std::vector<pgsPointOfInterest> vPoi = m_StrandDesignTool.GetDesignPoi(task.intervalIdx,refAttrib);
   
   std::vector<pgsPointOfInterest> morePoi = m_StrandDesignTool.GetDesignPoi(task.intervalIdx,POI_PSXFER);
   vPoi.insert(vPoi.end(),morePoi.begin(),morePoi.end());

   morePoi = m_StrandDesignTool.GetDesignPoi(task.intervalIdx,POI_CONCLOAD);
   vPoi.insert(vPoi.end(),morePoi.begin(),morePoi.end());

   morePoi = m_StrandDesignTool.GetDesignPoi(task.intervalIdx,POI_DIAPHRAGM);
   vPoi.insert(vPoi.end(),morePoi.begin(),morePoi.end());

   if (m_StrandDesignTool.GetFlexuralDesignType() == dtDesignForHarping)
   {
      morePoi = m_StrandDesignTool.GetDesignPoi(task.intervalIdx,POI_HARPINGPOINT);
      vPoi.insert(vPoi.end(),morePoi.begin(),morePoi.end());
   }
   else
   {
      // debonding needs more points
      morePoi = m_StrandDesignTool.GetDesignPoi(task.intervalIdx,POI_H);
      vPoi.insert(vPoi.end(),morePoi.begin(),morePoi.end());
   }
   ...
   for ( ; poiIter != poiIterEnd; poiIter++ )
   {
      ...
      //
      Float64 fTopPre = pPsStress->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::TopGirder,config);
      Float64 fBotPre = pPsStress->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::BottomGirder,config);
...
void pgsDesigner2::RefineDesignForUltimateMoment(IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,IProgress* pProgress)
{
   const CSegmentKey& segmentKey = m_StrandDesignTool.GetSegmentKey();

   std::vector<pgsPointOfInterest> vPoi( m_StrandDesignTool.GetDesignPoi(intervalIdx,POI_ERECTED_SEGMENT) );
   ...
   for ( ; poiIter != poiIterEnd; poiIter++ )
   {
      ...
      Float64 check_loss = pILosses->GetPrestressLoss(poi,pgsTypes::Permanent,pIntervals->GetLiveLoadInterval(segmentKey),pgsTypes::Start,pgsTypes::ServiceIII,config);
...
// Stirrup Design
void pgsDesigner2::DesignShear(pgsSegmentDesignArtifact* pArtifact, bool bDoStartFromScratch, bool bDoDesignFlexure)
{
