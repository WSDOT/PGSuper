...
void pgsDesigner2::GetHaunchDetails(const CSpanKey& spanKey,HAUNCHDETAILS* pHaunchDetails)
{
   GDRCONFIG dummy_config;
   GetHaunchDetails(spanKey,false,dummy_config,pHaunchDetails);
}

void pgsDesigner2::GetHaunchDetails(const CSpanKey& spanKey,const GDRCONFIG& config,HAUNCHDETAILS* pHaunchDetails)
{
...
const stbLiftingCheckArtifact* pgsDesigner2::GetLiftingCheckArtifact(const CSegmentKey& segmentKey)
{
   std::map<CSegmentKey,stbLiftingCheckArtifact>::iterator found;
   found = m_LiftingCheckArtifacts.find(segmentKey);
...
const stbLiftingCheckArtifact* pgsDesigner2::CheckLifting(const CSegmentKey& segmentKey)
{
...
const pgsGirderArtifact* pgsDesigner2::Check(const CGirderKey& girderKey)
{
   ...
   for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
   {
      ...
      std::vector<pgsPointOfInterest> storagePois( pPoi->GetPointsOfInterest(segmentKey,POI_STORAGE_SEGMENT) );
      ...
      std::sort(storagePois.begin(),storagePois.end());
      std::sort(erectedPois.begin(),erectedPois.end());
      std::sort(spanPois.begin(),spanPois.end());

      // remove any duplicates
      releasePois.erase(std::unique(releasePois.begin(),releasePois.end()),releasePois.end());
      storagePois.erase(std::unique(storagePois.begin(),storagePois.end()),storagePois.end());
      erectedPois.erase(std::unique(erectedPois.begin(),erectedPois.end()),erectedPois.end());
      spanPois.erase(std::unique(spanPois.begin(),spanPois.end()),spanPois.end());
      ...
      pProgress->UpdateMessage(_T("Checking segment stability"));
...
pgsGirderDesignArtifact pgsDesigner2::Design(const CGirderKey& girderKey,const std::vector<arDesignOptions>& desOptionsColl)
{
   // The design artifact
   ASSERT_GIRDER_KEY(girderKey);
...
void pgsDesigner2::DoDesign(const CGirderKey& girderKey,const arDesignOptions& options, pgsGirderDesignArtifact& girderDesignArtifact)
{
#if defined _DEBUG
...
void pgsDesigner2::CheckSegmentStresses(const CSegmentKey& segmentKey,const std::vector<pgsPointOfInterest>& vPoi,const StressCheckTask& task,pgsSegmentArtifact* pSegmentArtifact)
{
   ...
   BOOST_FOREACH(const pgsPointOfInterest& poi,vPoi)
   {
      ...
      for ( int i = 0; i < nElementsToCheck; i++ )
      {
         ...
         pPretensionStresses->GetStress(pretensionIntervalIdx,poi,topStressLocation,botStressLocation,task.bIncludeLiveLoad,&fPretension[TOP],&fPretension[BOT]);
...
void pgsDesigner2::ComputeConcreteStrength(pgsFlexuralStressArtifact& artifact,pgsTypes::StressLocation stressLocation,const pgsPointOfInterest& poi,const StressCheckTask& task)
{
...
void pgsDesigner2::CheckSegmentStressesAtRelease(const CSegmentKey& segmentKey, const GDRCONFIG* pConfig,pgsTypes::StressType type, pgsSegmentArtifact* pSegmentArtifact)
{
   ...
   BOOST_FOREACH(const pgsPointOfInterest& poi,vPoi)
   {
      ...
      if (pConfig == NULL)
      {
         fTopPretension = pPretensionStresses->GetStress(task.intervalIdx,poi,pgsTypes::TopGirder,task.bIncludeLiveLoad);
         fBotPretension = pPretensionStresses->GetStress(task.intervalIdx,poi,pgsTypes::BottomGirder,task.bIncludeLiveLoad);
      }
      else
      {
         fTopPretension = pPretensionStresses->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::TopGirder,*pConfig,task.bIncludeLiveLoad);
         fBotPretension = pPretensionStresses->GetDesignStress(task.intervalIdx,task.limitState,poi,pgsTypes::BottomGirder,*pConfig,task.bIncludeLiveLoad);
      }
...
void pgsDesigner2::CreateFlexuralCapacityArtifact(const pgsPointOfInterest& poi,IntervalIndexType intervalIdx,pgsTypes::LimitState limitState,bool bPositiveMoment,const MOMENTCAPACITYDETAILS& mcd,const MINMOMENTCAPDETAILS& mmcd,bool bDesign,pgsFlexuralCapacityArtifact* pArtifact)
{
   ...
   if ( bPositiveMoment )
   {
      ...
      if ( bDesign )
      {
         ...
         GET_IFACE(IProductForces,pProductForces);
         Float64 dMslab     = pProductForces->GetDesignSlabMomentAdjustment(config,poi);
         Float64 dMslab_pad = pProductForces->GetDesignSlabPadMomentAdjustment(config,poi);
                                                    ...
                                                    pgsStirrupCheckAtPoisArtifact* pArtifact)
{
   ...
   if (IsDeepSection( poi ))
   {
      ...
      std::_tostringstream os;
      os << _T("Cannot perform shear check. The Span-to-Depth ratio is less than ")<< MIN_SPAN_DEPTH_RATIO <<_T(" for Span ")<< LABEL_SPAN(spanKey.spanIndex) << _T(" Girder ")<< LABEL_GIRDER(spanKey.girderIndex) << _T(" (See LRFD 5.8.1.1)");
                                      ...
                                      pgsLongReinfShearArtifact* pArtifact )
{
   ...
   if ( equation == 1 )
   {
      ...
      {
         pMomentCapacity->GetMomentCapacityDetails(intervalIdx,poi,*pConfig,scd.bTensionBottom,&mcd);
      }
...
void pgsDesigner2::CheckSplittingZone(const CSegmentKey& segmentKey,const GDRCONFIG* pConfig,pgsStirrupCheckArtifact* pStirrupArtifact)
{
   ...
   if (pConfig == NULL)
   {
      ...
      start_dFpT = pLosses->GetEffectivePrestressLoss( start_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End);

      end_Fpj  = pPrestressForce->GetEffectivePrestress(end_poi, pgsTypes::Permanent, jackingIntervalIdx, pgsTypes::Start);
      end_dFpT = pLosses->GetEffectivePrestressLoss( end_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End);
   }
   else
   {
      start_Fpj  = pPrestressForce->GetEffectivePrestress(start_poi, pgsTypes::Permanent, jackingIntervalIdx, pgsTypes::Start, *pConfig);
      start_dFpT = pLosses->GetEffectivePrestressLoss( start_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End, *pConfig);

      end_Fpj  = pPrestressForce->GetEffectivePrestress(end_poi, pgsTypes::Permanent, jackingIntervalIdx, pgsTypes::Start, *pConfig);
      end_dFpT = pLosses->GetEffectivePrestressLoss( end_poi, pgsTypes::Permanent, releaseIntervalIdx, pgsTypes::End, *pConfig);
   }

   StrandIndexType Ns, Nh, Nt;
   StrandIndexType Nsd_start, Nhd_start, Ntd_start;
   StrandIndexType Nsd_end, Nhd_end, Ntd_end;
   if ( pConfig == NULL )
   {
      ...
      Nsd_start = pStrandGeometry->GetNumDebondedStrands(segmentKey,pgsTypes::Straight,pgsTypes::dbetStart);
      Nhd_start = pStrandGeometry->GetNumDebondedStrands(segmentKey,pgsTypes::Harped,pgsTypes::dbetStart);
      Ntd_start = pStrandGeometry->GetNumDebondedStrands(segmentKey,pgsTypes::Temporary,pgsTypes::dbetStart);

      Nsd_end = pStrandGeometry->GetNumDebondedStrands(segmentKey,pgsTypes::Straight,pgsTypes::dbetEnd);
      Nhd_end = pStrandGeometry->GetNumDebondedStrands(segmentKey,pgsTypes::Harped,pgsTypes::dbetEnd);
      Ntd_end = pStrandGeometry->GetNumDebondedStrands(segmentKey,pgsTypes::Temporary,pgsTypes::dbetEnd);
   }
   else
   {
      ...
      Nsd_start = pConfig->PrestressConfig.Debond[pgsTypes::Straight].size();
      Nhd_start = pConfig->PrestressConfig.Debond[pgsTypes::Harped].size();
      Ntd_start = pConfig->PrestressConfig.Debond[pgsTypes::Temporary].size();
   ...
   Float64 Aps_start = (Ns - Nsd_start)*pMat->GetStrandMaterial(segmentKey,pgsTypes::Straight)->GetNominalArea();
   Aps_start +=        (Nh - Nhd_start)*pMat->GetStrandMaterial(segmentKey,pgsTypes::Harped)->GetNominalArea();
   Aps_start +=        (Nt - Ntd_start)*pMat->GetStrandMaterial(segmentKey,pgsTypes::Temporary)->GetNominalArea();

   pArtifact->SetStartAps(Aps_start);
   pArtifact->SetStartFpj(start_Fpj);
   pArtifact->SetStartLossesAfterTransfer(start_dFpT);

   Float64 Aps_end = (Ns - Nsd_end)*pMat->GetStrandMaterial(segmentKey,pgsTypes::Straight)->GetNominalArea();
   Aps_end +=        (Nh - Nhd_end)*pMat->GetStrandMaterial(segmentKey,pgsTypes::Harped)->GetNominalArea();
   Aps_end +=        (Nt - Ntd_end)*pMat->GetStrandMaterial(segmentKey,pgsTypes::Temporary)->GetNominalArea();
   pArtifact->SetEndAps(Aps_end);
...
void pgsDesigner2::CheckConstructability(const CGirderKey& girderKey,pgsConstructabilityArtifact* pArtifact)
{
   ...
   // Constructability check is for all spans in group
   for( SpanIndexType spanIdx = startSpanIdx; spanIdx <= endSpanIdx; spanIdx++)
   {
      // artifact for each span
      pgsSpanConstructabilityArtifact artifact;
   ...
   #pragma Reminder("Assumes constant slab thickness throughout bridge")
      ...
      pArtifact->AddSpanArtifact(spanIdx,artifact);
   }
}
...
void pgsDesigner2::DesignMidZoneFinalConcrete(IProgress* pProgress)
{
   ...
   vConcreteStrengthParameters.push_back(ConcreteStrengthParameters(pgsTypes::ServiceI,_T("Service I (BSS2)"),railingSystemIntervalIdx,false,pgsTypes::Compression,pgsTypes::TopGirder,POI_HARPINGPOINT|POI_PSXFER));

   GET_IFACE(IAllowableConcreteStress,pAllowable);
   if ( pAllowable->CheckTemporaryStresses() )
   {
      vConcreteStrengthParameters.push_back(ConcreteStrengthParameters(pgsTypes::ServiceI,_T("Service I (BSS1)"),castDeckIntervalIdx,false,pgsTypes::Compression,pgsTypes::TopGirder,(POI_SPAN | POI_5L)));
   }

   vConcreteStrengthParameters.push_back(ConcreteStrengthParameters(pgsTypes::ServiceI,_T("Service I (BSS2)"),railingSystemIntervalIdx,false,pgsTypes::Compression,pgsTypes::BottomGirder,(POI_SPAN | POI_5L)));
   ...
   for ( ; iter != end; iter++ )
   {
      ...
      for ( ; poiIter != poiIterEnd; poiIter++)
      {
         ...
         if (concParams.stress_type == pgsTypes::Tension)
         {
            ...
            {
               concParams.fmax = max;
               concParams.fbpre = pPrestress->GetDesignStress(concParams.intervalIdx,concParams.limit_state,poi,concParams.stress_location,config,concParams.bIncludeLiveLoad);
...
void pgsDesigner2::DesignMidZoneAtRelease(const arDesignOptions& options, IProgress* pProgress)
{
   ...
   for ( ; poiIter != poiIterEnd; poiIter++)
   {
      ...
      fBotPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::BottomGirder,config,false);
   ...
   for ( ; poiIter != poiIterEnd; poiIter++)
   {
      ...
      Float64 fTopPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::TopGirder,config,false);
...
void pgsDesigner2::DesignMidZoneInitialStrands(bool bUseCurrentStrands,IProgress* pProgress)
{
   ...
   LOG(_T("dM slab       = ") << ::ConvertFromSysUnits(pProductForces->GetDesignSlabMomentAdjustment(m_StrandDesignTool.GetSegmentConfiguration(),poi),unitMeasure::KipFeet) << _T(" k-ft"));
   ...
   LOG(_T("dM slab pad   = ") << ::ConvertFromSysUnits(pProductForces->GetDesignSlabPadMomentAdjustment(m_StrandDesignTool.GetSegmentConfiguration(),poi),unitMeasure::KipFeet) << _T(" k-ft"));
      ...
      if ( controllingParams.stress_type == pgsTypes::Compression )
      {
         ...
         else
         {
            ...
            else
            {
               ...
               Float64 fBotPre = pPsStress->GetDesignStress(pTensionDesignParameters->intervalIdx,pTensionDesignParameters->limit_state,poi,pTensionDesignParameters->stress_location,config,pTensionDesignParameters->bIncludeLiveLoad);
               Float64 fc_rq;
...
void pgsDesigner2::DesignEndZoneReleaseStrength(IProgress* pProgress)
{
   ...
   for ( ; poiIter != poiIterEnd; poiIter++ )
   {
      ...
      Float64 fTopPretension, fBotPretension;
      fTopPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::TopGirder,config,false);
      fBotPretension = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,poi,pgsTypes::BottomGirder,config,false);
...
void pgsDesigner2::DesignEndZoneReleaseHarping(const arDesignOptions& options, IProgress* pProgress)
{
   ...
   fTopPs = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,top_poi,pgsTypes::TopGirder,config,false);
   fBotPs = pPrestress->GetDesignStress(releaseIntervalIdx,pgsTypes::ServiceI,bot_poi,pgsTypes::BottomGirder,config,false);
...
std::vector<DebondLevelType> pgsDesigner2::DesignEndZoneReleaseDebonding(IProgress* pProgress,bool bAbortOnFail)
{
   ...
   GET_IFACE(IPretensionForce,pPrestressForce);
   Float64 strand_force = pPrestressForce->GetPrestressForcePerStrand(sample_poi, pgsTypes::Straight, releaseIntervalIdx, pgsTypes::End, config );
...
const stbLiftingStabilityProblem* SectionFinder::pStabilityProblem = NULL;
...
void pgsDesigner2::GetEndZoneMinMaxRawStresses(const CSegmentKey& segmentKey,const stbLiftingResults& liftingResults,const HANDLINGCONFIG& liftConfig,Float64* pftop, Float64* pfbot, Float64* ptop_loc,Float64* pbot_loc)
{
   GET_IFACE(IPretensionForce,pPrestressForce);
...
void pgsDesigner2::DesignForShipping(IProgress* pProgress)
{
   ...
   do
   {
      ...
      std::auto_ptr<pgsHaulingAnalysisArtifact> artifact ( hauling_checker->DesignHauling(segmentKey,haulConfig,m_bShippingDesignWithEqualCantilevers,m_bShippingDesignIgnoreConfigurationLimits,pPoiLd,&bResult,LOGGER));
...
void pgsDesigner2::RefineDesignForAllowableStress(const StressCheckTask& task,IProgress* pProgress)
{
   ...
   if ( m_StrandDesignTool.IsDesignHarping() )
   {
      ...
      vPoi = m_StrandDesignTool.GetDesignPoi(intervalIdx,refAttrib | POI_5L);
   
      std::vector<pgsPointOfInterest> morePoi = m_StrandDesignTool.GetDesignPoi(intervalIdx,POI_PSXFER | POI_CONCLOAD | POI_HARPINGPOINT);
      vPoi.insert(vPoi.end(),morePoi.begin(),morePoi.end());
   }
   else
   {
      PoiAttributeType refAttrib = (intervalIdx < erectSegmentIntervalIdx ? POI_RELEASED_SEGMENT : POI_ERECTED_SEGMENT);
      vPoi = m_StrandDesignTool.GetDesignPoi(intervalIdx,refAttrib);

      std::vector<pgsPointOfInterest> morePoi = m_StrandDesignTool.GetDesignPoi(intervalIdx,POI_H | POI_PSXFER | POI_DEBOND);
      vPoi.insert(vPoi.end(),morePoi.begin(),morePoi.end());
   }
   ...
   for ( ; poiIter != poiIterEnd; poiIter++ )
   {
      ...
      //
      Float64 fTopPre = pPsStress->GetDesignStress(intervalIdx,task.limitState,poi,pgsTypes::TopGirder,config,bIncludeLiveLoad);
      Float64 fBotPre = pPsStress->GetDesignStress(intervalIdx,task.limitState,poi,pgsTypes::BottomGirder,config,bIncludeLiveLoad);
      ...
      case pgsTypes::Tension:
         ...
         if ( fBotMax < fTopMax && (intervalIdx == releaseIntervalIdx   || intervalIdx == liftSegmentIntervalIdx || 
                                    intervalIdx == tsRemovalIntervalIdx || intervalIdx == castDeckIntervalIdx) )
         {
...
void pgsDesigner2::DumpLiftingArtifact(const stbLiftingStabilityProblem* pStabilityProblem,const stbLiftingCheckArtifact& artifact,dbgDumpContext& os)
{
