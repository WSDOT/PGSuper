...
pgsHaulingAnalysisArtifact* pgsWsdotGirderHaulingChecker::CheckHauling(SpanIndexType span,GirderIndexType gdr, SHARED_LOGFILE LOGFILE)
{
   ...
   if (pGirderHaulingSpecCriteria->IsHaulingCheckEnabled())
   {
      ...
      return AnalyzeHauling(span,gdr);
   }
   else
   {
      return NULL;
...
pgsHaulingAnalysisArtifact*  pgsWsdotGirderHaulingChecker::AnalyzeHauling(SpanIndexType span,GirderIndexType gdr)
{
...
pgsHaulingAnalysisArtifact* pgsWsdotGirderHaulingChecker::AnalyzeHauling(SpanIndexType span,GirderIndexType gdr,const HANDLINGCONFIG& haulConfig,IGirderHaulingDesignPointsOfInterest* pPOId)
{
   std::auto_ptr<pgsWsdotHaulingAnalysisArtifact> pArtifact(new pgsWsdotHaulingAnalysisArtifact());
...
void pgsWsdotGirderHaulingChecker::AnalyzeHauling(SpanIndexType span,GirderIndexType gdr,bool bUseConfig,const HANDLINGCONFIG& haulConfig,IGirderHaulingDesignPointsOfInterest* pPOId,pgsWsdotHaulingAnalysisArtifact* pArtifact)
{
   ...
   if ( bUseConfig )
   {
      ...
      poi_vec = pPOId->GetHaulingDesignPointsOfInterest(span,gdr,10,Loh,Roh,POI_FLEXURESTRESS);
   }
   else
   {
...
pgsHaulingAnalysisArtifact* pgsWsdotGirderHaulingChecker::DesignHauling(SpanIndexType span,GirderIndexType gdr,const GDRCONFIG& config,bool bDesignForEqualOverhangs,bool bIgnoreConfigurationLimits,IGirderHaulingDesignPointsOfInterest* pPOId, bool* bSuccess, SHARED_LOGFILE LOGFILE)
{
...
// hauling
////////////////////////////////////////////////////////
void pgsWsdotGirderHaulingChecker::PrepareHaulingAnalysisArtifact(SpanIndexType span,GirderIndexType gdr,Float64 Loh,Float64 Roh,Float64 Fc,Float64 Ec,pgsTypes::ConcreteType concType,pgsWsdotHaulingAnalysisArtifact* pArtifact)
{
   GET_IFACE(IBridge, pBridge);
...
void pgsWsdotGirderHaulingChecker::ComputeHaulingStresses(SpanIndexType span,GirderIndexType gdr,bool bUseConfig,
                                                      ...
                                                      pgsWsdotHaulingAnalysisArtifact* pArtifact)
{
   ...
   // Get allowable compression and tension for with and without rebar cases
   Float64 fLowTensAllowable, fHighTensAllowable , fCompAllowable;
   if (!bUseConfig)
   {
      fLowTensAllowable  = pGirderHaulingSpecCriteria->GetHaulingAllowableTensileConcreteStress(span,gdr);
      fHighTensAllowable = pGirderHaulingSpecCriteria->GetHaulingWithMildRebarAllowableStress(span,gdr);
      fCompAllowable     = pGirderHaulingSpecCriteria->GetHaulingAllowableCompressiveConcreteStress(span,gdr);
   }
   else
   {
   ...
   for(IndexType i=0; i<psiz; i++)
   {
      ...
      if ( bUseConfig )
      {
         hps_force = pPrestressForce->GetPrestressForce(poi,pgsTypes::Harped,pgsTypes::AtShipping,pgsTypes::ServiceI,haulConfig.GdrConfig);
         he = pStrandGeometry->GetHsEccentricity(poi,haulConfig.GdrConfig.PrestressConfig, &nfh);
         sps_force = pPrestressForce->GetPrestressForce(poi,pgsTypes::Straight,pgsTypes::AtShipping,pgsTypes::ServiceI,haulConfig.GdrConfig);
         se = pStrandGeometry->GetSsEccentricity(poi,haulConfig.GdrConfig.PrestressConfig,&nfs);
         tps_force = pPrestressForce->GetPrestressForce(poi,pgsTypes::Temporary,pgsTypes::AtShipping,pgsTypes::ServiceI,haulConfig.GdrConfig);
         te = pStrandGeometry->GetTempEccentricity(poi,haulConfig.GdrConfig.PrestressConfig,&nft);
      }
      else
      {
         hps_force = pPrestressForce->GetPrestressForce(poi,pgsTypes::Harped,pgsTypes::AtShipping,pgsTypes::ServiceI);
         he = pStrandGeometry->GetHsEccentricity(poi,&nfh);
         sps_force = pPrestressForce->GetPrestressForce(poi,pgsTypes::Straight,pgsTypes::AtShipping,pgsTypes::ServiceI);
         se = pStrandGeometry->GetSsEccentricity(poi,&nfs);
         tps_force = pPrestressForce->GetPrestressForce(poi,pgsTypes::Temporary,pgsTypes::AtShipping,pgsTypes::ServiceI);
...
void pgsWsdotGirderHaulingChecker::ComputeHaulingFsForCracking(SpanIndexType span,GirderIndexType gdr,
                                                           const std::vector<pgsPointOfInterest>& rpoiVec,
                                                           const std::vector<Float64>& momVec,
                                                           pgsWsdotHaulingAnalysisArtifact* pArtifact)
{
...
void pgsWsdotGirderHaulingChecker::ComputeHaulingFsForRollover(SpanIndexType span,GirderIndexType gdr,
                                                           pgsWsdotHaulingAnalysisArtifact* pArtifact)
{
   Float64 zo = pArtifact->GetZo();
   CHECK(zo>0.0);
...
void pgsWsdotGirderHaulingChecker::ComputeHaulingMoments(SpanIndexType span,GirderIndexType gdr,
                                                     ...
                                                     std::vector<Float64>* pmomVec, Float64* pMidSpanDeflection)
{
   ...
   pgsGirderHandlingChecker::ComputeMoments(m_pBroker, &factory, span, gdr,
                                            pgsTypes::Hauling,
                                            leftOH, glen, rightOH,
                                            E,
                                            rpoiVec,
...
void pgsWsdotGirderHaulingChecker::ComputeHaulingRollAngle(SpanIndexType span,GirderIndexType gdr,
                                                       pgsWsdotHaulingAnalysisArtifact* pArtifact, 
                                                       const std::vector<pgsPointOfInterest> rpoiVec,
                                                       std::vector<Float64>* pmomVec, Float64* pMidSpanDeflection)
{
