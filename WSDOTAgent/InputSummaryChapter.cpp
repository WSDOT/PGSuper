   ...
   *p << color(Red) << _T("NOTE: Several details have been omitted from this report") << color(Black) << rptNewLine;
   ...
   for ( SegmentIndexType segIdx = 0; segIdx < nSegments; segIdx++ )
   {
      ...
      *pChapter << p;
      ...
      {
         *p << color(Red) << Bold(_T("Warning: This is a non-standard girder because it utilizies straight adjustable strands. WSDOT Standard Girders utilize harped strands.")) << color(Black) << rptNewLine;
      }
      ...
      {
         *p << color(Red) << Bold(_T("Warning: This is a non-standard girder because it utilizies Direct Strand Fill. WSDOT Standard Girders utilize sequentially filled strands.")) << color(Black) << rptNewLine;
      }
...
void girder_line_geometry(rptChapter* pChapter,IBroker* pBroker,const CSegmentKey& segmentKey,IEAFDisplayUnits* pDisplayUnits)
{
   ...
   *p << pTable << rptNewLine;
   ...
   (*pTable)(row++,1) << CPierData2::AsString(pPrevPier->GetBoundaryConditionType());
   ...
   (*pTable)(row++,1) << CPierData2::AsString(pNextPier->GetBoundaryConditionType());
   ...
   if (pSegmentLiftingSpecCriteria->IsLiftingAnalysisEnabled())
   {
      ...
      (*pTable)(row++,1) << length.SetValue(pSegmentLifting->GetLeftLiftingLoopLocation(segmentKey));

      (*pTable)(row,0) << _T("Right Lifting Point Location");
      (*pTable)(row++,1) << length.SetValue(pSegmentLifting->GetRightLiftingLoopLocation(segmentKey));
   }
   ...
   if (pSegmentHaulingSpecCriteria->IsHaulingAnalysisEnabled())
   {
      ...
      (*pTable)(row++,1) << length.SetValue(pSegmentHauling->GetLeadingOverhang(segmentKey));

      (*pTable)(row,0) << _T("Trailing Truck Support Location");
      (*pTable)(row++,1) << length.SetValue(pSegmentHauling->GetTrailingOverhang(segmentKey));
   }
}
...
void prestressing(rptChapter* pChapter,IBroker* pBroker,const CSegmentKey& segmentKey,IEAFDisplayUnits* pDisplayUnits)
{
   ...
   *p << pTable << rptNewLine;
   ...
   bool bIsSymmetric = pGirder->IsSymmetricSegment(segmentKey);
   ...
   if (are_harped_straight)
   {
      ...
      {
         (*pTable)(row,0) << _T("C.G. of ") << LABEL_HARP_TYPE(are_harped_straight)<< _T(" Strands from top at left end of girder");
         (*pTable)(row,1) << component.SetValue( Fo[pgsTypes::metStart] );
         row++;

         (*pTable)(row,0) << _T("C.G. of ") << LABEL_HARP_TYPE(are_harped_straight)<< _T(" Strands from top at right end of girder");
   ...
   if ( bIsSymmetric )
   {
      ...
      (*pTable)(row,1) << component.SetValue( E[pgsTypes::metStart] );
      row++;
   }
   else
   {
      (*pTable)(row,0) << _T("C.G. of Straight Strands at left end, E");
      (*pTable)(row,1) << component.SetValue( E[pgsTypes::metStart] );
      row++;

      (*pTable)(row,0) << _T("C.G. of Straight Strands at right end, E");
      (*pTable)(row,1) << component.SetValue( E[pgsTypes::metEnd] );
      row++;
   }
}
