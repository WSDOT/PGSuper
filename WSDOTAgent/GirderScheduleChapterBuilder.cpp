...
#pragma Reminder("UPDATE: assuming precast girder bridge")
   ...
   StrandIndexType NhMax = pStrandGeometry->GetMaxStrands(segmentKey,pgsTypes::Harped);
   ...
   *p << p_table;
   ...
   else
   {
      ...
      (*p_table)(row,  1) << pGirder->GetGirderName();
   }
   ...
   (*p_table)(row  ,1) << stress.SetValue(pMaterial->GetSegmentFc(segmentKey,finalIntervalIdx));
