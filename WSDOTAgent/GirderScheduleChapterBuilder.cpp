    ...
    const std::shared_ptr<const WBFL::Reporting::ReportSpecification>& pRptSpec, Uint16 level) const
{
   ...
   EAF_EAF_GET_IFACE2(pBroker,ISectionProperties,pSectProp);
   EAF_EAF_GET_IFACE2(pBroker,IEAFDisplayUnits,pDisplayUnits);
   EAF_EAF_GET_IFACE2(pBroker,IIntervals,pIntervals);
   ...
   EAF_EAF_GET_IFACE2( pBroker, IStrandGeometry, pStrandGeometry );
   ...
   EAF_EAF_GET_IFACE2(pBroker,IArtifact,pIArtifact);
   ...
   *pChapter << p;
   ...
   EAF_EAF_GET_IFACE2(pBroker,ICamber,pCamber);
   ...
   EAF_EAF_GET_IFACE2(pBroker, IPointOfInterest, pPointOfInterest );
   PoiList pmid;
   ...
   EAF_EAF_GET_IFACE2(pBroker,IBridge,pBridge);
   EAF_EAF_GET_IFACE2(pBroker,IGirder,pIGirder);
   ...
   *p << pTable;
   ...
   EAF_EAF_GET_IFACE2(pBroker, IMaterials, pMaterial);
   ...
   {
      EAF_EAF_GET_IFACE2(pBroker,ISegmentLifting,pSegmentLifting);
   ...
   if ( pHaulingArtifact != nullptr )
   {
      ...
      EAF_EAF_GET_IFACE2(pBroker,IProductForces,pProduct);
      pgsTypes::BridgeAnalysisType bat = pProduct->GetBridgeAnalysisType(pgsTypes::Minimize);

      EAF_EAF_GET_IFACE2(pBroker,ISegmentHauling, pSegmentHauling);
   ...
   *p << _T("*** ") << Sub2(_T("L"),_T("d")) << _T(" to be determined by the Designer.") << rptNewLine;
   ...
   {
       EAF_EAF_GET_IFACE2(pBroker, IPointOfInterest, pPoi);
...
{
   EAF_EAF_GET_IFACE2(pBroker,IStirrupGeometry,pStirrupGeometry);
   ...
   *pz3Length = zoneLength;
   ...
   else if (familyCLSID == CLSID_SlabBeamFamily)
   {
      ...
      EAF_EAF_GET_IFACE2(pBroker,IGirder, pGirder);
